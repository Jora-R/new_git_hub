bind("preMatch", function($context) {
    ...
    // sessionCrashNotification(); //для активации уведомлений в начале обращения (не повторяется внутри диалога)
});
...
function sessionCrashNotification() {
    if($jsapi.context().session.crashedNotification != true) {
        $jsapi.context().session.crashedNotification = true;
        $reactions.answer("В связи с введением США ограничительных мер в отношении Московской биржи, с 13 июня 2024 года приостановлено проведение биржевых торгов валютами доллар США, евро и гонконгский доллар. [Подробнее|https://www.moex.com/n70160?nt=0]");
        // $reactions.answer("С 14 июня на срочном, валютном и рынке драгоценных металлов ММВБ торги проводятся с 9:50 МСК. [Подробнее|https://www.moex.com/n70206?nt=0]");
    }
}
...
function checkNewDay() {
    ...
    if($jsapi.context().request.query != "/start" && $jsapi.context().request.query != "Подробнее" && $jsapi.context().request.query != "Перейдите по ссылке") {
        ...
        if($jsapi.context().client.currentDay != day){
            ...
            // $reactions.answer("В настоящий момент могут наблюдаться проблемы с выставлением заявок по ряду инструментов Фондового рынка со стороны Московской биржи.");
