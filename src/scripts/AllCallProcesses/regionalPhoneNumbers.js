...
function getOperatorFromNumberAndCurrentTime(additionalNumber){
    ...
    var transformDay = dayToDatType(day);
    
    // $reactions.answer(transformDay);
    if(!schedule[transformDay]){
        return'1000';
    } 
    var hoursStart = +moment(schedule[transformDay].startTime,'H:mm').format('H');
    var minutesStart = +moment(schedule[transformDay].startTime,'H:mm').format('mm');
    var hoursEnd = +moment(schedule[transformDay].endTime,'H:mm').format('H');
    var minutesEnd = +moment(schedule[transformDay].endTime,'H:mm').format('mm');
...
function dayToDatType(day){
    var time = $jsapi.timeForZone("Europe/Moscow");
    var date = currentDate();
    if(date.format("D/M/YYYY") == '27/4/2024'){
        return 'будние';
    }
    switch (day){
        case 'суббота':
