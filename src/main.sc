...
theme: /
    ...
    state: Открытие_закрытие_счета
        ...
        state: open_Банк
            ...
            a: Дистанционно подать заявку на открытие счета, можно в личном кабинете на сайте фина'м точка ру, кнопка открыть новый счет, находится под списком ваших открытых счетов. Далее выберите раздел, банковские продукты, далее выберите, карты, или тип счета.
        ...
        state: MoneyTransfer Инструкция вывод Реквизиты
            script:
                if (identificationAO($client.profileCRM) == "success"){
                    ...
                    $session.insrtuctionText = "В продолжение разговора направляю вам ссылку на [вывод средств по реквизитам в личном кабинете|https://edox.finam.ru/orders/MoneyOut/MoneyOut/Default.aspx]";
                    $reactions.transition("/Отправка инструкции в чат");
                }
            a: Хотите, я расскажу подробнее про комиссии?
            script: 
        ...
        state: MoneyTransfer Инструкция вывод Карта
            script:
                if (identificationAO($client.profileCRM) == "success"){
                    ...
                    $session.insrtuctionText = "В продолжение разговора направляю вам ссылку на [вывод средств по реквизитам карты в личном кабинете|https://edox.finam.ru/orders/MoneyOut/MoneyOut/Default.aspx]";
                    $reactions.transition("/Отправка инструкции в чат");
                }
            a: Хотите, я расскажу подробнее про комиссии?
            script: 
    ...
    state: Валюта_покупка|продажа
        ...
        state: Ответ_Брокер
            ...
            a: Клиентам фина'м доступна покупка и продажа долларов США, и гонконгских долларов, через сервис внебиржевых торгов.
    ...
    state: Яндекс
        ...
        a: Участвовать в обмене могут инвесторы, которые приобрели акции Яндекса на Московской или СПБ бирже. Брокер Фина'м автоматически проведет биржевой обмен акций Yandex эН Вэ для своих клиентов. Дополнительных действий от инвесторов не требуется.
        a: В связи с автоматическим обменом ценных бумаг, ранее поданные поручения будут отменены. Обратите внимание, заявки по биржевой оферте выставлены по месту хранения бумаг; при расторжении счёта ИИС — обмен не состоится.
        # script:
        #     if (identificationAO($client.profileCRM) == "success"){
        #         $context.session.lastState = $context.currentState;
    ...
    state: Элемент
        ...
        a: При необходимости продать актив, его можно перевести из эНэРДэ' в депозитарий СПБ; перевод доступен в личном кабинете, едо'кс точка фина'м точка ру, в разделе Услуги; Операции с ценными бумагами; Перемещение ценных бумаг.
    ...
    state: Престиж
        ...
        #         $context.session.lastState = $context.currentState;
        #         $session.questionText = "Хотите получить иллюстрированную инструкцию по пополнению счета в чат?";
        #         $session.insrtuctionText = "В продолжение разговора направляю вам ссылку на [пополнение брокерского счета в личном кабинете|https://www.finam.ru/dicwords/file/files_chatbot_instrukciyapopopolneniyuscheta]";
        #         $reactions.transition("/Отправка инструкции в чат");
        #     }
        a: Чем я могу еще помочь?
        script: 
