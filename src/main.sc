...
require: ./scripts/sendMessageTxchatHttp.js
# Функция проверки пренадлежит ли телефон клиенту АО
require: ./scripts/AOcheck.js
...
theme: /
    ...
    state: Авторизация_Логин и пароль
        ...
        state: FinamBonus
            ...
            script: 
                $context.session = {};
            q: @repeat_please * ||toState = "."
            q: @agree  ||toState = "/Авторизация_Логин и пароль/FinamRU"
            q: @disagree ||toState = "/Могу еще чем то помочь?/NO"
            # final answer
    ...
    state: Контакты_Почта
        ...
        state: Уточнение компании
            a: Информация о какой компании холдинга Фина'м, Вас интересует? Брокер; управляющая компания; Банк; или компания Форекс.
            q: @repeat_please * ||toState = "."
            state: Ожидание ответа
                q: * @company *
                script:
    ...
    state: Сегрегированные счета_Открытие счета
        ...
        state: Ответ_Брокер
            ...
            go!: /Сегрегированные счета_Открытие счета/Ответ_Брокер/Авторизация
            
            state: Авторизация
                script:
                    $session.resultPreSendMessageTxchat = preSendMessageTxchat();
            
                    if ($session.resultPreSendMessageTxchat == "error"){
                
                        //$reactions.answer("Функции отработали с ошибкой, НЕ предоставляем клиенту выбор прислать ли инструкцию");
                        $reactions.transition("/Сегрегированные счета_Открытие счета/Ответ_Брокер/Чем могу помочь?");

                    }
                    else {
                        $session.personGlobalID = $session.resultPreSendMessageTxchat;
                        $reactions.transition("/Сегрегированные счета_Открытие счета/Ответ_Брокер/Инструкция?");
                    }    
                
            state: Инструкция?
                a: Хотите получить иллюстрированную инструкцию по открытию счета в чат?
                q: @agree ||toState = "/Сегрегированные счета_Открытие счета/Ответ_Брокер/Инструкция?/Отправка инструкции"    
                q: @repeat_please * ||toState = "."
                q: @disagree ||toState = "/Сегрегированные счета_Открытие счета/Ответ_Брокер/Чем могу помочь?"
                
                state: Отправка инструкции
                    script:
                        $session.text = "В продолжение разговора направляю вам иллюстрированную [инструкцию по открытию Сегрегированного счета|https://www.finam.ru/dicwords/file/files_chatbot_instrukciysegregopen]"
                        $session.resultSendMessage = sendMessageTxchatHttp($session.personGlobalID, $session.text);
                        $reactions.transition("/Сегрегированные счета_Открытие счета/Ответ_Брокер/Чем могу помочь?");
                    
                
            state: Чем могу помочь?
                a: Чем я могу еще помочь?
                q: @repeat_please * ||toState = "/Сегрегированные счета_Открытие счета"
                q: @disagree ||toState = "/Могу еще чем то помочь?/NO"
            # повтор должен быть на весь ответ
            # # final answer
    ...
    state: Разблокировка ЦБ_Обмен по указу 844
        ...
        a: В рамках Указа номер 844; О дополнительных мерах экономического характера, связанных с обращением иностранных ценных бумаг; предполагается обмен заблокированных в России и за рубежом активов российских инвесторов. Общая стоимость бумаг, которые можно обменять, ограничена суммой в 100000 рублей.
        a: Обмену подлежат бумаги, хранящиеся в Национальном расчетном депозитарии энэрдэ'; то есть бумаги, заблокированные в мае 2022-го года. С 25 марта по 8 мая 2024 года, клиенты Фина'м могут подать заявку на обмен в личном кабинете.
        a: Для этого авторизуйтесь в личном кабинете на сайте фина'м точка ру; перейдите в раздел Помощь; слева выберите, перейти в старый дизайн. Далее перейдите в раздел Услуги, Операции с ценными бумагами. В этом же разделе вы найдете подробную информацию и вспомогательные материалы по теме выкупа ценных бумаг.
        go!: /Разблокировка ЦБ_Обмен по указу 844/Авторизация
        ...
        state: Авторизация
            script:
                ...
                if ($session.resultPreSendMessageTxchat == "error"){
                    ...
                    //$reactions.answer("Функции отработали с ошибкой, НЕ предоставляем клиенту выбор прислать ли инструкцию");
                    $reactions.transition("/Разблокировка ЦБ_Обмен по указу 844/Чем могу помочь?");

                }
                else {
                    $session.personGlobalID = $session.resultPreSendMessageTxchat;
                    $reactions.transition("/Разблокировка ЦБ_Обмен по указу 844/Инструкция?");
                }    
                
        state: Инструкция?
                a: Хотите получить в чат подробную инструкцию по подаче заявки на выкуп иностранных ценных бумаг в рамках 844 указа?
                q: @agree ||toState = "/Разблокировка ЦБ_Обмен по указу 844/Инструкция?/Отправка инструкции"    
                q: @repeat_please * ||toState = "."
                q: @disagree ||toState = "/Разблокировка ЦБ_Обмен по указу 844/Чем могу помочь?"
                
                state: Отправка инструкции
                    script:
                        $session.text = "В продолжение разговора направляю вам подробную иллюстрированную [инструкцию по подаче заявки на выкуп иностранных ценных бумаг в рамках 844 указа|https://www.finam.ru/publications/item/finam-nachal-priem-zayavok-na-obmen-zablokirovannymi-aktivami-20240325-1446/]"
                        $session.resultSendMessage = sendMessageTxchatHttp($session.personGlobalID, $session.text);
                        $reactions.transition("/Разблокировка ЦБ_Обмен по указу 844/Чем могу помочь?");
        ...
        state: Чем могу помочь?
                a: Чем я могу еще помочь?
                q: @repeat_please * ||toState = "/Разблокировка ЦБ_Обмен по указу 844"
                q: @disagree ||toState = "/Могу еще чем то помочь?/NO"
            # повтор должен быть на весь ответ
            # # final answer
    ...
    state: Оператор
        ...
        state: Оператор по номеру
            ...
            elseif: $session.operatorPhoneNumber == '3666'
                a: Чтобы получить персональную информацию по вашему брокерскому счету, подготовьте и назовите оператору ваш уникальный торговый код, который находится в личном кабинете в разделе Детали по счету. Перевожу вас на {{ $session.departmentName }}.
