...
theme: /
    ...
    state: Ввод добавочного
        ...
        state: Digits
            ...
            script:
                ...
                } else if(($request.query == "1806") || ($request.query == "2291") || ($request.query == "1342")){
                    $reactions.answer("Мне не удалось найти указанный номер");
        ...
        state: Заказ_w8_Брокер
            ...
            script:
             if (identificationAO($client.profileCRM) == "success"){
                    ...
                    $session.insrtuctionText = "В продолжение разговора направляю вам [ссылку для заказа Формы W-8BEN|https://lk.finam.ru/Orders/FormW8BEN]";
                    $reactions.transition("/Отправка инструкции в чат");
                }
            a: Чем я могу еще помочь?
            script: 
    ...
    state: MoneyTransfer || modal = true
        ...
        state: MoneyTransfer Комиссии вывод Реквизиты
            a: Вывод валюты рубль РФ без комиссии. Вывод и зачисление иностранной валюты на счета банка фина'м - без комиссии; комиссия за хранение валюты в банке от 3000 единиц валюты.
            a: За вывод иностранной валюты на счета в других банках российской федерации, взимаются следующие комиссии, За вывод в валюте доллар США — 0,3% от суммы вывода, но не менее 30 и не более 150 долларов, дополнительная комиссия за обработку поручений на вывод долларов сша: 0,4% от суммы вывода, но не менее 1500 и не более 250000 рублей.
            a: За вывод валют, Юань и Тенге' — 0,07% от суммы вывода, но не менее 25 € и не более 100 €. Дополнительно при оформлении вывода по реквизитам, можно воспользоваться опцией срочного вывода рублей рф. Деньги поступят на один день раньше. стоимость данной опции 300 рублей. по тарифам Дневной СПБ, Консультационный СПБ, Стратег Ю эС, комиссия 7 с половиной долларов. Ограничение по сумме срочного вывода, от 1000 до 5000000 рублей, но не более 80% от оценки вашего счета. 
            a: Обращаем ваше внимание, вывод происходит не ранее проведения биржевых расчётов по сделкам в соответствии с режимом торгов. Комиссии удерживаются в рублях, по курсу центробанка, на дату исполнения вывода.
            a: Чем я могу еще помочь?
            script: 
                $context.session = {};
            q: @repeat_please * ||toState = "."
        ...
        state: MoneyTransfer Инструкция вывод Карта
            script:
                if (identificationAO($client.profileCRM) == "success"){
                    ...
                    $session.insrtuctionText = "В продолжение разговора направляю вам ссылку на [вывод средств по реквизитам карты в личном кабинете|https://lk.finam.ru/orders/MoneyOut/MoneyOut/Default.aspx]";
                    $reactions.transition("/Отправка инструкции в чат");
                }
            a: Хотите, я расскажу подробнее про комиссии?
            script: 
    ...
    state: ИИС_Дата открытия
        ...
        state: Ответ_Брокер
            ...
            a: Если счет ИИС переведен от другого брокера, первичную дату открытия можно уточнить у менеджера фина'м, или в личном кабинете брокера на сайте, Фина'м точка ру, кликнув на счет с названием КЛФ ИИС. 
    ...
    state: ИИС 3
        ...
        a: Трансформация ИИС доступна в личном кабинете на сайте фина'м точка ру. В разделе личного кабинета Услуги; Прочие услуги. Договоры ИИС, открытые в период с 2024ого по 2026й год, нельзя закрыть раньше, чем через 5 лет.
    ...
    state: Маржинальная торговля
        ...
        a: Обращаем ваше внимание, в связи с исполнением заявок на обмен акций Яндекса, у держателей акций возможно временно некорректное отображение портфеля. Балансовая стоимость акций будет скорректирована с началом торгов на Московской бирже 24 июля. 
    ...
    state: Валюта_покупка|продажа
        ...
        state: Ответ_Брокер
            a: Обращаем ваше внимание, В связи с введением ограничительных мер в отношении Московской биржи, с 13 июня приостановлено проведение биржевых торгов валютами доллар США, евро, и гонконгский доллар. Со 2 июля ограничена торговля валютными парами с китайским юанем в режиме today. 
    ...
    state: Валюта_внебиржевая торговля
        ...
        a: Клиентам фина'м доступна покупка и продажа долларов США через сервис внебиржевых торгов.
        a: Менеджеры собирают заявки клиентов, и после окончания сбора заявок, и проведения необходимых расчетов, при наличии встречного поручения менеджер отдела поддержки перезванивает клиенту, и информирует, в каком объеме и по какой цене возможна сделка.
        a: Если параметры подходят, звонок будет переадресован в отдел голосового трейдинга. Клиенту нужно сообщить трейдеру свой торговый код и проговорить параметры выставляемой заявки. Без подтверждения заявки отделу голосового трейдинга, операция осуществлена не будет.
        a: Расчёты по конвертации долларов проходят до 19:00 по московскому времени. Комиссии за услуги голосового трейдинга: до 5 поручений в день — без комиссии, и за последующие — комиссия 236 рублей за поручение.
    ...
    state: Доступные биржи
        ...
        state: Гонконгская биржа
            a: С 13 июня на бирже Гонконга доступно только закрытие позиций.
    ...
    state: Время торгов
        ...
        state: Московская биржа
            ...
            state: Валютная секция
                ...
                a: Па'ра юань рубль торгуется до 11:50. Па'ры с белорусским рублём, турецкой лирой, и казахстанским тенге' к рублю эРэФ, торгуются до 11:45 по московскому времени.
                a: Чем я могу еще помочь?
                script:
                    # SessionClearing036();
                    $session.exchanges = undefined;
    ...
    state: Яндекс
        intent!: /058 Яндекс
        a: МК ПАО ЯНДЕКС - будущая головная компания Яндекса, к которой переходит большинство сервисов и активов Yandex эН Вэ. Для держателей Yandex эН Вэ, был проведен биржевой обмен акций.
        a: В связи с исполнением заявок на обмен, у держателей акций возможно временно некорректное отображение портфеля. Балансовая стоимость акций будет скорректирована с началом торгов на Московской бирже 24 июля под новым тикером Уай, Ди Ии X.
        a: Биржевой обмен проходит через проведение сделок продажи и покупки бумаг на бирже. Комиссии за сделки взимаются в соответствии с тарифным планом.
        a: Если комиссия списалась в минус, в валюте доллар США, то нужно пополнить счёт рублями на недостающую сумму для погашения минуса. После пополнения счёта, в течение нескольких дней валютный минус будет автоматически сконвертирован в рубли и погашен.
        a: Обратите внимание, что с изменением торгового кода акции яндекс, ранее выставленные стоп-заявки по инструменту со старым тикером не сработают. При необходимости нужно заново выставить стоп-заявки, указав новый тикер инструмента.
        # script:
        #     if (identificationAO($client.profileCRM) == "success"){
        #         $context.session.lastState = $context.currentState;
    ...
    state: Кодовое слово
        ...
        a: Для дополнительной защиты ваших персональных данных, в компании фина'м вводится обязательный способ идентификации клиентов; кодовое слово. С 1 сентября 2024 года клиенты компании смогут получить персональную информацию, или подать голосовую заявку, только после того как назовут сотруднику своё кодовое слово. 
        a: Создать, изменить, или удалить кодовое слово, можно в личном кабинете на сайте фина'м точка ру; авторизуйтесь в личном кабинете; в верхнем правом углу нажмите на значок персоны, и перейдите в раздел Учетные данные.
        # script:
        #     if (identificationAO($client.profileCRM) == "success"){
        #         $context.session.lastState = $context.currentState;
        #         $session.questionText = "Хотите я направлю ссылку на раздел Учётные данные, вам в чат?";
        #         $session.insrtuctionText = "Для дополнительной защиты ваших персональных данных, в компании «Финам» вводится обязательный способ идентификации клиентов - кодовое слово. С 1 сентября 2024 года клиенты компании смогут получить персональную информацию или подать голосовую заявку только после того, как назовут сотруднику своё кодовое слово. Создать, изменить или удалить кодовое слово можно в личном кабинете в [разделе «Учетные данные»|https://lk.finam.ru/user/credentials]";
        #         $reactions.transition("/Отправка инструкции в чат");
        #     }
        a: Чем я могу еще помочь?
        script: 
