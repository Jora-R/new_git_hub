...
theme: /
    ...
    state: Start
        ...
        # a: Возможны временные затруднения при доставке СМС для авторизации. Приносим извинения за доставленные неудобства. Повторите попытку позже.
    ...
    state: Санкции_СПБ_биржа
        ...
        a: Касательно выплат в рамках Указа номер 665, депозитарии используют рубли РФ для выплат по заблокированным активам. Выплаты первой очереди, поступившие брокеру от СПБ Биржи, уже перечислены инвесторам. В настоящий момент информации о предстоящих выплатах от биржи не поступало.
