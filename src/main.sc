...
require: name/name.sc
  module = sys.zb-common
require: scripts/fetch.js
    type = scriptEs6
    name = fetch 
# Получение профиля клиента в СРМ
require: scripts/profileCRM.js    
# Проверка нажата ли рассылка
require: scripts/mailing.js
...
require: scripts/technicalBreak.js
# Функция активирует текст в праздники
require: scripts/holidays.js
...
theme: /
    ...
    state: Сбой
        a: На данный момент ведутся работы по восстановлению работоспособности систем. Приносим извинения за доставленные неудобства. Вы можете продолжить диалог с виртуальным консультантом, либо обратится к оператору.
        buttons:
        ...
        state: Отправка запроса
            ...
            scriptEs6:
                ...
                if ($session.response_crm.ok == true){
                    ...
                    $analytics.setSessionResult("Отклик CRM");
                    $reactions.timeout({ interval: 2, targetState: "/Закрытие обращения БТ"});
                } else {
                    $analytics.setMessageLabel("Отклик не сформирован", "Ошибка");
    ...
    state: Рекомендации
        script:
            ...
            $session.url_profileCRM = "http://msa-vcbtu1-db01:3000/do_recommendations?globalid=eq." + $session.clientGlobalId + "&type_info=eq.product_fin&order=probability.desc&limit=1"; //Для теста
    
            # $session.url_profileCRM = "http://msa-vcbtu1-db01:3000/do_recommendations?globalid=eq." + $request.data.clientGlobalId + "&type_info=eq.product_fin&order=probability.desc&limit=1"; //Prod
        ...
        state: Предоставление рекомендации
            script:
                var difference = moment().diff($client.sendRecommendation,'minutes') || 0
                # log(difference)
                if($session.recommendation.name != null && (!$client.sendRecommendation || difference >=7)){
                    $client.sendRecommendation = moment()
                    $reactions.answer("Так же обратите внимание , что наш Искусственный интеллект считает, что Вам может быть интересна наша услуга: {{$session.recommendation.name}} {{$session.recommendation.landing}}.");
                    $analytics.setMessageLabel("Рекомендация отправлена", "Рекомендации");
        ...
        state: Error
            script:
                $analytics.setMessageLabel("Ошибка отправки рекомендации", "Рекомендации");
                $reactions.timeout({ interval: 2, targetState:  "/Закрытие обращения" });
            
    
    # state: Получение данных по профилю клиента функцмя
    #     q!: крякозябла
    #     script:
    #         profileCRM()
    
    # state: Получение данных по профилю клиента
    #     # q!: крякозябла
    #     script:
    #         # $session.clientGlobalId = "47a818f7-ce8b-4528-99a3-b166ca26dca0";
    #         $session.clientGlobalId = "46f99ae2-46ba-4cac-99fc-225b6d85790b";
    #         # $reactions.answer($session.clientGlobalId);
    #         # $reactions.answer($injector.url_profileCRM);
    #         $session.url_profileCRM = $injector.url_profileCRM + $session.clientGlobalId;
            
    #         # $session.url_profileCRM = $injector.url_profileCRM + $session.request.data.clientGlobalId;
    #         $reactions.timeout({ interval: 2, targetState: "/Получение данных по профилю клиента/Отправка запроса"});
        
    #     state: Отправка запроса
    #         HttpRequest:
    #             method = GET
    #             url = {{$session.url_profileCRM}}
    #             headers = [{ name: "X-ApiKey", value: "{{$injector.api_key_callCRM}}"},{ name: "Accept", value: "application/json"}]
    #             vars = [ { "name": "result", "value": "$httpResponse" } ]
    #             okState = /Получение данных по профилю клиента/ok
    #             errorState = /Получение данных по профилю клиента/error
        
        # state: ok
        #     script:
        #         $reactions.answer(JSON.stringify($session.result.Result.ClientStatus));
        #         $session.profileCRM = false;
        #         if ($session.result.Result.ClientStatus != null){
        #             $reactions.answer(JSON.stringify($session.result.Result.ClientStatus));
        #             }
                
        #         for (var i in $session.result.Result.ClientProfiles) {
        #             //$reactions.answer(JSON.stringify($session.result.Result.ClientProfiles[i].Name));
        #             if ($session.result.Result.ClientProfiles[i].Name == "АО"){
        #                 $session.profileCRM = true;
        #                 $reactions.answer($session.profileCRM);
        #                 break;
        #             }
        #         }
    ...
    state: Перевод на оператора || modal = true
        ...
        script:
            ...
            if ($session.to_division == undefined){
                //$session.to_division = "5ec8de6b-7786-49b5-a3e5-d7bcc938cbb4";
        ...
        state: Отправка запроса
            ...
            state: errorState
                script:
                    $analytics.setMessageLabel("Ошибка перевода на оператора", "Ошибка");
        ...
        state: Перевод на оператора noMatch
            ...
            script:
                $analytics.setMessageLabel("Клиент перебил перевод", "Ошибка");
                $reactions.timeout({ interval: 2, targetState:  "/Перевод на оператора/Отправка запроса" });
    
    state: Перевод на оператора КАЦ
        script:
            kac();
            $analytics.setMessageLabel("Перевод на оператора КАЦ", "КАЦ");
    ...
    state: Закрытие обращения
        ...
        state: Отправка запроса
            ...
            state: okState
                script:
                    $analytics.setSessionResult("Обращение закрыто");
                EndSession:
                
            state: errorState
                script:
                    $analytics.setMessageLabel("Ошибка закрытия обращений", "Ошибка");
    ...
    state: Высокодоходный портфель + Защитный портфель
        ...
        script:
            $analytics.setMessageLabel("Общее", "КАЦ");
    ...
    state: Рассылки КАЦ
        q!: @mailing_KAC_HC
        script:
            $analytics.setMessageLabel("Рассылки КАЦ", "КАЦ");
    ...
    state: Открытие_закрытие счетов || sessionResultColor = "#15952F"
        ...
        # Доделать прямой переход в открытие/закрытие
    ...
    state: ИИС || sessionResultColor = "#15952F"
        intent!: /005 ИИС
        a: ❗ По распоряжению Правительства РФ от 22 января 2024 года № 104-р приобретение в рамках ИИС (и старого и нового образца) недоступно:
                - ценных бумаг иностранных эмитентов (ИЦБ)
                - ценных бумаг, эмитентами которых являются иностранные государства, таких как CIAN, ETLN, FIXP, GLTR, HHRU, MDMG, OKEY, OZON, POLY, YNDX, AGRO, TCSG, FIVE и др.
                - сделки РЕПО с вышеуказанными инструментами
                - выход на поставку ИЦБ по поставочным производным финансовым инструментам
                ✅ На данный момент доступно поддержание (хранение) и закрытие позиций.
        a: На ИИС доступны операции с инструментами:
            ✅ фондового рынка Московской биржи: акции, облигации, паи ПИФов
            ✅ срочного рынка МБ: фьючерсы и опционы (опционы на фьючерсы только в рамках моносчёта)
            ✅ валютного рынка МБ
            ✅ СПБ Биржи: российские ценные бумаги
    ...
    state: ИИС_Пополнение ИИС
        a: ❗ При оформлении вычета налоговая имеет право запросить платежное поручение с подтверждением внесения средств на ИИС. Не рекомендуется переводить средства с брокерских счетов, которые были пополнены с банковской карты и со счетов третьих лиц для исключения проблем оформления вычетов.
                ...
                Максимальная сумма пополнения инвестиционного счета в год составляет 1000000 ₽. Минимальный порог пополнения отсутствует. 
    ...
    state: ИИС_Третьего типа || sessionResultColor = "#CD4C2B"
        a: Согласно ФЗ от 19.12.2023 № 600-ФЗ, ИИС нового типа работает с 1 января 2024 года. Сопутствующие поправки в НК РФ прошли первое чтение Госдумы, но еще не приняты законодательно. Ссылка на законопроект: https://sozd.duma.gov.ru/bill/462670-8#bh_histras 
            Отличия нового вида ИИС от старого типа:
    ...
    state: Срочный рынок_Ещё_Исполнение
        ...
        a: ✅ Поставочные фьючерсы на ценные бумаги закрываются брокером, начиная с вечерней сессии за день до последнего дня обращения.
            ❗ Для выхода на поставку базового актива нужно подписать Уведомление о поставке не позднее 14:00 МСК за два рабочих дня (Т-2) до исполнения контракта, обратившись к менеджеру компании.
    ...
    state: Маржа || sessionResultColor = "#15952F"
        intent!: /011 Маржа
        a: ❗ В ближайшее время ставки риска по ценным бумагам на иностранных биржах и валютам будут повышаться в связи с требованием рыночной конъектуры.
    ...
    state: Документы_Общие_Регламент_П 6.1.6
        a: Согласно пункту 6.1.6 регламента брокер может отказать в исполнении поручения на вывод денежных средств через систему СБП.
    ...
    state: Документы_Налоговые_2-НДФЛ
        a: 1. Справка 2-НДФЛ — официальный документ по форме ИФНС РФ о рассчитанных доходах и удержанных с них налогах за отчетный период (календарный год). Справка 2-НДФЛ за 2023 год будет доступна к заказу после 10 марта 2024 года.
            ✅ Заказ справки 2-НДФЛ доступен в личном кабинете [в разделе «Налоги».|https://lk.finam.ru/reports/tax]
    ...
    state: Контакты || sessionResultColor = "#418614"
        intent!: /028 Контакты
        if: holidays()
            a: В период с 8 по 10 марта - Выходные дни в офисах «Финам».
    ...
    state: Доступные биржи || sessionResultColor = "#418614"
        ...
        if: holidays()
            a: 8 марта 2024 года — Международный женский день в РФ, торги и расчеты на российских биржах не проводятся.
    ...
    state: Ограничение ЦБ || sessionResultColor = "#418614"
        ...
        buttons:
            ...
            "Указ Президента РФ № 844" -> /Ограничение ЦБ_844
    ...
    state: Ограничение ЦБ_844 || sessionResultColor = "#B65A1E"
        a: ✅ В рамках Указа № 844 «О дополнительных временных мерах экономического характера, связанных с обращением иностранных ценных бумаг» предполагает обмен заблокированных в России и за рубежом активов российских инвесторов. Общая стоимость бумаг, которые можно обменять, ограничена суммой в 100000 ₽.
            ✅ С 25 марта по 8 мая 2024 года клиенты «Финам» смогут подать согласие на заявку и обмен. О том, как будет проходить процедура, будет сообщено дополнительно. [Подробнее|https://www.finam.ru/publications/item/obmen-zablokirovannymi-aktivami-glavnoe-20240312-1738/]
    ...
    state: Санкции || sessionResultColor = "#418614"
        ...
        a: 1. Влияет ли новый пакет санкций на торговлю через «Финам» и на возможность приобретать иностранные активы.
            ...
            Нет, санкции не затронули ни одну из компаний холдинга.
    ...
    state: Корпоративные действия_Размещение || sessionResultColor = "#B65A1E"
        a: Инвесторам «Финам» доступно участие в публичных размещениях облигаций и ценных бумаг (IPO).
            ...
            — в веб-терминале или в мобильном приложении FinamTrade в левом вертикальном меню в разделе «Первичные размещения» (значок ракеты) выбрать «Мои заявки»
    ...
    state: Сторонее ПО || sessionResultColor = "#418614"
    ...
    state: Сторонее ПО_TRANSAQ Connector || sessionResultColor = "#CD4C2B"
    ...
    state: Услуги компании_Помощники_ИИ советник || sessionResultColor = "#CD4C2B"
        script:
            $analytics.setMessageLabel("ИИ советник", "КАЦ");
    ...
    state: Высокодоходный портфель
        script:
            $analytics.setMessageLabel("Высокодоходный портфель", "КАЦ");
    ...
    state: Защитный портфель
        script:
            $analytics.setMessageLabel("Защитный портфель", "КАЦ");
    ...
    state: Услуги компании_Готовые решения_Сопровождение
        script:
            $analytics.setMessageLabel("Инвест сопровождение", "КАЦ");
    ...
    state: Финам-бонус || sessionResultColor = "#15952F"
        ...
        a: Участвуйте в акции «Финам бонус»!
            ✅ Пройдите регистрацию и получите 500 приветственных бонусов. Накопите 3000 бонусов за использование сервисов «Финам» и получите их на брокерский счет. 1 бонус = 1 ₽
            ✅ [Регистрация, выбор заданий для получения бонусов, детали акции|https://www.finam.ru/landing/finam-bonus-2024/]
            ✅ Бонусный баланс обновляется 1 раз в месяц: 15 февраля 2024 года, 15 марта 2024 года
            ✅ Когда вы накопите на своем бонусном счете не менее 3000 бонусов, в период с 5 по 15 апреля 2024 года деньги будут автоматически перечислены на ваш брокерский счет (не ИИС).*
    ...
    state: Финам-бонус_Кабинет участника
        a: Для входа в кабинет участника «Финам бонус» нужно: 
            1.  Авторизоваться на сайте [finam.ru|https://www.finam.ru]
            ✅ в верхнем правом углу выбрать иконку входа
            ✅ использовать логин/пароль, полученный при регистрации на сайте
            2. Перейти в [личный кабинет участника «Финам бонус»|https://www.finam.ru/landings/bonus-profile/]
            ❗ Увидеть накопленные бонусы участник сможет только в том случае, если будет авторизован на сайте finam.ru. В противном случае участнику будет отображаться нулевой баланс. 
            ❗ Если ранее вы не регистрировались на сайте [finam.ru|https://www.finam.ru], то на вашу электронную почту одновременно с письмом о регистрации в акции «Финам бонус» пришло письмо с логином и паролем от [finam.ru|https://www.finam.ru] 
    ...
    state: Счет Иностранные биржи_пополнение_вывод
        a: 1. Пополнить брокерский счет «Иностранные биржи» можно только рублями РФ:
            ...
            – Онлайн в личном кабинете по ссылке: https://lk.finam.ru/deposit 
            – С действующих брокерских счетов, открытых в «Финам» доступен перевод средств только в рублях РФ
            2. Вывод средств доступен в рублях РФ или в долларах США в личном кабинете по ссылке: https://lk.finam.ru/withdraw
