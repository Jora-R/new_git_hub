...
# Проверка времени для учебного центра при переводе на оператора
require: scripts/Mailing/educationalCenter.js
...
# Проверка наличия статуса квалифицированного инвестора у клиента
require: ./scripts/EDOX/isQualified.js
# Проверка пройденных клиентом тестов (квал. аспекты)
require: ./scripts/EDOX/qualifiedAspects.js
# Функция проверки на статус квала + квал аспекты клиента и вывод необходимых для прохождения тестов
require: ./scripts/EDOX/testsPassedCheck.js
# Словарь по облигациям (ISIN,название,дата погашения)
require: ./scripts/AIScreenerBonds/bondsIsin.js
# API к данным по AI скриннеру облигаций
require: ./scripts/AIScreenerBonds/getBondsData.js
# Словарь ответов бота на запросы показателей скриннера облигаций
require: ./scripts/AIScreenerBonds/screenerAnswers.js
# Функция поиска облигации в словаре по указанным данным (дата погашения, название эмитента) + преобразователь дат
require: ./scripts/AIScreenerBonds/getIsinName.js
    type = scriptEs6
    name = getIsin
# API подключения к диалогу к GPT
require: ./scripts/GPT/gpt.js
require: ./scripts/GPT/anonymizer.js


theme: /
    
    # state: тест auth_ID
    #     q!: ("идентификация пользователя"|идентификация пользователя)
    #     script:
    #         $reactions.answer(JSON.stringify($request.data.authenticationId));
    ...
    state: Рекомендации (тест)
        q!: тест персональных рассылок
        script:
            ...
            $session.clientGlobalId = $request.data.clientGlobalId;
            ...
            $session.url_profileCRM = "http://msa-vcbtr1-db01:3000/do_recommendations?globalid=eq." + $session.clientGlobalId + "&type_info=eq.product_fin&order=date_valid.desc,probability.desc&limit=1"; //PROD
        go!: /Рекомендации (тест)/Отправка запроса
        ...
        state: Отправка запроса
            HttpRequest:
                ...
                okState = /Рекомендации (тест)/Предоставление рекомендации
                errorState = /Рекомендации (тест)/Error
    
        state: Предоставление рекомендации
            a: Искусственный интеллект «Финам» предполагает, что вам может быть интересна услуга: [{{$session.recommendation.name}}|{{$session.recommendation.landing}}?utm_source=cuk&utm_medium=chatbotrs&utm_campaign=gratitude]
            script:
    ...
    state: Рекомендации
        ...
        state: Предоставление рекомендации
            script:
                var differenceRecommendation    = moment().diff($client.sendRecommendation,'days') || 0;
                var differenceNotification      = moment().diff($client.sendNotification,'days') || 0;
                # log(difference)
                if($session.recommendation.name != null && (!$client.sendRecommendation || differenceRecommendation >=7)){
                    $client.sendRecommendation = moment();
                    $reactions.answer("Искусственный интеллект «Финам» предполагает, что вам может быть интересна услуга: [{{$session.recommendation.name}}|{{$session.recommendation.landing}}?utm_source=cuk&utm_medium=chatbotrs&utm_campaign=gratitude]");
                    $analytics.setMessageLabel("Рекомендация отправлена", "Рекомендации TB");
                }
                # if(!$client.sendNotification || differenceNotification >=7){
                #     $client.sendNotification = moment();
                #     $reactions.answer("Для дополнительной защиты ваших персональных данных, в компании «Финам» вводится обязательный способ идентификации клиентов - кодовое слово. С 1 сентября 2024 года клиенты компании смогут получить персональную информацию или подать голосовую заявку только после того, как назовут сотруднику своё кодовое слово. Создать, изменить или удалить кодовое слово можно в личном кабинете [в разделе «Персональные данные»|https://lk.finam.ru/Client/CodeWords]");
                #     $analytics.setMessageLabel("Уведомление отправлено", "Рекомендации TB");
                # }
        ...
        state: Отправка запроса
            ...
            state: errorState
                ...
                a: На данный момент перевод на оператора недоступен. Напишите нам позже, либо обратитесь по контактам: 
                    тел. +7 (495) 1-346-346 — доб. 2222
                    тел. *1945 — доб. 2222 (Бесплатно по РФ для МТС, Билайн, МегаФон и Tele2)
    ...
    state: Перевод на оператора УЦ
        script:
            educationalCenter();
            $analytics.setMessageLabel("Перевод на оператора УЦ", "УЦ TB");
        go!: /Перевод на оператора    
    ...
    state: Приложение банка || modal = true 
        ...
        a: В АО «Банк ФИНАМ» вас ждут низкие тарифы на все финансовые операции и квалифицированный сервис. [Информация по тарифам и услугам Банка|https://www.finambank.ru/person/rates/]
        ...
        buttons:
            ...
            "Контакты Банка" -> /Приложение банка/Приложение банка_Контакты банка
            "Перевод на оператора" -> /Перевод на оператора
        ...
        state: Приложение банка_Счет
            a: ✅ Счета в Банке «Финам» открываются в рублях РФ, долларах США, евро, китайских юанях, казахстанских тенге, армянских драмах.
                    ...
                    ✅ [Актуальные тарифы на сайте Банка|https://www.finambank.ru/person/rates] в разделе «Рассчетно-кассовое обслуживание»
        ...
        state:  Приложение банка_Счет_Как открыть
            a: Если вы ранее открывали брокерский счет в «Финам» при личном визите в офисе компании, то для вас доступно [открытие банковского счета дистанционно в личном кабинете|https://lk.finam.ru/NewAccount/Product]
        ...
        state: Приложение банка_Счет_Наличные
            a:  ✅ Получение наличных рублей со счетов/вкладов, открытых в рублях на сумму более 100000 ₽ осуществляется Банком при условии их предварительного заказа клиентом до 12:30 рабочего дня, предшествующего дню получения.
                    ...
                    ✅ Получение наличной иностранной валюты в китайских юанях возможно в отделениях банка «Финам» в г. Москва в Настасьинском пер. дом 7, стр.2, в г. Благовещенск и г. Владивосток.
        ...
        state: Приложение банка_Счет_Валюта
            a: ✅ Зачисление долларов США, евро и иных валют на банковские счета Банка «Финам» – без комиссии.
                ✅ Комиссия за обслуживание банковских счетов в долларах США и евро:
                     – Если совокупный остаток не превышает 3000 единиц валюты – хранение бесплатно
                     – Если совокупный остаток равен либо превышает 3000 единиц валюты – 0,03% в день от остатка
                ❗ Комиссия за хранение удерживается ежедневно на сумму остатка на начало дня: учитывается совокупный (суммарный) остаток денежных средств по всем валютным текущим счетам/карточным счетам, открытым после 15.08.2022 включительно в долларах США/евро. Комиссия взимается в валюте Счета отдельно с каждого счета (счетов) в долларах США/евро.
            buttons:
                "В основное меню" -> /Приложение банка
                "Назад" -> /Приложение банка/Приложение банка_Счет
        
        state: Приложение банка_Счет_Вклады
            a: Подробная [информация о вкладах и накопительных счетах|https://www.finambank.ru/person/deposits/]
            buttons:
                "В основное меню" -> /Приложение банка
                "Назад" -> /Приложение банка/Приложение банка_Счет
        
        state: Приложение банка_Карта
            a: «Банк Финам» выпускает банковские карты платежной системы МИР по пакетам услуг «Комфорт», «Престиж», «Премиум», «Корпоративный».
            ...
            buttons:
                ...
                "Заблокировать карту" -> /Приложение банка/Приложение банка_Карта_Заблокировать карту
                "Реквизиты карты" -> /Приложение банка/Приложение банка_Карта_Реквизиты карты
                "Тарифы на обслуживание карт" -> /Приложение банка/Приложение банка_Карта_Тарифы
                "Программа лояльности «CASHBACK»" -> /Приложение банка/Приложение банка_Карта_Программа лояльности
                "Снятие наличных" -> /Приложение банка/Приложение банка_Карта_Наличные
                "Назад" -> /Приложение банка
        
        state: Приложение банка_Карта_Как открыть
            a: ✅ Если вы ранее дистанционно открывали брокерский счет в «Финам», то для вас доступно [открытие банковской карты по тарифу «Комфорт» дистанционно в личном кабинете|https://lk.finam.ru/NewAccount/Product]
        ...
        state: Приложение банка_Карта_Заблокировать карту
            a: Заблокировать банковскую карту 24/7 можно в интернет-банке, в мобильном приложении, в личном кабинете или позвонить в отдел поддержки.
                1. В мобильном приложении Банка «Финам» выбрать счёт нужной карты → нажать три точки в верхнем правом углу → нажать на иконку с изображением карты → снова нажать три точки в верхнем правом углу → Заблокировать
                2. В [личном кабинете|https://lk.finam.ru/] 
                ✅ выбрать нужный счет → в разделе таблицы «Владелец/Дата окончания» выбрать «Заблокировать»
                ✅ или в разделе личного кабинета [«Операции с банковскими картами»|https://lk.finam.ru/List/BankOrders] выбрать «Активация/блокировка/разблокировка карты»
                3. В [«Интернет-банке»|https://ibank.finam.ru/] выбрать счёт нужной карты → справа в разделе «Основная информация» нажать «Заблокировать»
                4. Обратиться в поддержку Банка можно по телефонам +7(495) 796-90-23 или +7 (800) 200-44-00 (бесплатно по России). При звонке понадобится полная идентификация, в том числе назвать кодовое слово.
            buttons:
                    "В основное меню" -> /Приложение банка
                    "Назад" -> /Приложение банка/Приложение банка_Карта
                
        state: Приложение банка_Карта_Реквизиты карты
            a: Реквизиты вашей банковской карты можно получить в интернет-банке, в мобильном приложении или в личном кабинете.
                1. В мобильном приложении Банка «Финам»
                ✅ Посмотреть реквизиты карточного счета, скачать или поделиться файлом: 
                выберите счёт нужной карты → нажмите три точки в верхнем правом углу → Реквизиты для пополнения 
                ✅ Посмотреть реквизиты (номер, срок действия, CVP код) карты: выберите счёт нужной карты → нажмите три точки в верхнем правом углу → нажмите на иконку с изображением карты → снова нажмите три точки в верхнем правом углу → Реквизиты карты
                2. В [личном кабинете|https://lk.finam.ru/] скачать реквизиты карточного счета: 
                Выберите нужный счет → выберите раздел «Реквизиты для пополнения»
                3. В [«Интернет-банке»|https://ibank.finam.ru/]:
                ✅ Посмотреть и скачать реквизиты карточного счета: 
                выберите счёт нужной карты → справа в разделе «Основная информация» нажмите «Реквизиты для пополнения»
                ✅ Посмотреть реквизиты (номер, срок действия, CVP код) карты: выберите счёт нужной карты → нажмите на символ «Глаз» возле номера карты        
            buttons:
                    "В основное меню" -> /Приложение банка
                    "Назад" -> /Приложение банка/Приложение банка_Карта
        
        state: Приложение банка_Карта_Тарифы
            a: ✅ Актуальная информация по [тарифам «Корпоративный», «Комфорт» и «Премиум»|https://www.finambank.ru/person/cards/]
                    ❗ Информация по обслуживанию архивных пакетов услуг по банковским картам доступна на [сайте Банка|https://www.finambank.ru/person/rates] в разделе «Архив тарифов»
            buttons:
                "В основное меню" -> /Приложение банка
                "Назад" -> /Приложение банка/Приложение банка_Карта
        
        state: Приложение банка_Карта_Программа лояльности
            a: Программа лояльности «CASHBACK» подключается автоматически при выпуске первой карты банка «Финам». Повышенный кешбэк по выбранным категориям до 7%, 1% — на остальные покупки.
                ✅ Выбирать и подключать категории можно в [интернет-банке|https://ibank.finam.ru/] и в мобильном приложении банка. 
                ✅ Подключенные категории устанавливаются на все карты, выпущенные в рамках одного пакета услуг. При первом подключении категории повышенная ставка начинает действовать на следующий день. Менять категории можно 1 раз в месяц, действие новой категории начнется с 1-ого числа следующего месяца.
                ✅ Минимальная сумма операций по карте и количество подключенных категорий зависит от подключенного пакета услуг, подробнее:
                —  [«Комфорт» и «Премиум»|https://www.finam.ru/files/u/dw/files/presentations/bank/cashback/programma_loyal_nosti_cashback.pdf]
                — [«Корпоративный»|https://www.finam.ru/files/u/dw/files/presentations/bank/cashback-corp/2083_onepager_programma_loyal_nosti_cashback_korporativny_j_03.pdf]
                ✅ [Регламент Программы и перечень МСС-кодов, объединенных в категории|https://www.finambank.ru/files/u/dw/bank/person/documents/file3/sajt_programma_loyal_nosti_cashback_17072024.pdf]
            buttons:
                "В основное меню" -> /Приложение банка
                "Назад" -> /Приложение банка/Приложение банка_Карта
        ...
        state: Приложение банка_Карта_Наличные
            a: ✅ Снять денежные средства можно в [кассе офиса «Финам»|https://www.finambank.ru/about/offices]
                ...
                ✅ Актуальные тарифы на снятие наличных представлены на [сайте Банка|https://www.finambank.ru/person/rates] в разделах «Пакет услуг Корпоративный» и «Пакеты услуг Комфорт, Престиж и Премиум». 
        ...
        state: Приложение банка_Конвертация валюты
            a: Конвертация валюты в рамках банковского обслуживания возможна дистанционно в [интернет-банке|https://ibank.finam.ru] и в отделениях банка «Финам» в Москве.
        ...
        state: Приложение банка_За границу
            a: 1. Через «Банк Финам» доступны переводы по реквизитам заграницу валют рубль РФ, армянский драм, казахстанский тенге, китайский юань (кроме банков Еврозоны, банков Швейцарии, банков Великобритании, банков США, как банков посредников, так и банков получателей).
                ✅ [Ознакомиться с тарифами|https://www.finambank.ru/person/rates] можно в разделе «Рассчетно-кассовое обслуживание»
                ...
                * ПС – платёжная система.
                ✅ Осуществить такой перевод можно в [разделе интернет-банка «Перевод с карты на карту»,|https://ibank.finam.ru/Operations/Card2Card/Init] для этого нужно:
                – выбрать карту МИР, с которой будет произведено списание
                – указать карту получателя перевода, на которую будет проведено зачисление
                – указать фамилию и имя получателя перевода на латинице
                – подтвердить операцию кодом из СМС
                ✅ Комиссия за перевод взимается в соответствии с действующими тарифами банка «Финам»:
                – По тарифу «Комфорт» до 20000 ₽ в месяц – без комиссии, свыше 20000 ₽ в месяц – 1,5% от суммы перевода, но не менее 50 ₽
                – По тарифу «Престиж» до 50000  ₽ в месяц – без комиссии, свыше 50000 рублей в месяц - 1,5% от суммы перевода, но не менее 50 ₽
                – По тарифу «Премиум» до 100000 ₽ в месяц – без комиссии, свыше 100000 ₽ в месяц – 1,5% от суммы перевода, но не менее 50 ₽
                – По тарифу «Корпоративный» до 30000 ₽ в месяц – без комиссии, свыше 30000 ₽ в месяц – 1,5% от суммы перевода, но не менее 50 ₽
        ...
        state: Приложение банка_СБП
            a: ✅ Максимальная сумма одного перевода/платежа с использованием Системы быстрых платежей (СБП) составляет 1000000 ₽
                    ...
                    2. «Престиж» — до 500000 ₽ в месяц — без комиссии, свыше 500000 ₽ в месяц — 0,5% от суммы перевода, не более 1500 ₽
                    3. «Корпоративный» — до 300000 ₽ в месяц — без комиссии, свыше 300000 ₽ в месяц — 0,5% от суммы перевода, не более 1500 ₽
                    4. «Премиум» — до 5000000 ₽ — без комиссий
                    ...
                    ✅ Актуальные тарифы на [сайте Банка|https://www.finambank.ru/person/rates] в разделах «Пакет услуг Корпоративный» и «Пакеты услуг Комфорт, Престиж и Премиум»
            buttons:
                "В основное меню" -> /Приложение банка
        
        state: Приложение банка_Контакты банка
            a: ✅ Обратиться в поддержку Банка:
                    — по телефонам +7(495) 796-90-23 или +7 (800) 200-44-00 (бесплатно по России)
                    — в чате
                    — посетить офис компании «Финам». [Адреса и контактная информация|https://www.finambank.ru/about/offices]
                ✅ Режим работы Банка:
    ...
    state: Корпоративные действия_Другие_Полиметалл_ЖП
        q!: * @polymetal_HC *
        go!: /Корпоративные действия_Другие_Полиметалл
    
    # state: Корпоративные действия_втб_ЖП
    #     q!: * @VTB_futures_HC *
    #     go!: /Корпоративные действия_втб
    
    state: Корпоративные действия_Яндекс_ЖП
        q!: * @yandex_HC *
        go!: /Корпоративные действия_Яндекс    
        
    state: Ограничение ЦБ_Обращение в OFAC_ЖП
        q!: * @ofac_HC *
        go!: /Ограничение ЦБ_Обращение в OFAC
    ...
    state: Комиссии_За сделку
        ...
        buttons:
            ...
            "Долгосрочный портфель" -> /Комиссии_За сделку_Долгосрочный портфель
    ...
    state: Комиссии_За обслуживание
        ...
        buttons:
            ...
            "Долгосрочный портфель" -> /Комиссии_За обслуживание_Долгосрочный портфель
    ...
    state: Комиссии_Биржа_ВалютныйМБ
        a: 1. Валютные пары:
                ✅ За торговлю полными лотами (контракты TOD и TOM) — 0% для мейкеров и 0,0045% от оборота для тейкеров, при этом минимальная комиссия за сделку 50 ₽, если заявка на совершение сделки подана объемом менее 50 лотов; если более 50 лотов – минимальная комиссия 0,02 ₽ для мейкеров и 1 ₽ для тейкеров. 
    ...
    state: Комиссии_За сделку_Долгосрочный портфель
        a: С детальным описанием тарифа можно ознакомиться по ссылке: https://www.finam.ru/landings/tariffs/
    ...
    state: Комиссии_За обслуживание_Долгосрочный портфель
        a: Комиссия 0 ₽.
    ...
    state: Комиссии_За обслуживание_Другие
        ...
        buttons:
            ...
            "Тест-Драйв" -> /Комиссии_За обслуживание_Долгосрочный портфель
            "Долгосрочный инвестор" -> /Комиссии_За обслуживание_Долгосрочный портфель
            "Стандартный ФОРТС" -> /Комиссии_За обслуживание_Долгосрочный портфель
    ...
    state: Комиссии_Другие_ВводВыводХранение
        ...
        buttons:
            ...
            "Комиссии за хранение валюты" -> /Валютный рынок_Ввод и вывод_Хранение валюты
    ...
    state: Комиссии_Другие_Маржинальная
        ...
        buttons:
            ...
            "Долгосрочный портфель" -> /Комиссии_Другие_Маржинальная_Долгосрочный портфель
            "Единый Консультационный" -> /Комиссии_Другие_Маржинальная_ЕдиныйКонсульт
            "Другие" -> /Комиссии_Другие_Маржинальная_Другие
            
    state: Комиссии_Другие_Маржинальная_Долгосрочный портфель
        a: За маржинальные позиции, открытые и закрытые внутри одной торговой сессии, комиссия не удерживается, так как обязательства не переносятся на следующую торговую сессию.
                ✅ Комиссии за займ по тарифу «Долгосрочный портфель»: 
                — в рублях — ключевая ставка ЦБ РФ + 8%
                — в долларах — 15% (при сумме займа до 50 000 $)
                — в гонконгских долларах — 50%
                — в китайских юанях — 10% (при сумме займа до 160 000 ¥)
                — в иной валюте — 9% (при сумме займа до 25 000 ед.)
                — ценных бумаг (Московская биржа) — 13%
                — ценных бумаг (зарубежные биржи) — 8%
    ...
    state: Открытие_счета лично
        a: Вам понадобится мобильный телефон и документ удостоверяющий личность.
                ✅ [Перечень документов для иностранных граждан|https://www.finam.ru/services/OpenAccount0000A/]
    ...
    state: Открытие_счета для новых клиентов
        a: Дистанционное открытие счета доступно совершеннолетним гражданам Российской Федерации, а также гражданам дружественных государств (Беларуси, Казахстана, Азербайджана, Армении, Молдовы, Таджикистана, Туркменистана, Узбекистана).
                [Открыть счет|https://account.finam.ru/Registration]
        buttons:
            "Перевод на оператора" -> /Перевод на оператора

    state: Открытие_счета для действующих клиентов
        a: Вы можете иметь неограниченное количество действующих брокерских счетов.
                ✅ [Открыть дополнительный брокерский счет дистанционно в личном кабинете|https://lk.finam.ru/NewAccount/Product]
                ❗ Для открытия моносчетов выберите «Договор с отдельными брокерскими счетами».
    ...
    state: Отсутствует выбор адреса
        a: 1. Если не удается внести почтовый адрес при заполнении анкеты, пожалуйста, убедитесь, что названия объектов актуальны, вносите адрес до номера квартиры полностью, далее выберите вариант из выпадающего списка.
                2. Если адрес в анкете указан ошибочно, и нет возможности его изменить в рамках анкеты, то вы сможете изменить его после открытия счета в разделе «Изменение анкетных данных» по ссылке: https://lk.finam.ru/Client/EditInfo 
                ❗ Если рекомендации не помогли, для скорейшего решения вопроса, пожалуйста, нажмите кнопку «Перевод на оператора» и направьте в чат следующие данные:
                ✅ ваши ФИО
                ✅ адрес электронной почты, используемый для регистрации       
        buttons:
    ...
    state: Клиент с такими данными уже существует
        a: Уведомление о том, что вы являетесь персоной холдинга «Финам» или «Клиент с указанными данными уже существует» говорит о том, что вы когда-либо уже регистрировались в компании «Финам», и повторная регистрация недоступна.  
            ✅ Вы можете [войти в личный кабинет|https://lk.finam.ru/], при необходимости восстановить данные для входа - нажать «Забыли пароль?» и продолжить открытие счёта уже из вашего личного кабинета. 
    ...
    state: Мой брокерский счет
        a: ✅ Перечень действующих счетов доступен в личном кабинете: https://lk.finam.ru/
            ...
            ✅ Историю движения средств на брокерском счете можно посмотреть в справке по счету по ссылке: https://lk.finam.ru/Home/Account/Info
    ...
    state: Закрытие_счета
        a: Поручение на расторжение брокерского договора можно подать только по пустым счетам (на них не должно быть активов и задолженностей).
            ✅ Сформировать поручение можно самостоятельно в личном кабинете по ссылке: https://lk.finam.ru/orders/contractAbrogation/Default.aspx 
    ...
    state: Выбор_смена тарифа_Смена
        a: Сменить тариф – легко!
            ✅ [В личном кабинете выберите нужный счёт и нажмите «Сменить» тариф|https://lk.finam.ru/Home/Account/Details]
            ...
            ❗ С 3 июля 2024 года подключение тарифа «ФриТрейд» недоступно. По счетам, с подключенным ранее тарифом «ФриТрейд», по истечении 30 дней со дня подключения, пройдет смена тарифа на «Стратег». 

    state: Выбор_смена тарифа_Мой тариф
        a: Текущий тарифный план отображается в личном кабинете в разделе [«Детали» по счету.|https://lk.finam.ru/Home/Account/Details]
    ...
    state: Типы счетов_Единый счет (ЕДП)
        a: «Единая денежная позиция», «Единый счет» (ЕДП) — универсальный счет для торговли различными финансовыми инструментами.
            ✅ В рамках счёта ЕДП предоставляется доступ к:
            — Московской бирже: фондовый, валютный и срочный рынки (торговля опционами на акции РФ доступна через ИТС QUIK)
            — Бирже СПБ (российские ЦБ) 
            — NYSE/NASDAQ (кроме счетов, открытых в период с 15.08.2022 по 13.02.2023 и с 13.06.2024)
            — HKEX (закрытие позиций)
            ✅ Торговля иностранными ценными бумагами доступна со статусом квалифицированного инвестора
        buttons:
    ...
    state: Типы счетов_US Market options
        a: Пакет US Market Options — счет для торговли на иностранных биржах, в рамках данного счета доступна торговля акциями и опционами на американские акции на торговых площадках CBOE, NYSE, NASDAQ. [Подробнее|https://broker.finam.ru/landings/usaoptions/]
            ❗ С 13 июня открытие новых счетов типа US Market Options приостановлено
    ...
    state: ИИС_Пополнение ИИС
        a: ❗ При оформлении вычета налоговая имеет право запросить платежное поручение с подтверждением внесения средств на ИИС. Не рекомендуется переводить средства с брокерских счетов, которые были пополнены с банковской карты и со счетов третьих лиц для исключения проблем оформления вычетов.
                ✅ На ИИС можно переводить только рубли РФ. Поручение на перевод можно сформировать в [личном кабинете.|https://lk.finam.ru/Tiles/MoneyIn]              
    ...
    state: ИИС_Доступные инструменты
        a: ✅ На счетах ИИС доступны операции с инструментами российских компаний на Московской бирже: 
                ...
                - ценных бумаг, эмитентами которых являются иностранные государства, таких как CIAN, ETLN, FIXP, GLTR, HHRU, MDMG, OKEY, OZON, AGRO, FIVE и др.
    ...
    state: ИИС_Открытие ИИС_Открытие для новых клиентов
        a: ✅ Для [дистанционного открытия ИИС|https://www.finam.ru/open/order/iis/] понадобится только действующий паспорт совершеннолетнего гражданина РФ.
            ✅ Также открыть ИИС можно лично посетив ближайший [офис компании|https://www.finam.ru/about/contacts]

    state: ИИС_Открытие ИИС_Открытие для действующих клиентов
        a: Для действующих клиентов компании открытие ИИС доступно на главной странице [личного кабинета|https://lk.finam.ru/NewAccount/Product]
    ...
    state: ИИС_Пополнение ИИС_Наличными
        a: Для пополнения счета наличными, обратитесь в офис компании и воспользуйтесь услугами кассы. [Адреса офисов|https://www.finam.ru/about/contacts]

    state: ИИС_Пополнение ИИС_По реквизитам
        a: ✅ В личном кабинете можно [пополнить ИИС по реквизитам|https://lk.finam.ru/money/topup/requisites]
    ...
    state: ИИС_Налоговый вычет ИИС_A
        a: Максимальная сумма для вычета по типу «А», за календарный год составляет 400000 ₽. В зависимости от ставки налога на ваш доход, государство вернет вам 13% или 15% от той суммы, которую вы внесли на ИИС в отчетном году. Таким образом, максимальная сумма налога, подлежащая возврату, составит до 52000 ₽ или до 60000 ₽ соответственно.
                ...
                ✅ Скачать пакет документов для самостоятельной подачи или подать заявку (предоставить сведения о ИИС) для получения вычета в упрощенном порядке, можно в разделе личного кабинета [«Инвестиционный налоговый вычет»|https://lk.finam.ru/List/Extracts]
                ...
                ✅ Пакет документов об открытии счета и брокерский отчет можно скачать в личном кабинете в [разделе «Запрос документов для самостоятельной подачи».|https://lk.finam.ru/orders/TaxDeduction] Для заказа пакета документов в бумажном варианте необходимо обратиться к менеджеру компании.
        buttons:
            "Перевод на оператора" -> /Перевод на оператора
    ...
    state: ИИС_Налоговый вычет ИИС_Б
        a: Максимальная сумма для вычета по типу «Б» равна доходу, полученному от торговых операций на договоре ИИС. Данная инвестиционная прибыль при вычете по типу «Б» налогом не облагается. Претендовать на вычет можно по истечению 3х лет с открытия ИИС.
                ✅ Скачать пакет документов для самостоятельной подачи или подать заявку (предоставить сведения о ИИС в ФНС) для получения вычета по типу «Б» в упрощенном порядке, можно в разделе личного кабинета [«Инвестиционный налоговый вычет»|https://lk.finam.ru/List/Extracts]
                ...
                ✅ Если в течение 30 дней после подтверждения заявления со стороны ФНС счет не будет расторгнут, необходимо подать заявление повторно.
                ...
                1. Пакет документов об открытии счета и брокерский отчет. В электронном виде их можно заказать в личном кабинете в [разделе «Запрос документов для самостоятельной подачи».|https://lk.finam.ru/orders/TaxDeduction]
                Пакет будет подготовлен с заверением и выгружен в личном кабинете в [разделе «История операций»|https://lk.finam.ru/Journals/UnionDocumentJournal]
                Для заказа пакета документов в бумажном варианте необходимо обратиться к менеджеру компании.
                2. Справка 2-НДФЛ. [Заказать ее можно в личном кабинете.|https://lk.finam.ru/orders/Ndfl]
                Справка будет подготовлена с заверением и выгружена в личном кабинете в [разделе «История операций».|https://lk.finam.ru/Journals/UnionDocumentJournal]
    ...
    state: ИИС_Еще_Перевод ИИС
        a: Перевод активов в рамках ИИС с сохранением срока действия осуществляется следующим образом:
                1. Откройте ИИС в «Финам», скачайте уведомление об открытии счета и предоставьте его брокеру, у которого находится ваш действующий ИИС. [Скачать документы можно в личном кабинете|https://lk.finam.ru/catalog/documents.aspx]
    ...
    state: ИИС_Еще_Расторжение ИИС_Вывод средств
        a: Поручение на вывод денежных средств можно [сформировать в личном кабинете|https://lk.finam.ru/orders/MoneyOut/MoneyOut/Default.aspx]
    ...
    state: ИИС_Еще_Расторжение ИИС_Расторжение пустого
        a: В личном кабинете доступно [расторжение пустого ИИС|https://lk.finam.ru/orders/contractAbrogation/Default.aspx]
    ...
    state: ИИС_Еще_Расторжение ИИС_Расторжение перевод
        a: Перевод активов в рамках ИИС с сохранением срока действия:
            ...
            Реквизиты депозитарного счета для ввода ценных бумаг доступны в [личном кабинете|https://lk.finam.ru/global/Requisites/DepoAdmission.aspx]
            ...
            4. Перевод денежных средств и ценных бумаг между счетами ИИС осуществляется только напрямую между брокерами. После вывода активов, ИИС в «Финам» автоматически закроется на 5-й рабочий день.
             [Сформировать поручение на вывод денежных средств на новый ИИС можно в личном кабинете|https://lk.finam.ru/orders/MoneyOut/MoneyOut/Default.aspx]: выбрать вывод на счета третьих лиц, обязательно вложить документы об открытии нового ИИС у другого брокера. (Доступно для клиентов, прошедших идентификацию в офисе компании.)
    ...
    state: Налоги_Расчет НДФЛ
        a: В личном кабинете можно скачать предварительный расчет налога по закрытым позициям в разделе [«Предварительный расчет НДФЛ»|https://lk.finam.ru/Orders/NdflPreCalculation]
    ...
    state: Налоги_Излишне удержанный налог
        a: ✅ Заявление на возврат излишне удержанного налога можно сформировать в личном кабинете при наличии актуального уведомления об излишне удержанном налоге в разделе [«Заявление на возврат НДФЛ»|https://lk.finam.ru/orders/TaxReturnToBrokerageAccount.aspx]
                Подписать документ можно в течение трех лет с момента завершения отчетного периода. 
                ✅ Для этого нужно заказать у брокера в личном кабинете [справку об убытках|https://lk.finam.ru/orders/NdflLoss] и [2-НДФЛ|https://lk.finam.ru/orders/Ndfl].
    ...
    state: Налоги_Трехгодичная льгота
        a: ✅ Если у вас в портфеле (за исключением ИИС) есть бумаги, приобретенные после 01.01.2014, и вы владеете ими непрерывно более трех лет, то можете претендовать на трехгодичную льготу.
                ...
                ✅ Проверить наличие бумаг, попадающих под льготу на счетах в «Финам» можно в личном кабинете по ссылке: https://lk.finam.ru/journals/ThreeYearPrivilegeRestsJournal 
    ...
    state: Выплата дохода || sessionResultColor = "#15952F"
        ...
        a: Клиентам «Финам» начисляются дивиденды и купонные выплаты в рублях — по российским ценным бумагам и в валюте — по еврооблигациям, ETF фондам, а также по иностранным ценным бумагам.
            ✅ Доходы в валюте от СПБ Биржи поступают в рублях РФ, распределение поступивших сумм происходит пропорционально количеству ценных бумаг на всех владельцев, остальная часть выплаты не поступает из-за блокировки цепочки с Euroclear/Clearstream. СПБ Биржа по причине санкций осуществляет конверсионные операции с валютой, на основаниях, предусмотренных ее регламентом. Изменить валюту выплаты или отказаться от выплаты в рублях РФ – невозможно.
            ❗ Выплаты от корпоративных действий поступают на [предоставленные в личном кабинете реквизиты.|https://lk.finam.ru/orders/depoBankAccountDetails.aspx]
    ...
    state: Выплата дохода_Вывод
        a: Выплата дивидендов, купонов и прочих доходов по ценным бумагам по умолчанию предусмотрена на счет учета бумаг. 
            ...
            ✅ Подать заявление можно в личном кабинете в разделе [Выплата дохода по ЦБ на другие счета|https://lk.finam.ru/orders/IncomePayments] 
            ❗ Выплата доходов с ИИС на другие счета недоступна.
    ...
    state: Выплата дохода_Ещё_Проверить начисление
        a: Сумма начисленных дивидендов и купонов отображается в виде свободного остатка или уменьшает сумму займа по счету в соответствующей валюте.
            ...
            ✅ в разделе [«Брокерские операции»|https://lk.finam.ru/Journals/AccountOperationsJournal]
            ✅ в [справке по счету|https://lk.finam.ru/Home/Account/Info] с детальным описанием операций и сделок (можно загрузить только за закрытый торговый период)
    ...
    state: Выплата дохода_Документы_Московская биржа
        a: Для отчетности в налоговую по валютным дивидендам от иностранных компаний в личном кабинете нужно [скачать справку 1042S|https://lk.finam.ru/orders/Form1042S]
            ...
            ✅ Уведомления доступны в личном кабинете в [«Журнале уведомлений»|https://lk.finam.ru/Journals/NotificationJournal] 

    state: Выплата дохода_Документы_Биржа СПБ
        a: Для отчетности в налоговую по валютным дивидендам от иностранных компаний в личном кабинете нужно [скачать справку 1042S|https://lk.finam.ru/orders/Form1042S]
            ✅ Справка предоставляется депозитарием за завершенный отчетный период (календарный год).
            ✅ Детальная информация по каждому зачислению отображается в уведомлениях о выплате дохода в личном кабинете в [«Журнале уведомлений»|https://lk.finam.ru/Journals/NotificationJournal]
    ...
    state: Выплата дохода_Документы_Иностранные биржи
        a: Для отчетности в налоговую по валютным дивидендам от иностранных компаний, запросите Уведомление от вышестоящего брокера у менеджера «Финам».
                ...
                ✅ Дополнительно в ваш личный кабинет будет выгружено Уведомление о присвоении торгового кода в раздел [«Журнал уведомлений».|https://lk.finam.ru/Journals/NotificationJournal]
    ...
    state: Срочный рынок_Обеспечение_ПГО || sessionResultColor = "#B65A1E"
        a: Услуга пониженного гарантийного обеспечения (ПГО) предоставляется клиентам «Финам», удовлетворяющим одновременно нескольким требованиям.
            ...
            ✅ [Подключить или отключить услугу в личном кабинете|https://lk.finam.ru/orders/ReducedGuarantee.aspx]
    ...
    state: Срочный рынок_Обеспечение_Отдельные счета
        a: По договорам с раздельными счетами (моносчетами) размер ГО равен указанному на бирже.
            ✅ Открыть новый счет можно в личном кабинете по ссылке: https://lk.finam.ru/NewAccount/Product?ContextId=fff807f6-7837-4bff-9484-014dc3c5f94c
    ...
    state: Срочный рынок_Маржа
        a: Прибыль (убыток) по фьючерсам и опционам зачисляется (списывается) в виде вариационной маржи.
            ...
            Движение вариационной маржи отображается в [справке по счету|https://lk.finam.ru/Home/Account/Info], а также в разделе личного кабинета [«Брокерские операции» (только посделочная вариационная маржа).|https://lk.finam.ru/Journals/AccountOperationsJournal]
            ✅ Параметры инструментов для расчета вариационной маржи на [сайте биржи|https://www.moex.com/ru/derivatives/] 
    ...
    state: Срочный рынок_Ещё_Комиссия || sessionResultColor = "#15952F"
        a: При торговле на срочном рынке FORTS возникает два вида комиссий — биржевая и брокерская.
                ...
                ✅ Выберите свой тариф:
        buttons:
            "Единый Тест-Драйв" -> /Срочный рынок_Ещё_Комиссия_Долгосрочный портфель
            "Стратег" -> /Срочный рынок_Ещё_Комиссия_Стратег
            "Инвестор" -> /Срочный рынок_Ещё_Комиссия_Долгосрочный портфель
            "Единый Дневной" -> /Срочный рынок_Ещё_Комиссия_Долгосрочный портфель
    ...
    state: Срочный рынок_Ещё_Комиссия_Долгосрочный портфель
        a: 1. Московская биржа:
            ✅ Заявка через ИТС:
            — фьючерсы и опционы на фьючерсы – 0,45 ₽ за контракт 
            — опционы на акции – 0,20 ₽ за контракт или 2 % от общего оборота по сделкам с опционами (наибольшее из двух значений в рамках торгового дня по счету)
            ✅ Заявка с голоса (или закрытие брокером):
            — фьючерсы – 0,0354% от стоимости контракта
            — опционы – 2 ₽ за контракт
            2. Биржи США:
            ✅ Заявка через ИТС:
            — стандартные фьючерсы и мини-фьючерсы – 10 $ за контракт 
            — микро – 5 $ за контракт
    ...
    state: Срочный рынок_Ещё_Комиссия_Стратег
        a: 1. Московская биржа:
            ✅ Заявка через ИТС:
            — фьючерсы и опционы на фьючерсы – 0,9 ₽ за контракт 
            — опционы на акции – 0,20 ₽ за контракт или 2 % от общего оборота по сделкам с опционами (наибольшее из двух значений в рамках торгового дня по счету)
            ✅ Заявка с голоса (или закрытие брокером):
            — фьючерсы – 0,0354% от стоимости контракта
            — опционы – 2 ₽ за контракт
    ...
    state: Срочный рынок_Ещё_Комиссия_Консультационный ФОРТС
        a: ✅ Заявка через ИТС:
            — фьючерсы и опционы на фьючерсы – 4,65 ₽ за контракт 
            — опционы на акции – 0,20 ₽ за контракт или 2 % от общего оборота по сделкам с опционами (наибольшее из двух значений в рамках торгового дня по счету)
            ✅ Заявка с голоса (или закрытие брокером):
    ...
    state: Срочный рынок_Ещё_Ошибки || sessionResultColor = "#BC3737"
        a: При торговле на срочном рынке нужно знать ряд моментов:
            1. Чтобы получить доступ к срочному рынку, нужно [пройти тестирование «Производные финансовые инструменты»|https://lk.finam.ru/Questionnaire/Questionnaires] в личном кабите.
    ...
    state: Срочный рынок_Ещё_Тестирование
        a: Для того, чтобы начать торговать на срочном рынке, в личном кабинете [пройдите тестирование на тему «Производные финансовые инструменты»|https://lk.finam.ru/Questionnaire/Questionnaires]
    ...
    state: Валютный рынок || sessionResultColor = "#15952F"
        ...
        a: ❗ Со 2 июля ограничена торговля валютными парами с юанем в режиме TOD, сделки с валютной парой CNY/RUB с расчётами «сегодня» недоступны.
        a: ❗ В связи с введением США ограничительных мер в отношении Московской биржи, с 13 июня 2024 года приостановлено проведение биржевых торгов и расчетов в валютах доллар США, евро и гонконгский доллар. [Подробнее на бирже.|https://www.moex.com/n70160?nt=0]
        a: ✅ НКЦ конвертировал гонконгские доллары (HKD), находящиеся в свободном остатке на счетах инвесторов, в китайский юань (CNY). [Новость на бирже.|https://www.moex.com/n70640?nt=0] 
        a: ✅ Согласно [новости на Московской бирже|https://www.moex.com/n70579], конвертация долларов США и евро, заблокированных в НКЦ, в рубли РФ осуществлена по центральному курсу, установленному на 13 июня 2024 года по итогам биржевых торгов 11 июня 2024 года.
        a: Пожалуйста, выберите один из предложенных вариантов:
        buttons:
            "Сервис конвертации валют на внебирже" -> /Сервис конвертации валют
    ...
    state: Валютный рынок_Как купить
        a: ❗ В связи с введением США ограничительных мер в отношении Московской биржи, с 13 июня 2024 года приостановлено проведение биржевых торгов и расчетов в валютах доллар США, евро и гонконгский доллар. [Подробнее на сайте биржи|https://www.moex.com/n70160?nt=0]
        a: ❗ Со 2 июля ограничена торговля валютными парами с юанем в режиме TOD, сделки с валютной парой CNY/RUB с расчётами «сегодня» недоступны.
    ...
    state: Валютный рынок_Комиссия брокера
        a: Комиссия брокера зависит от выбранного тарифа.
            Выберите тариф:
        buttons:
            "Долгосрочный портфель" -> /Валютный рынок_Комиссия брокера_Долгосрочный портфель
            "Стратег" -> /Валютный рынок_Комиссия брокера_Долгосрочный портфель
            "Инвестор" -> /Валютный рынок_Комиссия брокера_Долгосрочный портфель
            "Единый Дневной" -> /Валютный рынок_Комиссия брокера_Долгосрочный портфель
    ...
    state: Валютный рынок_Комиссия брокера_Долгосрочный портфель
    ...
    state: Валютный рынок_Ввод и вывод_Хранение валюты
        a: ✅ С 17 июня 2024 года хранение валюты на брокерском счёте в «Финам» – бесплатно
            ✅ Комиссия за хранение валюты также не взимается по счетам «Сегрегированный Global»
        buttons:
            "Сегрегированный Global" -> /Сегрегированный
    ...
    state: Валютный рынок_Инструменты
        a: ✅ Сделки с валютными парами доступны как в рамках стандартных брокерских договоров, так и договоров ИИС.
            ✅ В рамках счетов АО «Финам» предоставляется доступ к валютным парам: китайский юань/российский рубль, турецкая лира/российский рубль, белорусский рубль/российский рубль, казахстанский тенге/российский рубль, армянский драм/российский рубль, узбекский сум/российский рубль.
    ...
    state: Драгметаллы_Комиссии_Купить физическое золото || sessionResultColor = "#CD4C2B"
        a: Клиенты «Финам» могут купить запатентованные золотые слитки и монеты 999,9 пробы и любого веса по индивидуальной цене.
            ✅ [Узнать подробнее|https://www.finam.ru/landing/golden-house-by-shakhovskaya-and-finam/%20/]
    ...
    state: Драгметаллы_Комиссии
        a: При торговле контрактами на металлы возникает два вида комиссий: биржевая и брокерская.
            ...
            ✅ серебро — при покупке — 0,006375%, но не менее 1 ₽, при продаже — 0,017875%, но не менее 1 ₽
            2. Комиссия брокера: 
            ✅ по тарифам «Единый дневной», «Инвестор» и «Дневной валютный» — 0,05% при обороте до 1000000 ₽ за торговую сессию
            ✅ «Стратег», «Консультационный» и «Долгосрочный портфель» — 0,3% от оборота
    ...
    state: Маржа || sessionResultColor = "#15952F"
        ...
        buttons:
            ...
            "Подключить маржинальную торговлю" -> /Маржа_Подключить
            "Отключить маржинальную торговлю" -> /Маржа_Отключить
            "Комиссия за маржинальную торговлю" -> /Комиссии_Другие_Маржинальная
    ...
    state: Маржа_Подключить
        a: Подключить маржинальную торговлю можно двумя способами:
            ✅ пройти тестирование по категории [«Необеспеченные сделки»|https://lk.finam.ru/Questionnaire/Questionnaires]
            ✅ получить статус квалифицированного инвестора
            ❗ После выполнения одного из условий подключения, функция активируется автоматически в течение дня.
        script:
            $session.request = $request;

            $client.testsPassedCheck = testsPassedCheck($session.request.data.authenticationId, "Маржа");
            
            if ($client.testsPassedCheck == "isQualified"){
                $reactions.transition("/Маржа_Подключить/Клиент Квал");
            }
            if ($client.testsPassedCheck == "complied"){
                $reactions.transition("/Маржа_Подключить/Тесты пройдены");
            }
            if ($client.testsPassedCheck == "notComplied"){
                $reactions.transition("/Маржа_Подключить/Не пройдены тесты");
            }


        state: Клиент Квал
            a: ✅ У вас открыт доступ к маржинальной торговле, так как вы являетесь квалифицированным инвестором.
                ❗ Если маржинальная торговля всё же недоступна, либо вы отключали её ранее, обратитесь к менеджеру поддержки.
            
        state: Тесты пройдены
            a: ✅ У вас открыт доступ к маржинальной торговле, так как у вас пройдено тестирование на тему «Необеспеченные сделки». 
                ❗ Если маржинальная торговля всё же недоступна, либо вы отключали её ранее, обратитесь к менеджеру поддержки.
         
        state: Не пройдены тесты
            a: ❗ У вас не соблюдены условия для подключения маржинальной торговли.
                ❗ Если вы выполнили условия сегодня, ожидайте подключения в течение дня.
 
            
            
    state: Маржа_Отключить
        a: Для отключения маржинальной торговли нужно обратиться к менеджеру компании. Самостоятельно отключить маржинальную торговлю через личный кабинет и торговый терминал нельзя.  
        ...
        script:
            ...
            if ($client.testsPassedCheck == "notComplied"){
                $reactions.transition("/Маржа_Отключить/Не пройдены тесты");
            }
         
        state: Не пройдены тесты
            a: ❗ У вас не подключена маржинальная торговля, так как не соблюдено ни одно из условий:
                ✅ не пройдено тестирование по категории [«Необеспеченные сделки»|https://lk.finam.ru/Questionnaire/Questionnaires]
                ✅ отсутствует статус квалифицированного инвестора

    state: Маржа_Уровень маржи
        a: 1. Информацию о состоянии портфеля, значениях маржи и запас портфеля до принудительного закрытия можно посмотреть в [личном кабинете|https://lk.finam.ru/Home/Account/Details] — раскрыть строку «Показатели риска»
    ...
    state: Маржа_Принудительное закрытие
        a: ❗ Использование заёмных средств (плеча) ведёт к увеличению доходности и риска по сравнению с торговлей на собственные средства.
            ...
            ✅ Резкие колебания рыночных цен, которые повлекли уменьшение стоимости вашего портфеля ниже уровня Минимальных требований (Минимальной маржи).
            ...
            ✅ Принудительное закрытие может быть вызвано изменением значений ставок риска, рассчитываемых клиринговой организацией.
    ...
    state: КВАЛ || sessionResultColor = "#15952F"
        ...
        a: Статус квалифицированного инвестора открывает доступ к большему количеству финансовых инструментов: 
            ...
            ✅ [Проверить ваш инвестиционный статус (квал/неквал инвестор) в личном кабинете|https://lk.finam.ru/Client/InvestStatus] 
    ...
    state: КВАЛ_Статус_Активы
        a: Владеете активами или денежными средствами на брокерских и банковских счетах на общую сумму от 6000000 ₽. При определении общей стоимости активов также учитываются финансовые инструменты, переданные в доверительное управление.
            ...
            3. Направить документы можно в этом чате 
        buttons:
            "Назад" -> /КВАЛ_Статус
    ...
    state: КВАЛ_Статус_Опыт работы
        a: 1. Имеете опыт работы от двух лет, непосредственно связанный с совершением сделок с финансовыми инструментами, подготовкой индивидуальных инвестиционных рекомендаций, управления рисками, связанными с совершением указанных сделок, в российской и (или) иностранной организации.
            ...
            4. Направить документы можно в этом чате
        buttons:
            "Назад" -> /КВАЛ_Статус
    ...
    state: КВАЛ_Статус_Образование
        a: 1. У Вас есть диплом о высшем экономическом образовании государственного образца РФ, выданный организацией, которая на момент выдачи диплома осуществляла аттестацию граждан в сфере профессиональной деятельности на рынке ценных бумаг. Проверить аккредитацию ВУЗа можно на сайте ЦБ РФ: https://www.cbr.ru/vfs/finmarkets/files/supervision/list_Accred_org.xlsx
            ...
            5. Направить документы можно в этом чате
        buttons:
            "Назад" -> /КВАЛ_Статус
    ...
    state: КВАЛ_Статус_Другой брокер
        a: Если у вас уже есть статус квалифицированного инвестора у другого брокера, то можно подтвердить его, предоставив выписку из реестра квалифицированных инвесторов.
            ...
            ✅ Направить документы можно в этом чате 
    ...
    state: КВАЛ_Документы
        a: В личном кабинете можно [заказать выписку из реестра квалифицированных лиц «Финам».|https://lk.finam.ru/orders/QualifiedInvestorRequestStatementStatus]
        buttons:
            "Назад" -> /КВАЛ
    ...
    state: КВАЛ_Тестирование
        a: Прохождение всех тестирований для неквалифицированных инвесторов не подразумевает присвоение статуса квалифицированного инвестора, но открывает доступ ко многим категориям инструментов. 
            ✅ [Пройти тестирование в личном кабинете|https://lk.finam.ru/Questionnaire/Questionnaires]
            ✅ Доступ к инструментам и сделкам предоставляется сразу после прохождения тестирования, для этого нужно подписать результат прохождения тестирования в личном кабинете в [разделе «Результаты»|https://lk.finam.ru/Questionnaire/QuestionnairesResults] 
    ...
    state: КВАЛ_Смена профиля || sessionResultColor = "#B65A1E"
        a: Инвестиционный профиль (риск-профиль) – характеристика инвестора, его своеобразный портрет, описывающий поведение на финансовом рынке, готовность принимать риски.
            ✅ [Смена риск-профиля доступна в личном кабинете|https://lk.finam.ru/Client/InvestProfileNatural] 
        buttons:
            "Назад" -> /КВАЛ
    ...
    state: Депозитарное поручение || sessionResultColor = "#D93275"
        ...
        a: ❗ После перевода СПБ Биржей бумаг на неторговый раздел, бумаги исключены из торговых лимитов биржи, и не отображаются в терминале, но их наличие отражено во вкладке «Портфель» в [личном кабинете|https://lk.finam.ru/].
            ❗ Заблокированные бумаги, которые учитывались в СПБ Банке на субсчетах с разделами типа BN, были переведены 15.02.2024 на новый счет депо типа Y в СПБ Банке, данная операция отображается в [«Брокерских операциях»|https://lk.finam.ru/] по счету как «Перевод ценных бумаг». Нажмите кнопку ниже, чтобы узнать подробнее.
        a: ✅ Заявки на ввод/вывод ценных бумаг (ЦБ) можно сформировать дистанционно в личном кабинете, в разделе [«Операции с ценными бумагами»|https://lk.finam.ru/orders/capitalIssuesTransfer/selectAccount.aspx]
            ✅ Реквизиты депозитарного счета для ввода/вывода ценных бумаг также доступны в [личном кабинете|https://lk.finam.ru/global/Requisites/DepoAdmission.aspx].
            ✅ Статус перевода ЦБ можно отслеживать в [«Истории операций»|https://lk.finam.ru/Journals/UnionDocumentJournal].
    ...
    state: Депозитарное поручение_Перевод другой брокер || sessionResultColor = "#BC3737"
        a: ✅ Как перевести бумаги от другого брокера в «Финам»?
            Шаг 1: в личном кабинете нужно взять [депозитарные реквизиты|https://lk.finam.ru/global/Requisites/DepoAdmission.aspx]
            Шаг 2: по данным реквизитам подать поручение на вывод активов у другого брокера
            Шаг 3: подать поручение на ввод ценных бумаг в личном кабинете [по ссылке|https://lk.finam.ru/orders/capitalIssuesTransfer/selectAccount.aspx]  
    ...
    state: Депозитарное поручение_Перевод между счетами
        a: 1. Поручение на перевод ценных бумаг между своими счетами можно подать в [личном кабинете|https://lk.finam.ru/orders/AccountSecurityTransfer]. Выберите счет списания, затем счет зачисления и ценные бумаги с указанием их количества, которые хотите перевести.
            ...
            ✅ Подача поручения на прием/снятие ценных бумаг со сменой прав собственности по ссылке: https://lk.finam.ru/orders/capitalIssuesTransfer/selectAccount.aspx 
            ...
            ✅ Статус перевода ЦБ можно отслеживать в [«Истории операций»|https://lk.finam.ru/Journals/UnionDocumentJourna]

    state: Депозитарное поручение_Перевод ДЕПО
        a: Подать поручение на перемещение ценных бумаг между торговыми разделами или площадками (биржами) можно через личный кабинет по ссылке: https://lk.finam.ru/Orders/ChangeSecurityPlace
    ...
    state: Депозитарное поручение_Предоставить документы
        a: Важно предоставить затратные документы по введенным ценным бумагам до конца года, в котором они были проданы.
            ✅ Уведомление с перечнем необходимых документов будет отображено в личном кабинете после ввода ценных бумаг по ссылке: https://lk.finam.ru/info/debts.aspx 
    ...
    state: Депозитарное поручение_Счеn депо типа Y
        a: Заблокированные иностранные ценные бумаги, которые учитывались в СПБ Банке на субсчетах с разделами типа BN, были переведены 15.02.2024 на новый счет депо типа Y в СПБ Банке. 
            ...
            ❗ Если в [«Брокерских операциях»|https://lk.finam.ru/] по счету отображается частичный перевод бумаг, то это означает, что остальные заблокированные акции хранились на разделе неторгового счета, где данный перевод не требовался, или был уже произведен ранее, при предыдущих блокировках.
    ...
    state: Движение ДС || sessionResultColor = "#15952F"
        ...
        a: ✅ С 17 июня отменена комиссия за хранение валюты на брокерском счёте в «Финам».
    ...
    state: Движение ДС_Пополнение
        intent!: /015 Движение ДС/015 Движение ДС_Пополнить
        a: Пополнение доступно в валюте рубль РФ, доллар США, китайский юань, белорусский рубль, казахстанский тенге.
            ✅ [Пополнить счёт можно в личном кабинете.|https://lk.finam.ru/Tiles/MoneyIn]
    ...
    state: Движение ДС_Пополнение_СБП
        a: В личном кабинете доступно [пополнение брокерского счета через систему быстрых платежей|https://lk.finam.ru/money/topup/sbp]
            ❗ Для успешного пополнения данные отправителя и получателя должны совпадать (ФИО, номер телефона). Если у вас изменились ФИО или номер телефона, их можно изменить в разделе личного кабинета [«Изменение анкетных данных»|https://lk.finam.ru/Client/EditInfo]
    ...
    state: Движение ДС_Пополнение_Карта
        a: [Пополнение брокерского счета с помощью банковской карты|https://lk.finam.ru/money/topup/bankcardtopup]
    ...
    state: Движение ДС_Пополнение_Реквизиты
        a: [Пополнение брокерского счёта по реквизитам|https://lk.finam.ru/money/topup/requisites]
    ...
    state: Движение ДС_Перевод
        intent!: /015 Движение ДС/015 Движение ДС_Перевод между счетами
    ...
    state: Движение ДС_Перевод_Средств
        a: Поручение на перевод денежных средств можно [подать в личном кабинете|https://lk.finam.ru/orders/moneyTransfer/mmvb/selectAccount.aspx]. Выберите счёт списания, затем счёт зачисления и сумму, которую хотите перевести.
    ...
    state: Движение ДС_Вывод || sessionResultColor = "#CD4C2B"
        intent!: /015 Движение ДС/015 Движение ДС_Вывод
        a: ✅ Рубли РФ можно выводить на счета сторонних банков [в личном кабинете любым удобным способом.|https://lk.finam.ru/Tiles/MoneyTransfer] 
            ✅ Выводы валют доллар США*, евро*, китайский юань и казахстанский тенге доступны по реквизитам счёта.
            * Вывод долларов США и евро доступен только на счета банка «Финам» и «Райффайзенбанка».
            ✅ После исполнения вывода поступает уведомление от брокера с указанием фактической суммы вывода и удержанной суммы налога.
        a: ✅ Выберите способ вывода средств, чтобы узнать подробнее:
        buttons:
    ...
    state: Движение ДС_Вывод_СБП
        a: Поручение на вывод денежных средств по СБП можно [подать в личном кабинете|https://lk.finam.ru/money/moneytransfer/SbpMoneyOut]
    ...
    state: Движение ДС_Вывод_Реквизиты
        a: Вывод денежных средств происходит не ранее проведения биржевых расчётов по сделкам в соответствии с режимом расчётов Т+1/Т+2. 
            Валютные резиденты РФ могут выводить валюту с брокерского счета только в банки РФ.
            ✅ Валюты рубль РФ, казахстанский тенге и китайский юань можно вывести в личном кабинете [по реквизитам.|https://lk.finam.ru/money/moneytransfer/MoneyOutByRequisites]
            ✅ Дополнительно можно воспользоваться опцией «Срочный вывод» рублей РФ (вывод происходит на 1 день раньше, услуга платная).
            ✅ Вывод долларов США и евро доступен только на счета банка «Финам» и «Райффайзенбанка»:
                — [Подать поручение на вывод долларов США можно в личном кабинете|https://lk.finam.ru/orders/MoneyOut/MoneyOut/Default.aspx] → выбрать тип вывода денежных средств «Иностранная валюта».
                — Подать поручение на вывод евро можно через менеджера поддержки.
            ✅ При выводе валюты на счета в банке «Финам» комиссия отсутствует. Но есть комиссия со стороны банка за хранение долларов и евро от 3000 единиц валюты.
            ❗ При выводе средств в иностранной валюте на счета в сторонних банках удерживается комиссия. 
        a: ✅ Выберите, чтобы узнать подробнее:
        buttons:
    ...
    state: Движение ДС_Вывод_Реквизиты_Комиссии
        a: ✅ За вывод рублей РФ комиссия не взимается.
            ✅ Опция «срочный вывод» рублей РФ — 300 ₽ (по тарифам «Дневной СПБ», «Консультационный СПБ», «Cтратег US» комиссия 7,5 $). Ограничение по сумме вывода: от 1000 ₽ до 5000000 ₽ (но не более 80% от счета).
            ✅ За вывод иностранной валюты на счета в Банке «Финам» комиссия не взимается, зачисление валюты на счета банка – без комиссии, хранение в банке от 3000 единиц валюты – 0,03% в день.
            ✅ За вывод иностранной валюты физическим лицом на счета по реквизитам в сторонние банки взимаются комиссии:
            — китайские юани, казахстанские тенге — 0,07% от суммы вывода, но не менее 25 € и не более 100 €
            — доллары США — 0,3% от суммы вывода, но не менее 30 $ и не более 150 $, дополнительно удерживается комиссия за обработку поручений на вывод долларов — 0,4% от суммы вывода, но не менее 1500 ₽ и не более 250000 ₽
            ✅ Комиссии удерживаются в рублях по курсу ЦБ РФ на дату исполнения вывода (по тарифам «Дневной СПБ», «Консультационный СПБ», «Cтратег US» комиссия удерживается в долларах США). 
        buttons:
            "Комиссии банка «Финам»" -> /Банк_Банковский счет_ЗачислениеХранение Валюты
    ...
    state: Движение ДС_Вывод_Наличными
        a: Оформлять и получать вывод средств наличными с брокерского счета можно только при условии, что вы уже подтверждали ранее свою личность при личном визите в офисе компании.
            ...
            ✅ [Подать поручение можно в личном кабинете|https://lk.finam.ru/money/moneytransfer/MoneyOutCash]
    ...
    state: Движение ДС_Вывод_24-7 || sessionResultColor = "#CD4C2B"
        a: Чтобы не ожидать расчеты по сделкам и выводить деньги с брокерского счета в удобное для вас время, даже в выходные и праздничные дни, можно подключить услугу «Вывод 24/7». Денежные средства после продажи ценных бумаг можно мгновенно выводить на карту в Банке «Финам».
            ...
            1. [подключить услугу можно в личном кабинете|https://lk.finam.ru/money/moneytransfer/MoneyOutOverdraft] в рабочие дни с 10:00 до 18:00 МСК 
            ...
            3. по факту подключения услуги поступает уведомление по SMS, с этого момента можно использовать данную услугу [в личном кабинете в разделе «Вывод»|https://lk.finam.ru/money/moneytransfer/MoneyOutOverdraft]
    ...
    state: Документы_Общие_Открытие счета || sessionResultColor = "#B65A1E"
        a: Документы, подтверждающие открытие брокерского счета, находятся в [личном кабинете|https://lk.finam.ru/catalog/documents.aspx]
            Основными документами об открытии счета являются: 
            ...
            — «Уведомление для ИФНС» (документ находится в [«Журнале уведомлений»|https://lk.finam.ru/Journals/NotificationJournal] и не является обязательным)

    state: Документы_Общие_Реквизиты
        a: Реквизиты для ввода денежных средств и ценных бумаг доступны в разделе личного кабинета [«Реквизиты для перевода средств»|https://lk.finam.ru/global/Requisites/Forex.aspx] 

    state: Документы_Общие_Соглашение Казахстан
        a: «Дополнительное соглашение об ограничении суммы инвестирования» для банков Казахстана можно получить в личном кабинете в разделе [«Основные документы»|https://lk.finam.ru/catalog/documents.aspx] 
    ...
    state: Документы_Общие_Согл для торговли иностранными активами || sessionResultColor = "#CD4C2B"
        a: ✅ «Согласие на торговые операции с заблокированными иностранными ценными бумагами» для торговли на внебирже доступно по [ссылке|https://lk.finam.ru/ForeignSecurities/BlockedSecuritiesConsent]
        buttons:
    ...
    state: Документы_Налоговые_2-НДФЛ
        a: 1. Справка 2-НДФЛ — официальный документ по форме ИФНС РФ о рассчитанных доходах и удержанных с них налогах за отчетный период (календарный год).
            ✅ [Заказать справку 2-НДФЛ в личном кабинете|https://lk.finam.ru/orders/Ndfl]
    ...
    state: Документы_Налоговые_Справка об убытках
        a: Справка об убытках — официальный документ по форме ИФНС РФ о рассчитанных убытках за отчетный период (календарный год), доступна для формирования за последние 10 лет (за каждый убыточный период заказывается отдельная справка).
            ✅ [Заказать справку об убытках в личном кабинете|https://lk.finam.ru/orders/NdflLoss]
            ✅ Электронный формат справки будет доступен в личном кабинете в течение 3-х рабочих дней.
            Изготовление справки на бумажном носителе в течение одной рабочей недели.

    state: Документы_Налоговые_Справка 1042S || sessionResultColor = "#CD4C2B"
        a: Справку формы 1042s формируют НРД и СПБ-биржа и направляют брокеру. Готовые формы 1042S загружаются автоматически в личном кабинете. 
            ✅ [Скачать форму 1042S|https://lk.finam.ru/orders/Form1042S]
    ...
    state: Документы_Налоговые_Справка госслужащего || sessionResultColor = "#CD4C2B"
        a: Справку для госслужащего (по форме 5798-У) можно [запросить в личном кабинете|https://lk.finam.ru/Reports/Tax]
            ...
            Для получения сведений об остатках средств на брокерских счетах можно получить [справку по счету|https://lk.finam.ru/Home/Account/Info] 
    ...
    state: Документы_Депозитарные_Справка об активах
        a: Справку об активах за закрытую торговую сессию можно [запросить в личном кабинете|https://lk.finam.ru/orders/AssetStatement]
            Изготовление справки занимает до двух рабочих дней.
            
    state: Документы_Депозитарные_Выписки из депозитария
        a: Депозитарные документы можно [заказать в личном кабинете|https://lk.finam.ru/Orders/DepoReport]
    ...
    state: Справка по счету || sessionResultColor = "#15952F"
        ...
        a: ✅ НКЦ конвертировал гонконгские доллары (HKD), находящиеся в свободном остатке на счетах инвесторов, в китайский юань (CNY). [Новость на бирже|https://www.moex.com/n70640?nt=0]
        a: ❗ После перевода СПБ Биржей бумаг на неторговый раздел, бумаги исключены из торговых лимитов биржи, и не отображаются в терминале, но их наличие отражено во вкладке «Портфель» в [личном кабинете.|https://lk.finam.ru/]
        a: ✅ Детальное описание операций и сделок, цены покупок и продаж, начисления и списания, корпоративные действия с бумагами в портфеле и другие движения по счету можно изучить в [Справке по счету.|https://lk.finam.ru/Home/Account/Info]
            ...
            ✅ Также в личном кабинете автоматически выгружаются [брокерские отчеты|https://lk.finam.ru/info/reports.aspx] на подписание.
            ✅ Историю операций в онлайн режиме можно посмотреть в личном кабинете [в разделе «Брокерские операции»|https://lk.finam.ru/Journals/AccountOperationsJournal], а также в [FinamTrade|https://trading.finam.ru/] (Android, IOS и Web) 
    ...
    state: Справка_Торговый оборот
        a: ✅ Оборот за выбранный закрытый торговый период можно изучить в [Справке по счету|https://lk.finam.ru/Home/Account/Info] в разделе справки «Виды движений денежных средств».
            ✅ Торговый оборот за последние 4 завершенных квартала с целью получения статуса квалифицированного инвестора можно уточнить в личном кабинете [в разделе «Инвестиционный статус»|https://lk.finam.ru/Client/InvestStatus]
    ...
    state: Форма W8BEN || sessionResultColor = "#CD4C2B"
        ...
        a: В личном кабинете можно [подписать форму W-8BEN для Московской биржи».|https://lk.finam.ru/Orders/FormW8BEN]
            ...
            ✅ Срок действия подписанной формы три года. По истечении этого срока, а также в случае изменения персональных данных, ее необходимо оформить заново.
            ❗ С 1 января 2024 года СПБ Клиринг приостановил обработку идентификаций бенефициаров доходов, так как даже в случае предоставления корректных форм W-8ben к таким бенефициарам применяется штрафная ставка американского налога – 30%, так же отменены ранее подписанные формы W-8BEN для СПБ биржи.
    ...
    state: Личный кабинет_Торговый код
        a: Торговый код присваивается к каждому брокерскому счету. Узнать свой торговый код можно в личном кабинете в [разделе «Детали» счёта|https://lk.finam.ru/Home/Account/Details]
    ...
    state: Личный кабинет_Торговый код_Кодовое слово || sessionResultColor = "#CD4C2B"
        a: Для дополнительной защиты ваших персональных данных, в компании «Финам» вводится обязательный способ идентификации клиентов - кодовое слово. С 1 сентября 2024 года клиенты компании смогут получить персональную информацию или подать голосовую заявку только после того, как назовут сотруднику своё кодовое слово.
            ✅ Создать, изменить или удалить кодовое слово можно в личном кабинете в [разделе «Персональные данные»|https://lk.finam.ru/Client/CodeWords] 
    ...
    state: Замена персональных данных || sessionResultColor = "#418614"
        ...
        a: ✅ Подать поручение на смену паспортных данных, номера телефона, электронной почты можно в личном кабинете: https://lk.finam.ru/Client/EditInfo
    ...
    state: Авторизация_Сайт Finam.ru
        a: После регистрации на сайте [Finam.ru|https://www.finam.ru/] вам придет письмо со сгенерированными никнеймом и паролем. В качестве логина для авторизации на сайте можно использовать:
            — никнейм
    ...
    state: Подпись || sessionResultColor = "#418614"
        intent!: /023 Подпись
        a: ✅ Подписать документы можно в личном кабинете в [разделе «Журнал неподписанных документов»|https://lk.finam.ru/info/UnSignedDocuments.aspx]
            Пожалуйста, выберите один из предложенных вариантов:
        buttons:
    ...
    state: Подпись_Электронная
        a: Перед созданием ключа ЭП в личном кабинете перейдите в [раздел «Инструкции, шаблоны, ПО»,|https://lk.finam.ru/global/software.aspx] скачайте и установите «Плагин для генерации электронной подписи», он доступен на устройствах с системой Windows, необходимо использовать браузер Google Chrome.
            Далее следуйте по пунктам:
            1. В личном кабинете перейдите в [раздел «Создание ключей»|https://lk.finam.ru/Sign/CreateCertificate] и нажмите «Создать сертификат».
            2. В открывшемся диалоговом окне выберите пустую папку для хранения нового ключа и нажмите «Старт». Шевелите мышкой до заполнения индикатора.
            ...
            ✅ [Скачать расширение плагина для работы с электронной подписью через браузер Google Chrome|https://chrome.google.com/webstore/detail/signal-com-signature-plug/ceifjolbdjihdddppedlcocpgckafcak]
    
    state: Подпись_SMS
        a: Чтобы подписывать электронные документы в личном кабинете, [подключите SMS-подпись|https://lk.finam.ru/info/ApplicationSms/ApplicationSmsOn.aspx]
    ...
    state: Ошибки СМС_Почта
        ...
        a: Рекомендации при возникновении проблем с доставкой
            ...
            2. убедиться, что в личном кабинете указан предпочтительный адрес электронной почты в разделе [«информация о клиенте»|https://lk.finam.ru/client/info]
            3. изменить еmail можно в разделе [«изменение анкетных данных»|https://lk.finam.ru/Client/EditInfo]
    ...
    state: Банк || sessionResultColor = "#15952F"
        ...
        buttons:
            ...
            "Контакты Банка" -> /Банк_Контакты банка
    ...
    state: Банк_Банковская карта
        ...
        buttons:
            ...
            "Заблокировать карту" -> /Банк_Банковская карта_Заблокировать карту
            "Реквизиты карты" -> /Банк_Банковская карта_Реквизиты карты
            "Тарифы на обслуживание карт" -> /Банк_Банковская карта_Тарифы
            "Программа лояльности «CASHBACK»" -> /Банк_Банковская карта_Программа лояльности
    ...
    state: Банк_Платежи по СБП
        a: ✅ Максимальная сумма одного перевода/платежа с использованием Системы быстрых платежей (СБП) составляет 1000000 ₽
                ...
                1. «Комфорт» — до 200000 ₽ в месяц — без комиссии
                свыше 200000 ₽ в месяц — 0,5% от суммы перевода, не более 1500 ₽
                2. «Престиж» — до 500000 ₽ в месяц — без комиссии,
                свыше 500000 ₽ в месяц — 0,5% от суммы перевода, не более 1500 ₽
                3. «Корпоративный» — до 300000 ₽ в месяц — без комиссии, свыше 300000 ₽ в месяц — 0,5% от суммы перевода, не более 1500 ₽
                4. «Премиум» — до 5000000 ₽ — без комиссий
                ✅ При расчете максимальной суммы переводов, совершенных в течение календарного месяца, а также при расчете комиссии за исходящий перевод, учитывается совокупный объем денежных средств по исходящим переводам с использованием СБП за текущий календарный месяц по всем счетам клиента, открытым в Банке.
                ✅ Актуальные тарифы на [сайте Банка|https://www.finambank.ru/person/rates] в разделах «Пакет услуг Корпоративный» и «Пакеты услуг Комфорт, Престиж и Премиум»
    
    state: Банк_Контакты банка
        a: ✅ Обратиться в поддержку Банка:
                — по телефонам +7(495) 796-90-23 или +7 (800) 200-44-00 (бесплатно по России)
                — в чате
                — посетить офис компании «Финам». [Адреса и контактная информация|https://www.finambank.ru/about/offices]
    ...
    state: Банк_Банковская карта_Заблокировать карту
        a: Заблокировать банковскую карту 24/7 можно в интернет-банке, в мобильном приложении, в личном кабинете или позвонить в отдел поддержки.
            1. В мобильном приложении Банка «Финам» выбрать счёт нужной карты → нажать три точки в верхнем правом углу → нажать на иконку с изображением карты → снова нажать три точки в верхнем правом углу → Заблокировать
            2. В [личном кабинете|https://lk.finam.ru/] 
            ✅ выбрать нужный счет → в разделе таблицы «Владелец/Дата окончания» выбрать «Заблокировать»
    ...
    state: Банк_Банковская карта_Реквизиты карты
        a: Реквизиты вашей банковской карты можно получить в интернет-банке, в мобильном приложении или в личном кабинете.
            1. В мобильном приложении Банка «Финам»
            ✅ Посмотреть реквизиты карточного счета, скачать или поделиться файлом: 
            выберите счёт нужной карты → нажмите три точки в верхнем правом углу → Реквизиты для пополнения 
    ...
    state: Банк_Банковская карта_Программа лояльности
        a: Программа лояльности «CASHBACK» подключается автоматически при выпуске первой карты банка «Финам». Повышенный кешбэк по выбранным категориям до 7%, 1% — на остальные покупки.
            ✅ Выбирать и подключать категории можно в [интернет-банке|https://ibank.finam.ru/] и в мобильном приложении банка. 
            ✅ Подключенные категории устанавливаются на все карты, выпущенные в рамках одного пакета услуг. При первом подключении категории повышенная ставка начинает действовать на следующий день. Менять категории можно 1 раз в месяц, действие новой категории начнется с 1-ого числа следующего месяца.
            ✅ Минимальная сумма операций по карте и количество подключенных категорий зависит от подключенного пакета услуг, подробнее:
    ...
    state: Контакты || sessionResultColor = "#418614"
        intent!: /028 Контакты
        # if: holidays()
            # a: ✅ 12 июня 2024 – Нерабочий день для всех офисов компании «Финам».
        a: ❗ Служба технической поддержки «Финам» работает 24/7
            ...
            ✅ перед визитом в центральный офис «Финам» в Москве, Настасьинский пер. дом 7, стр.2, можно заказать парковочное место, предварительно обратившись к менеджеру компании
        buttons:
            "Перевод на оператора" -> /Перевод на оператора
    ...
    state: Доверенности || sessionResultColor = "#418614"
        ...
        a: Шаблон доверенности для физических лиц доступен в личном кабинете в [разделе «Инструкции, шаблоны, ПО».|https://lk.finam.ru/global/Requisites/Warrant.aspx]
    ...
    state: Доступные биржи || sessionResultColor = "#418614"
        ...
        if: holidays()
            a: ✅ 15 июля — Праздничный день в Турции. Торги и расчеты с турецкой лирой в режиме _TOD и СВОП не проводятся.
    ...
    state: Доступные биржи_Московская
        a: На Московской бирже предоставляется доступ к:
            ...
            ✅ Валютному рынку — валютные пары (турецкая лира, китайский юань, белорусский рубль, казахстанский тенге, армянский драм, узбекский сум)
    ...
    state: Доступные биржи_NYSE и NASDAQ
        a: В рамках биржи NYSE/NASDAQ предоставляется доступ к иностранным акциям и фондам.
            ...
            ✅ «Единым счетам» (кроме счетов, открытых в период с 15.08.2022 по 13.02.2023 и с 13.06.2024)
            ✅ по счетам «US Market Options» 
            ✅ по счетам «Сегрегированный Global»
            ✅ счетам «Иностранные биржи» (с 1.02.2024 открытие новых счетов недоступно)
            ❗ Для работы с данными инструментами требуется статус квалифицированного инвестора.
    ...
    state: Доступные биржи_Опционы США
        a: «Финам» предоставляет доступ к американским опционам на иностранные акции.
            Доступ предоставляется по:
            ✅ «Единым счетам» (для получения доступа обратитесь к менеджеру)
            ✅ по счетам «US Market Options» (с 13.06.2024 открытие новых счетов недоступно)
            ✅ по счетам «Сегрегированный Global»
            ✅ счетам «Иностранные биржи» (с 1.02.2024 открытие новых счетов недоступно)
            ❗ Для работы с данными инструментами требуется статус квалифицированного инвестора.
            ❗ Все расчеты производятся в долларах США, автоконвертация валюты при покупке не осуществляется.
    ...
    state: Доступные биржи_Ещё_Гонконгская
        a: ❗ Доступно только закрытие позиций.
    ...
    state: Доступные биржи_Ещё_Внебиржа
        a: Для клиентов «Финам» доступно совершение внебиржевых сделок:
            1. Внебиржевые сделки (кроме указанных ниже) — выставление заявок доступно через подписание поручения в личном кабинете: https://lk.finam.ru/Journals/UnionDocumentJournal 
    ...
    state: Время торгов_Московская биржа_Срочный рынок
        a: Торговая сессия начинается вечером и длится с 19:05 до 23:50, продолжается на следующий день — с 9:50 до 14:00 и с 14:05 до 18:50 МСК.
            Торговый календарь: https://www.moex.com/ru/tradingcalendar/

    state: Время торгов_Московская биржа_Валютный рынок
        a: ❗ В связи с введением США ограничительных мер в отношении Московской биржи, с 13 июня 2024 года приостановлено проведение биржевых торгов и расчетов в валютах доллар США, евро и гонконгский доллар. [Подробнее|https://www.moex.com/n70160?nt=0]
        a: ✅ Торги драгоценными металлами в режиме _TOM проводятся с 9:50 до 19:00 МСК
            ✅ Торги валютными парами в режиме _SPT, _TOM, _TMS проводятся с 9:50 до 19:00 МСК
            ✅ Торги по валютным парам в режиме _TOD и СВОП проводятся согласно регламенту брокерского обслуживания (Приложение 24.1) с 9:50 до:
            —  CNYRUB – 11:50
    ...
    state: Режим расчетов || sessionResultColor = "#418614"
        ...
        a: Торги на биржах осуществляются в режимах T+0, Т+1 и Т+2. Это значит, что регистрация прав на ценные бумаги/валюту происходит не в момент заключения сделки, а позднее.
            ...
            3. Торги валютой на Московской бирже осуществляются в режимах: T+0 (TOD), T+1 (TOM, TMS) и T+2 (SPT)
            4. Торги на срочном рынке (ПФИ) не имеют отложенных расчетов, но фактическое начисление вариационной маржи происходит только в основной клиринг (на FORTS в 18:50 — 19:00 МСК)
            5. Торги на СПБ Бирже российскими и квазироссийскими акциями проводятся в режиме Т+1
            6. Торги на бирже Гонконга HKEX проходят в режиме Т+2
            7. С 28 мая 2024 года американские биржи перешли на режим торгов Т+1. [Подробнее|https://www.finam.ru/publications/item/ssha-perekhodyat-na-t1-20240519-1800]
            8. Внебиржевые торги на ММВБ ОТС с ЦК осуществляются в режиме Т+1
            
    state: Ограничение ЦБ || sessionResultColor = "#418614"
        intent!: /048 Ограничения ЦБ
        a: ❗ Со 2 июля ограничена торговля валютными парами с юанем в режиме TOD, сделки с валютной парой CNY/RUB с расчётами «сегодня» недоступны.
        a: ❗ В связи с введением США ограничительных мер в отношении Московской биржи, с 13 июня 2024 года приостановлено проведение биржевых торгов и расчетов в валютах доллар США, евро и гонконгский доллар. [Подробнее на бирже.|https://www.moex.com/n70160?nt=0]
        a: ✅ НКЦ конвертировал гонконгские доллары (HKD), находящиеся в свободном остатке на счетах инвесторов, в китайский юань (CNY). [Новость на бирже.|https://www.moex.com/n70640?nt=0] (HKD, заблокированные на счетах в рамках санкций на СПБ Биржу, конвертированы не будут.) 
        a: ✅ Согласно [новости на Московской бирже|https://www.moex.com/n70579], конвертация долларов США и евро, заблокированных в НКЦ, в рубли РФ осуществлена по центральному курсу, установленному на 13 июня 2024 года по итогам биржевых торгов 11 июня 2024 года.
        ...
        buttons:
            "Обращение в OFAC" -> /Ограничение ЦБ_Обращение в OFAC
            "Сервис торговли заблокированными ЦБ" -> /Как закрыть позиции_Продажа БлокЦБ
            "Разблокировка ЦБ после конвертации ДР" -> /Депозитарное поручение_Ещё_Разблокировка ЦБ
            "Перевод активов из Банка в АО Финам" -> /Депозитарное поручение_Ещё_Перевод активов
    ...
    state: Ограничение ЦБ_Обращение в OFAC || sessionResultColor = "#B65A1E"
        a: У инвесторов есть право самостоятельно обратиться в OFAC, чтобы получить индивидуальные специальные разрешения на вывод иностранных активов из СПБ Банка в другие, не находящиеся под санкциями депозитарии. 
            ✅ Подробная информация, инструкции и шаблоны документов для самостоятельной подачи обращения в OFAC на сайте [разблокировкаактивов.рф|https://xn--80aaacfca8akdrcch9bchuh0c.xn--p1ai/]
            ✅ Полное наименование «Финам» на английском языке можно посмотреть в [разделе «Реквизиты»|https://zaoik.finam.ru/about/requisites/]
            ✅ Адрес указывается в соответствии с правилами английского языка: 7 Nastasinsky Lane, Building 2, Room 33, Moscow, 127006, Russia 
            1. Для клиентов «Финам» реализована возможность [заказать на английском языке выписку по счету ДЕПО|https://lk.finam.ru/Orders/DepoReport]
            ❗ Часть сведений в выписке по счету ДЕПО транслитерирована по стандарту ГОСТ, а не переведена на английский язык.
            2. «Документы на разблокировку активов в OFAC» можно заказать у менеджера поддержки.
            3. В ближайшее время будет реализована возможность заказать «Письмо - подтверждение от СПБ Банка о наличии договорных отношений СПБ Банка с депозитарием заявителя». (Ожидается информация от СПБ Банка)
    ...
    state: Ограничение ЦБ_844 || sessionResultColor = "#B65A1E"
        a: Прием заявок на обмен активов в рамках Указа №844 завершен.
            ✅ 6 мая завершился прием заявок на обмен заблокированных активов российских инвесторов в рамках Указа №844 «О дополнительных временных мерах экономического характера, связанных с обращением иностранных ценных бумаг». 
            ✅ Подробная информация, сроки и этапы выкупа в [презентации для инвесторов|https://vykupicb.investpalata.ru/documents/prezentacii/prezentaciya-dlya-roznichnyh-investorov/]
            ✅ Статус своей поданной заявки можно посмотреть в личном кабинете в [разделе «Журнал поручений»|https://lk.finam.ru/Journals/UnionDocumentJournal] 
    ...
    state: Санкции || sessionResultColor = "#418614"
        ...
        a: ✅ НКЦ конвертировал гонконгские доллары (HKD), находящиеся в свободном остатке на счетах инвесторов, в китайский юань (CNY). [Новость на бирже.|https://www.moex.com/n70640?nt=0] (HKD, заблокированные на счетах в рамках санкций на СПБ Биржу, конвертированы не будут.) 
        a: ✅ Согласно [новости на Московской бирже|https://www.moex.com/n70579], конвертация долларов США и евро, заблокированных в НКЦ, в рубли РФ осуществлена по центральному курсу, установленному на 13 июня 2024 года по итогам биржевых торгов 11 июня 2024 года.
        a: ❗ На СПБ Бирже приостановлены торги иностранными ценными бумагами. Актуальная информация на [сайте биржи|https://spbexchange.ru/ru/about/news2.aspx]
            ❗ После перевода СПБ Биржей бумаг на неторговый раздел, бумаги исключены из торговых лимитов биржи, и не отображаются в терминале, но их наличие отражено во вкладке «Портфель» в [личном кабинете.|https://lk.finam.ru/Home/Account]
    ...
    state: Корпоративные действия || sessionResultColor = "#418614"
        ...
        buttons:
            ...
            "Консолидация ВТБ" -> /Корпоративные действия_втб
    ...
    state: Корпоративные действия_Размещение || sessionResultColor = "#B65A1E"
        a: Инвесторам «Финам» доступно участие в публичных размещениях облигаций и ценных бумаг (IPO).
            ...
            — [в личном кабинете|https://lk.finam.ru/Ipo/Securities]
    ...
    state: Корпоративные действия_Замещение || sessionResultColor = "#B65A1E"
        a: Замещающие облигации — это локальные, выпущенные и обращающиеся в российской юрисдикции долговые бумаги, которые держатель получает взамен замещаемых евробондов.
            ✅ Подробная информация о доступных выпусках замещающих облигаций и подача заявки в личном кабинете в разделе [Публичные размещения акций и облигаций|https://lk.finam.ru/Ipo/Securities]
    ...
    state: Корпоративные действия_Оферта || sessionResultColor = "#B65A1E"
        a: Оферта по облигации — это возможность досрочно продать ее эмитенту. [Подробнее|https://www.finam.ru/publications/item/oferta-obligacii-chto-eto-zachem-o-neiy-znat-investoru-20200917-16480/]
            ✅ [Подать заявку на участие в оферте в личном кабинете|https://lk.finam.ru/orders/default.aspx?ts=OFFER%20BONDS]
    ...
    state: Корпоративные действия_Яндекс || sessionResultColor = "#B65A1E"
        a: МКПАО ЯНДЕКС — головная компания «Яндекса», к которой перешло большинство сервисов и активов Yandeх N.V. 
            ✅ 24 июля на Московской и СПБ биржах стартовали торги акциями МКПАО «Яндекс». Новый тикер Яндекса — YDEX. [Подробнее|https://ir.yandex.ru/press-releases?year=2024&id=01-24-07-2024]
            ❗ В связи с изменением торгового кода YNDX на YDEX, ранее выставленные стоп-заявки по инструменту YNDX не сработают. При необходимости нужно заново выставить стоп-заявки, указав код YDEX.
            ✅ Для держателей акций Yandex, приобретенных на Московской и СПБ Биржах, был проведен биржевой обмен акций. Подробнее на [сайте эмитента.|https://yatenderoffer.ru/%D0%BE%D0%B1%D1%89%D0%B8%D0%B5-%D0%B2%D0%BE%D0%BF%D1%80%D0%BE%D1%81%D1%8B/]
            ✅ Биржевой обмен проходил через проведение сделок продажи и покупки бумаг на бирже. Комиссии за сделки взимаются в соответствии с тарифным планом.
            ✅ Если комиссия списалась в минус в валюте доллар США, нужно пополнить счёт рублями РФ на недостающую сумму для погашения минуса, в течение нескольких дней валютный минус будет автоматически сконвертирован в рубли РФ.
        a: ❗ В обмене не участвовали акции Яндекса, приобретенные на иностранных биржах. Информация об обмене или выкупе такого актива не поступала. [Подробнее|https://yatenderoffer.ru/%D0%BE%D0%B1%D1%89%D0%B8%D0%B5-%D0%B2%D0%BE%D0%BF%D1%80%D0%BE%D1%81%D1%8B/]
            
    state: Корпоративные действия_втб
        a: ✅ С 15 июля возобновлены торги акциями ВТБ после проведения обратного сплита в пропорции 5000:1. 
            ❗ В течение дня возможно временно некорректное отображение графика котировок по VTBR. 
            ✅ С 16 июля на срочном рынке возобновляются торги фьючерсными контрактами и премиальными опционами на акции ПАО Банка ВТБ. Московская Биржа обращает внимание на уменьшение лота. [Подробнее|https://www.moex.com/n71114]
            ✅ Ранее Московская биржа опубликовала правила проведения торгов акциями ВТБ в связи с предстоящим обратным сплитом. [Подробнее|https://www.finam.ru/publications/item/s-13-iyunya-mosbirzha-vvedet-ogranicheniya-v-torgakh-aktsiyami-vtb-iz-za-obratnogo-splita-20240611-1446/]

    # state: Корпоративные действия_Элемент || sessionResultColor = "#B65A1E"
    #     a: Размещение ценной бумаги ПАО «Элемент» состоялось 30 мая 2024 года. Начисление ценных бумаг с местом хранения в НРД на счета инвесторов прошло 31 мая. 
    #         ✅ Московская биржа включила обыкновенные акции ПАО «Элемент» в свой котировальный список, торги ими начались 6 июня
    #         ✅ Бумагам присвоен торговый код ELMT
    ...
    state: Корпоративные действия_Другие
        a: Корпоративными действиями называются мероприятия эмитента, направленные на распределение доходов в денежной или иной форме между держателями ценных бумаг или изменение структуры ценных бумаг. Примеры:
            ...
            ❗ Реквизиты для зачисления средств необходимо [предоставить в личном кабинете|https://lk.finam.ru/orders/depoBankAccountDetails.aspx]
        ...
        buttons:
            ...
            "Обмен Solidcore (Polymetal)" -> /Корпоративные действия_Другие_Полиметалл
            "Назад" -> /Корпоративные действия
    ...
    state: Корпоративные действия_Размещение_Календарь
        a: Следите за предстоящими IPO в специальном [календаре|https://lk.finam.ru/Ipo/Securities]

    state: Корпоративные действия_Размещение_Отмена
        a: Чтобы отменить заявку на участие в IPO:
            ✅ зайдите в раздел личного кабинета [«Публичное размещение акций и облигаций»|https://lk.finam.ru/Ipo/Securities]
    ...
    state: Корпоративные действия_Другие_Как подать
        a: ✅ [Подать заявку на участие в добровольном выкупе ценных бумаг в личном кабинете|https://lk.finam.ru/Depo/CorporateActionsTypes]
            ✅ Заявка на участие в принудительном выкупе не требуется
            ❗ Реквизиты для зачисления средств необходимо [предоставить в личном кабинете|https://lk.finam.ru/orders/depoBankAccountDetails.aspx]
    
    state: Корпоративные действия_Другие_Полиметалл
        a: Solidcore Resources plc (ранее Polymetal) подала заявку на делистинг акций с Московской биржи. [Новость|https://www.finam.ru/publications/item/polymetal-podal-zayavku-na-delisting-aktsiy-s-mosbirzhi-20240625-0850/]
            ✅ Ценные бумаги эмитента можно продать до 30 сентября или вывести к уполномоченному брокеру для обмена.
            ✅ Подробная информация об участии в обмене акций и порядке действий на [сайте эмитента|https://www.solidcore-resources.com/ru/share-exchange/]
    ...
    state: Стороннее ПО_TRANSAQ Connector || sessionResultColor = "#CD4C2B"
        a: [Подключить счет к TRANSAQ Connector в личном кабинете|https://lk.finam.ru/ITS/AddTerminal]
            Доступ к системе бесплатный. 
            После того, как вы подпишите заявление на подключение терминала, вам придет СМС с паролем от системы. Логин находится в личном кабинете [по ссылке.|https://lk.finam.ru/Home/Account/Terminals?id=] Найдите в открывшемся списке идентификатор терминала TRANSAQ Connector.

    state: Стороннее ПО_QUIK
        a: [Подключить счет к QUIK в личном кабинете|https://lk.finam.ru/ITS/AddTerminal]
            Доступ к системе бесплатный. 
    ...
    state: Стороннее ПО_ComonTrade API
        a: Comon Trade Api — это REST API, предназначенное для организации взаимодействия пользовательских приложений с сервером TRANSAQ. [Детальнее ознакомится и получить токен|https://finamweb.github.io/trade-api-docs/]
    ...
    state: ИТС_FinamTrade_Работа с портфелем_История операций
        a: ✅ Историю операций в онлайн режиме можно посмотреть в [FinamTrade|https://trading.finam.ru/] (Android, IOS и [Web|https://trading.finam.ru/]), а также в личном кабинете во вкладке [«Брокерские операции».|https://lk.finam.ru/Journals/AccountOperationsJournal]
            ✅ Справку по счету с детальным описанием операций и сделок можно загрузить только за закрытый торговый день в [личном кабинете|https://lk.finam.ru/Home/Account/Info]
    ...
    state: ИТС_TRANSAQ_Авторизация
        a: Вы можете посмотреть логин в личном кабинете [по ссылке|https://lk.finam.ru/Home/Account/Terminals?id=]
            Нажмите на счет, к которому нужен логин в TRANSAQ. Вы увидите список подключенных ко счету платформ. Найдите в нем идентификатор TRANSAQ. Пароль к терминалу вы получали в виде СМС при открытии счета. Если сообщение утеряно, вы можете [восстановить пароль в личном кабинете|https://lk.finam.ru/ITS/ChangeTerminalPassword]
    ...
    state: ИТС_QUIK_Ключи
        a: Шаг 1. Скачайте и установите программу по ссылке: https://www.finam.ru/howtotrade/quik/
            Шаг 2. Чтобы открыть доступ к QUIK для уже имеющегося счета, зайдите в личный кабинет [по ссылке|https://lk.finam.ru/ITS/AddTerminal] и подключите терминал к желаемому счету.
            ...
            Шаг 5. Перейдите в раздел личного кабинета [«Регистрация ключей в Quik»|https://lk.finam.ru/cryptography/CreateQuikCertificates.aspx]
            ...
            [Видеоинструкция по установке QUIK|https://www.youtube.com/watch?v=A1dpP0fRToQ]
    ...
    state: ИТС_QUIK_Настройки_Подключение
        a: В QUIK можно одновременно подключить только счета одного типа, например, два счета типа «Единая денежная позиция».
            ...
            ✅ Создание нового терминала по счету доступно в [личном кабинете|https://lk.finam.ru/ITS/AddTerminal]
            ✅ [Подключение дополнительного счета к терминалу QUIK|https://lk.finam.ru/ITS/AddTerminalByAccount]
    ...
    state: ИТС_QUIK_Настройки_Поток сделок || sessionResultColor = "#CD4C2B"
        a: ✅ Чтобы подключить/отключить поток обезличенных сделок нужно:
            1. Оформить заявку в личном кабинете по [ссылке|https://lk.finam.ru/ITS/DepersonalizedTransactions]
    ...
    state: ИТС_Другие_QUIK X || sessionResultColor = "#CD4C2B"
        a: [Подключить WebQUIK к брокерскому счету в личном кабинете|https://lk.finam.ru/Items/ItsEnablePaidService]
            Торговая система платная — 420 ₽ в месяц. Пароль к терминалу вы получите в СМС при подключении к счету, логин — в [личном кабинете.|https://lk.finam.ru/Home/Account/Terminals?id=] Найдите в открывшемся списке идентификатор терминала QUIK.  
    ...
    state: ИТС_MetaTrader 5_Подключить счет
        a: [Подключить счет к терминалу в личном кабинете|https://lk.finam.ru/ITS/AddTerminal]
    ...
    state: ИТС_MetaTrader 5_Авторизация
        a: Логин [отображается в личном кабинете.|https://lk.finam.ru/Home/Account/Terminals?id=]  Идентификатор терминала является логином. Пароль вам придет в виде СМС после того, как вы [подпишете заявление на получение новой ИТС.|https://lk.finam.ru/ITS/AddTerminal]
            [Восстановить пароль в личном кабинете|https://lk.finam.ru/ITS/ChangeTerminalPassword]
    ...
    state: ИТС_MetaTrader 5_Таблица Портфель
        a: «Портфель» MetaTrader 5 – приложение-сервис разработки компании «Финам», визуально схожее с таблицей «Портфель» в терминале TRANSAQ.
            ✅ [Скачать «Портфель» и изучить инструкцию по установке|https://www.finam.ru/Files/htt/metatrader/portfolio/MQL5.zip]
    ...
    state: Заявки_Выставление
        a: ✅ Для выставления заявки через таблицу инструментов:
                ...
                ❗ Прежде чем подтвердить выставленную заявку, проверьте номер выбранного счета и наличие свободных средств на нем. Если у вас возникла ошибка в процессе выставления заявки с активного брокерского счета и в рабочее время биржи, обратитесь к менеджеру «Финам».
        a: ✅ Ознакомьтесь с особенностями выставления заявок на различных рынках:
        buttons:
    ...
    state: Заявки_Выставление_Фондовый
        a: ✅ С 01.01.2023 вступил в силу запрет Банка России на покупку инвесторами без статуса «квал» акций компаний из недружественных стран, доступно только закрытие позиций через отдел голосового трейдинга, системы QUIK и TRANSAQ.
            ...
            Дополнительно нужно:
            1. Подписать [«Согласие на торговые операции с иностранными бумагами с местом хранения недруж. инфраструктура»|https://lk.finam.ru/ForeignSecurities/UnfriendlyDepoConsent]
            2. Пройти тестирование [в личном кабинете «ИЦБ, требующие тестирования»|https://lk.finam.ru/Questionnaire/Questionnaires]
    ...
    state: Голосовой трейдинг || sessionResultColor = "#15952F"
        ...
        a: Связаться с отделом голосового трейдинга можно по одному из номеров:
            ...
            *1945 — доб. 2200 (Бесплатно по РФ для МТС, Билайн, МегаФон и Tele2)
            ✅ Выставление заявок доступно в рабочие дни бирж с 7:00 до 23:50 МСК
    ...
    state: Как закрыть позиции_Продажа БлокЦБ || sessionResultColor = "#B65A1E"
        a: «Финам» предоставляет сервис по продаже и покупке на Московской и СПБ Биржах* иностранных ценных бумаг, ранее заблокированных европейскими депозитариями Euroclear и Clearstream.
            *- торги на СПБ Бирже временно приостановлены.
            ...
            ❗ Недоступно для ИИС
            ❗ Предварительно перед совершением сделок нужно [подписать «Согласие на торговые операции с заблокированными ИЦБ»|https://lk.finam.ru/ForeignSecurities/BlockedSecuritiesConsent]
    ...
    state: Ошибки заявок || sessionResultColor = "#15952F"
        ...
        a: ✅ Популярные типы ошибок при выставлении заявок:
            ...
            Решение:
            — Тестирование для неквалифицированных инвесторов по ряду категорий можно пройти в [личном кабинете|https://lk.finam.ru/Questionnaire/Questionnaires]
            — Получить статус квалифицированного инвестора.
    ...
    state: Ошибки заявок_Другие_Запрет трейдера || sessionResultColor = "#CD4C2B"
        a: Открытие позиций в поставочных фьючерсах в последний день обращения (за день до экспирации и поставки) недоступно. Используйте следующие контракты.
    ...
    state: Некорректное отображение || sessionResultColor = "#15952F"
        ...
        a: Для устранения проблем, связанных с отображением информации, необходимо выполнить следующие действия:
            ...
            — [переустановить TRANSAQ|https://www.finam.ru/howtotrade/soft/transaq/]
    ...
    state: Актуальный портфель || sessionResultColor = "#15952F"
        ...
        a: На СПБ Бирже приостановлены торги иностранными ценными бумагами. Актуальная информация на [сайте биржи.|https://spbexchange.ru/ru/about/news2.aspx]
            ❗ После перевода СПБ Биржей бумаг на неторговый раздел, бумаги исключены из торговых лимитов биржи, и не отображаются в терминале, но их наличие отражено во вкладке «Портфель» в [личном кабинете|https://lk.finam.ru/].
        a: ✅ Актуальная стоимость портфеля с учетом текущих биржевых цен отображается во вкладке «Портфель» в [личном кабинете|https://lk.finam.ru/]
    ...
    state: Актуальный портфель_Баланс
        a: ✅ Наиболее распространенной причиной отсутствия счета в терминале является отсутствие подключения к выбранному идентификатору (логину).
            [Подключить счет к терминалу в личном кабинете|https://lk.finam.ru/ITS/AddTerminalAccount]
    ...
    state: Учебные видео || sessionResultColor = "#15952F"
        ...
        buttons:
            ...
            "MetaTrader 4" -> /Учебные видео_MetaTrader 4
    ...
    state: Учебные видео_MetaTrader 4
        a: Смотрите бесплатные обучающие курсы:
            ✅  [Торговый терминал MetaTrader 4|https://education.finam.ru/lk/course/torgovyi-terminal-metatrader4/]
            ✅  [Возможности торговли в MetaTrader 4|https://education.finam.ru/lk/course/vozmoznosti-torgovli-v-metatrader4/]
    ...
    state: Обучение на сайте_Учебный центр || sessionResultColor = "#CD4C2B" 
        ...
        buttons:
            ...
            "Детальнее у специалиста" -> /Перевод на оператора УЦ
    ...
    state: Обучение на сайте_Неквалы
        a: [Учебные материалы для подготовки к тестированию неквалифицированных инвесторов|https://www.finam.ru/landings/attestation-main] позволят успешно пройти тестирование и получить доступ к большему количеству финансовых инструментов. 
    ...
    state: Comon_Авторизация
        a: Данными для входа на сайт [comon.ru|https://www.comon.ru/] являются логин и пароль от [личного кабинета|https://lk.finam.ru/]
            ...
            2. нажмите на кнопку «Не удается войти?» и выберите подходящий вариант
            3. введите ФИО, паспортные данные и подтвердите восстановление
            4. на вашу электронную почту придет письмо с логином и ссылкой на создание нового пароля
    ...
    state: Comon_Подключить стратегию
        a: 1. Подключение:
            ...
            ✅ пройдите [инвестиционное профилирование|https://lk.finam.ru/Client/InvestProfileNatural]
    ...
    state: Услуги компании_Помощники_Аналитика
        a: Подпишитесь на бесплатную аналитику от «Финам» — и узнавайте в числе первых, как главные мировые события влияют на финансовые рынки. [Подробнее|https://www.finam.ru/landings/analytics/]
    ...
    state: Услуги компании_Помощники_AI-cкринер || sessionResultColor = "#CD4C2B"
        a: «AI-скринер» — это инструмент для инвесторов на основе искусственного интеллекта, обученный и протестированный на финансовых показателях компаний, макроэкономических данных, техническом анализе и других данных. Скринер дает годовой прогноз по российским и иностранным ценным бумагам.
            ✅ [Подробнее|https://www.finam.ru/landings/ai-screener/]
    ...
    # Улалили по просьбе КАЦ (кнопка удалена)
    ...
    state: Услуги компании_Готовые решения_ОФЗ
        a: [Подробности и подключение услуги|https://trading.finam.ru/investments] 
    ...
    state: Услуги компании_Партнерская программа
        a: Станьте партнером и зарабатывайте вместе с «Финам».
            ...
            ✅ [Подать заявку и узнать подробнее|https://trading.finam.ru/investments/rec]
    ...
    state: КУ_Лайт
        a: Пакет «Лайт» активируется при подключении тарифа «Консультационный». В рамках пакета услуг предоставляются инвестиционные идеи от профессиональных трейдеров.
            ...
            Подключить тариф можно в личном кабинете в [разделе «Детали» по счёту|https://lk.finam.ru/Home/Account/Details]
    ...
    state: Финам-бонус || sessionResultColor = "#15952F"
        ...
        a: Участвуйте в акции «Финам бонус»!
            ...
            ❗ Выплата вознаграждения для участников 2 этапа акции (с апреля по июнь 2024 года) будет перечислена на брокерский счет (не ИИС) в период с 5 по 15 июля 2024 года, если участники не предпочтут обменять бонусы на онлайн курс.
        buttons:
    ...
    state: Счет Иностранные биржи_пополнение_вывод
        a: 1. Пополнить брокерский счет «Иностранные биржи» можно только рублями РФ:
            ...
            – Онлайн в [личном кабинете|https://lk.finam.ru/Tiles/MoneyIn]
            – С действующих брокерских счетов, открытых в «Финам» доступен перевод средств только в рублях РФ
            2. Вывод средств доступен в рублях РФ или в долларах США в [личном кабинете|https://lk.finam.ru/Tiles/MoneyTransfer]
    ...
    state: Сегрегированный_Открытие
        a: Открытие счета «Сегрегированный Global» доступно только квалифицированным инвесторам, при личном посещении офиса или дистанционно. Помимо внутреннего паспорта РФ для открытия счета нужен второй документ из перечня на выбор:
            ...
            ✅ Дистанционное открытие счета «Сегрегированный Global» доступно для клиентов, у которых уже были ранее открыты счета в компании «Финам» при личном посещении офиса компании. Дистанционное открытие доступно в личном кабинете [lk.finam.ru|https://lk.finam.ru/] → «Открыть счет» → «Брокерская компания» → «Иностранные рынки» → «Сегрегированный Global». (Данный раздел личного кабинета доступен только квалифицированным инвесторам).
    ...
    state: Сегрегированный_Пополнение
        a: Пополнение счета «Сегрегированный Global» доступно в валютах рубли РФ и доллары США.
            ✅ Валюта счета «Сегрегированный Global» - доллар США. 
            ✅ Комиссии, сроки и реквизиты для пополнения счёта в рублях и долларах США в [иллюстрированной инструкции|https://www.finam.ru/dicwords/file/files_chatbot_rekvizityglobal]

    state: Сегрегированный_Инструменты
        a: На счете «Сегрегированный Global» предоставляется доступ к биржам NYSE, NASDAQ, СВОЕ, СME, в рамках которых доступна торговля Акциями, АДР, ETF, а также опционами, опционными стратегиями и фьючерсами.

    state: Сегрегированный_Доступ к ТС
        a: Торговля доступна через торговые системы TRANSAQ, TRANSAQ Америка (US) и FinamTrade. 
            ❗ Торговый терминал по счёту можно выбрать только один раз – при открытии счета. Если нужен другой торговый терминал, следует открыть другой счет «Сегрегированный Global» и при открытии нового счёта выбрать нужный терминал.
            ❗ Отличительные особенности торговых систем:
            ✅ TRANSAQ подойдет для торговли иностранными фьючерсами
            ✅ TRANSAQ Америка (US) подходит для торговли опционами
            Выберите, чтобы узнать подробнее об установке торговых систем: 
        buttons:
            "FinamTrade" -> /ИТС_FinamTrade_Авторизация
            "TRANSAQ/TRANSAQ US" -> /ИТС_TRANSAQ
    ...
    state: Сегрегированный_Налогообложение
        a: ✅ АО «Финам» является налоговым агентом на доходы, полученные при самостоятельной торговле в рамках счета «Сегрегированный Global», за исключением дивидендов, полученных в иностранной валюте до 2024 года, по ним отчитываться необходимо самостоятельно в ИФНС.
            ...
            Подать сведения об открытии счета в ФНС можно в мобильном приложении или через сайт [nalog.ru|https://www.nalog.gov.ru/].
    ...
    state: Сегрегированный_Вывод
        a: Поручение на вывод денежных средств со счета «Сегрегированный Global» можно подать через [личный кабинет Lime Trading|https://j2t.tech/ru/]
            ...
            ✅ зачисление долларов и евро на банковский счет – без комиссии
    ...
    state: Сегрегированный_ПереводЦБ
        a: К переводу на счёт «Сегрегированный Global» доступны ценные бумаги (ЦБ), приобретенные на иностранных биржах со счетов «Финам».
            ✅ Для успешного перевода ЦБ нужно обеспечить на счетах денежные средства для списания комиссии либо продать часть зачисленных бумаг. Комиссии: 
            — 1000 ₽ за эмитента — списание с брокерского счёта
            — 1% (минимум 500$) от стоимости переведенных активов — зачисление на Сегрегированный счёт
            ✅ При расчёте НДФЛ комиссии будут учтены как расходы и уменьшат налогооблагаемую базу по операциям с ЦБ
            ✅ Для перевода активов обратитесь к менеджеру
        buttons:
            "Пополнение счета Global" -> /Сегрегированный_Пополнение
            "Перевод на оператора" -> /Перевод на оператора
    ...
    state: Finam Invest || sessionResultColor = "#15952F"
        ...
        a: FinamInvest — это новое мобильное приложение от «Финам», включает в себя все необходимое для осознанных инвестиций, полагаясь на интересы и предпочтения самого пользователя:
    ...
    state: ИПИФ «Алгоритм роста» || sessionResultColor = "#15952F"
        ...
        a: ИПИФ «Алгоритм роста» позволяет воспользоваться всеми преимуществами высокочастотного трейдинга при относительно небольших вложениях, так как оборудование для такого вида инвестирования обычно стоит несколько миллионов рублей.
            ...
            1. [Открыть счет в управляющей компании (УК) «Финам»|https://lk.finam.ru/NewAccount/Product]
            2. Выбрать стратегию «Алгоритм роста»
            3. Пополнить новый счёт в УК от 100000 ₽
            ✅ [Подробности и инструкции|https://www.finam.ru/landings/asset-management-hft/]
    ...
    state: Пресса || sessionResultColor = "#15952F"
        ...
        a: ✅ Финансовая группа «Финам» рада сотрудничеству со СМИ и блогерами, освещающими деловые, экономические, общественно-политические и информационно-развлекательные темы. Пресс-служба и специалисты профильных подразделений,
            ...
            всегда готовы на регулярной основе предоставлять аналитические материалы, пресс-релизы и комментарии по широкому кругу экономических тем и вопросам, связанным с бизнесом финансовой группы. [Связаться с пресс-службой и узнать подробнее|https://www.finam.ru/landings/press]
            ✅ [Ознакомиться с опубликованными публикациями «Финам» в прессе|https://www.finam.ru/publications/section/press]
    ...
    state: Статус престиж
        intent!: /095 Статус престиж
        a: Статус «Престиж» предоставляется для клиентов с активами от 1 млн. руб. и даёт набор эксклюзивных привилегий:
            ✅ Персональный менеджер
            ✅ Выделенная линия поддержки 24/7 с приоритетным обслуживанием в чате и по телефону
            ✅ Индивидуальный подбор тарифа
            ✅ Расширенная аналитическая поддержка
            ✅ Индивидуальные инвестиционные решения
            ✅ Бесплатный доступ к платным обучающим мероприятиям 
            Для получения статуса «Престиж» необходимо, чтобы стоимость ваших активов составляла не менее 1 млн. руб.
            Пополните ваш счет на необходимую сумму без комиссии [через СБП|https://lk.finam.ru/money/topup/sbp]
    ...
    state: Скриннер облигаций
        intent!: /096 Скриннер облигаций
        # q!: * выпуклость облигации *
        script:
            $session.isinArray = {}
            $session.isin = {}
            $session.bondData = {}
        InputText:
            prompt = Введите название эмитента, например "Сбербанк"
            varName = bondName
            then = /Скриннер облигаций_дата погашения
        

    state: Скриннер облигаций_дата погашения
        InputText:
            prompt = Введите дату погашения облигации. Если не знаете точную дату, введите месяц и год погашения, к примеру "декабрь 2024":
            varName = matDate
            then = /Скриннер облигаций_Результат
            
            
    state: Скриннер облигаций_Результат
        scriptEs6:
            const data = bondsIsin()
            const date = getIsin.dateConverter($session.matDate)
            $session.isinArray = getIsin.getIsinName(data, $session.bondName, date)
            if ($session.isinArray.length >= 2){
                $reactions.answer("C такой датой погашения по облигациям {{$session.isinArray[0].EMITENTNAME}} нашлось несколько выпусков и дат погашения. Выберите облигацию подходящую под искомые параметры:");
                var massiveButtons = []
                for (var i = 0; i < $session.isinArray.length; i++){
                massiveButtons.push({text: $session.isinArray[i].NAME + " " + $session.isinArray[i].MATDATE, transition:"/Скриннер облигаций_Запрос данных облигации"})
                } 
                $reactions.buttons(massiveButtons)
                } else if ($session.isinArray.length == 1) { 
                $reactions.transition("/Скриннер облигаций_Запрос данных облигации"); 
                } else {
                $reactions.transition("/Скриннер облигаций_Запрос по ISIN")
                }
            
        
    state: Скриннер облигаций_Запрос по ISIN
        InputText:
            prompt = С таким названием эмитента и датой облигаций не найдено. Пожалуйста введите ISIN инструмента, посмотреть можно тут https://ai.finam.ru/
            varName = isinDirect
            then = /Скриннер облигаций_Запрос данных облигации
        
    state: Скриннер облигаций_Запрос данных облигации
        script:
            $session.isin = _.find($session.isinArray, function (item){
                var bondRequest = $request.query.slice(0,-11)
                return item.NAME === bondRequest
                })
            
            
            $session.bondData = getBondsData($session.isin && $session.isin.ISIN 
            ? "https://diagnostics.finam.ru/stocksadviser-tr1/api/bond?isin=" + $session.isin.ISIN 
            : ($session.isinArray && $session.isinArray[0] && $session.isinArray[0].ISIN 
            ? "https://diagnostics.finam.ru/stocksadviser-tr1/api/bond?isin=" + $session.isinArray[0].ISIN
            : "https://diagnostics.finam.ru/stocksadviser-tr1/api/bond?isin=" + $session.isinDirect)
            )
            
            
            $session.ratingData = getBondsData($session.isin && $session.isin.ISIN 
            ? "https://diagnostics.finam.ru/stocksadviser-tr1/api/bond/rating?isin=" + $session.isin.ISIN 
            : ($session.isinArray && $session.isinArray[0] && $session.isinArray[0].ISIN
            ? "https://diagnostics.finam.ru/stocksadviser-tr1/api/bond/rating?isin=" + $session.isinArray[0].ISIN
            : "https://diagnostics.finam.ru/stocksadviser-tr1/api/bond/rating?isin=" + $session.isinDirect)
            )
            
            

            if ($session.bondData.length === 0){
                $reactions.answer("К сожалению у нас не получилось найти облигацию подходящую под ваши параметры. Попробуйте ввести другое название, дату погашения или ISIN.")
                $reactions.transition("/Скриннер облигаций")
                } else {
                $reactions.transition("/Скриннер облигаций_Кнопки");
                }
                        
            
    state:  Скриннер облигаций_Кнопки
        a: Выберите что вас интересует
        buttons:
            "Доходность" -> /Скриннер облигаций_Ответ
            "Цена" -> /Скриннер облигаций_Ответ
            "Модифицированная дюрация" -> /Скриннер облигаций_Ответ
            "Номинал" -> /Скриннер облигаций_Ответ
            "Частота выплат в год" -> /Скриннер облигаций_Ответ
            "Купон" -> /Скриннер облигаций_Ответ
            "Ставка купона" -> /Скриннер облигаций_Ответ
            "Валюта" -> /Скриннер облигаций_Ответ
            "Дата начала торгов" -> /Скриннер облигаций_Ответ
            "Объем выпуска" -> /Скриннер облигаций_Ответ
            "Дата погашения" -> /Скриннер облигаций_Ответ
            "Следующая дата выплаты купона" -> /Скриннер облигаций_Ответ
            "НКД" -> /Скриннер облигаций_Ответ
            "Дюрация Маколея" -> /Скриннер облигаций_Ответ
            "Выпуклость облигации" -> /Скриннер облигаций_Ответ
            "Рейтинг" -> /Скриннер облигаций_Ответ
            "Выбрать другую облигацию" -> /Скриннер облигаций

    state: Скриннер облигаций_Ответ
        script:
            if($session.bondData == null){
                $reactions.transition("/Start");
                } else {
                $reactions.answer(screenerAnswers($request.query));
                $reactions.buttons({text:"Назад",transition:"{{$session.contextPath}}"});
                }
    ...
    state: GPT_FAQ
        ...
        script:
            anonymizer();
            $temp.gptAnswer = copilot.requestAnswerGpt();
            $analytics.setSessionData("Текст обращения", $request.query);
        a: {{$temp.gptAnswer}}  
        
    state: Сервис конвертации валют
        intent!: /098 Сервис конвертации валют
        a: ✅ Клиентам «Финам» доступна покупка долларов США через сервис внебиржевых торгов.
            ✅ Продажа долларов США:
            — доступна для клиентов, на чьих счетах ИИС учитываются доллары США
            — также продать доллары США можно в банке «Финам». Зачисление долларов США на банковские счета Банка «Финам» – без комиссии. Нажмите кнопку ниже, чтобы узнать подробнее.
            ✅ Для подачи внебиржевой заявки обратитесь к менеджеру поддержки в чате или по телефону.
            ✅ При наличии встречного поручения менеджер поддержки позвонит и проинформирует, в каком объеме и по какой цене возможна сделка. Если параметры подходят, звонок будет переадресован в отдел голосового трейдинга.
            ❗ Клиенту нужно сообщить трейдеру свой торговый код и проговорить параметры выставляемой заявки. Без подтверждения заявки отделу голосового трейдинга операция осуществлена не будет. 
            ✅ Узнать свой торговый код можно в личном кабинете в [разделе «Детали» счёта.|https://lk.finam.ru/Home/Account/Details]
            ✅ Расчёты по конвертации проходят в 19.00 МСК.
            ❗ Комиссии за услуги голосового трейдинга: до 5 поручений в день — без комиссии, за последующие — комиссия 236 ₽ за поручение. 
        buttons:
            "Конвертация валюты в банке «Финам»" -> /Банк_Конвертация валюты
            "Открыть счет в банке «Финам»" -> /Банк_Банковский счет
            "Вывод валюты с брокерского счета" -> /Движение ДС_Вывод_Реквизиты
            "Перевод на оператора" -> /Перевод на оператора
