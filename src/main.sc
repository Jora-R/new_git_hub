...
theme: /
    ...
    state: IVR меню бота
        ...
        script:
            ...
            if (($session.specificPhoneNumber != undefined) && ($session.specificPhoneNumber == "short")){
                //$reactions.answer("добавочный сотрудника");
                $reactions.transition("/IVR меню бота/Добавочный сотрудника");
            } else if (($session.specificPhoneNumber != undefined) && ($session.specificPhoneNumber == "webcall")){
                //$reactions.answer("звонок с сайта");
                $reactions.transition("/IVR меню бота/Звонок с сайта");
            } else if ($client.profileCRM == undefined){
            ...
            } else if (($client.profileCRM.Result.ClientStatus == "От контролера") || ($client.profileCRM.Result.ClientStatus == "Входящий по инициативе клиента")){
                //$reactions.answer("статус есть и = от контролера или входящий");
                $reactions.transition("/IVR меню бота/Контролер_Входящий");
            } else if ($client.profileCRM.Result.ClientStatus == "Новый клиент"){
                //$reactions.answer("статус есть и = новый клиент");
                $reactions.transition("/IVR меню бота/Новый клиент");
            } else{
                //$reactions.answer("прочие статусы");
                $reactions.transition("/IVR меню бота/Прочие статусы");
            }
        
        state: Добавочный сотрудника
            a: Вас приветствует голосовой помощник, группы компаний фина'м.
            script:
                $analytics.setMessageLabel("Добавочный сотрудника", "IVR VB");
                $session.needBot =  'true';
                $reactions.transition("/Ввод добавочного");
        
        state:Звонок с сайта
            a: Вас приветствует голосовой помощник, группы компаний фина'м.
            script:
                $analytics.setMessageLabel("Звонок с сайта", "IVR VB");
                $session.needBot =  'true';
                $reactions.transition("/Ввод добавочного");
        ...
        state: Контролер_Входящий
            if: (($client.getGlobalID.firstName != "") && ($client.getGlobalID.middleName != ""))
                a: {{$client.getGlobalID.firstName}} {{$client.getGlobalID.middleName}}
            a: Вас приветствует голосовой помощник, группы компаний фина'м.
            script:
                $analytics.setMessageLabel("Контролер_Входящий", "IVR VB");
                $session.needBot =  'true';
                $reactions.transition("/Ввод добавочного");
        ...
        state: Новый клиент
            ...
            a: Вас приветствует голосовой помощник, группы компаний фина'м.
            script:
                $analytics.setMessageLabel("Новый", "IVR VB");
                $session.needBot =  'false';
                $reactions.transition("/Ввод добавочного");
        
        state: Прочие статусы
            if: (($client.getGlobalID.firstName != "") && ($client.getGlobalID.middleName != ""))
                a: {{$client.getGlobalID.firstName}} {{$client.getGlobalID.middleName}}
            a: Вас приветствует голосовой помощник, группы компаний фина'м.
            script:
                $analytics.setMessageLabel("Прочие статусы", "IVR VB");
                $session.needBot =  'false';
                $reactions.transition("/Ввод добавочного");
    ...
    state: Ввод добавочного
        ...
        state: Digits
            ...
            a: Представленная в телефонном разговоре информация не является индивидуальной инвестиционной рекомендацией.
            # a: Обращаем ваше внимание, Торги на фондовом рынке Московской биржи приостановлены с 16 часов 18 минут по московскому времени. Новость размещена на сайте биржи. О времени возобновления торгов будет сообщено дополнительно. 
            script:
                ...
                } else if(($request.query == "1806") || ($request.query == "2291") || ($request.query == "1342") || ($request.query == "5561")){
                    $reactions.answer("Мне не удалось найти указанный номер");
                    $analytics.setMessageLabel("Добавочный ЧС", "IVR VB");
                    $session.operatorPhoneNumber = '1000';
                    $reactions.transition("/Оператор/Оператор по номеру");
                } else {
    ...
    state: Открытие_закрытие_счета
        ...
        state: open_Банк
            ...
            a: Дистанционно подать заявку на открытие счета, можно в личном кабинете на сайте фина'м точка ру, кнопка открыть новый счет, находится над списком ваших открытых счетов. Далее выберите раздел, банковские продукты, далее выберите, карты, или тип счета.
            ...
            a: Получить банковскую карту можно в офисе компании, или курьерской доставкой, в зависимости от города получения. Доступный способ получения отображается при заказе банковской карты при выборе города. По факту изготовления и доставки карты в выбранный офис компании, вам поступит смс.
        ...
        state: open_УК
            a: Открыть счет доверительного управления в управляющей компании Фина'м, можно дистанционно в личном кабинете брокера, на сайте фина'м точка ру. Авторизуйтесь в личном кабинете.
            a: Кнопка открыть новый счёт, находится над списком ваших открытых счетов. Далее выберите раздел Доверительное управление, и следуйте инструкциям сайта.
        ...
        state: open_Форекс
            ...
            a: Если у вас есть брокерский счет в компании Фина'м, вы можете дистанционно открыть счет форекс в личном кабинете брокера, на сайте Фина''м точка ру.
        ...
        state: close_Брокер
            a: Закрыть брокерский счет можно дистанционно в личном кабинете на сайте фина'м точка ру, для этого, в разделе, Услуги, выберите меню, Прочие операции.
        ...
        state: close_УК
            a: Закрыть счет доверительного управления в Управляющей компании можно дистанционно в личном кабинете фина'м точка ру.
            ...
            a: В случае, если у вас в договоре заранее была указана дата расторжения, не забудьте предоставить реквизиты для перечисления средств в личном кабинете на сайте фина'м точка ру, в разделе, Информация.
            a: Хотите получить консультацию у оператора?
            # script: 
            #     $context.session = {};
            q: @agree ||toState = "/Оператор/Оператор по номеру"
    ...
    state: Заказ_документов
        ...
        state: Заказ_справка_Брокер
            a: Заказать справку по брокерскому счету, можно в личном кабинете на сайте, фина'м точка ру; для этого на главной странице личного кабинета выберите необходимый счет, далее выберите вкладку, Справка.
            ...
            a: В разделе, Отчетность - брокерские отчеты, автоматически выгружаются отчеты брокера на подпись.
            ...
            a: Для заказа заверенного брокерского отчета обратитесь к менеджеру поддержки.
        ...
        state: Заказ_выписка_депо_Брокер
            a: Заказать выписки из депозитария, можно в личном кабинете на сайте, фина'м точка ру. Авторизуйтесь в личном кабинете. Выберите раздел Услуги, далее выберите раздел налоги выписки справки. В поле меню Депозитарий, выберите, Справки и выписки из депозитария.
            ...
            script:
                 if (identificationAO($client.profileCRM) == "success"){
                        ...
                        $session.insrtuctionText = "В продолжение разговора направляю вам [ссылку для заказа Выписки со счета ДЕПО|https://lk.finam.ru/Orders/DepoReport]";
        ...
        state: Заказ_налог_справка_Брокер
            a: Заказать справку о доходах 2НДФЛ, и справку об убытках, можно за отчетный период, то есть один календарный год, в личном кабинете на сайте, фина'м точка ру. Авторизуйтесь в личном кабинете. Выберите раздел Услуги, далее выберите раздел налоги выписки справки. В поле меню Налоги, выберите, Заказ справки 2-НДФЛ.
        ...
        state: Заказ_справка_актив_Брокер
            a: Заказать справку об активах можно в личном кабинете на сайте, фина'м точка ру. Авторизуйтесь в личном кабинете. Выберите раздел Услуги, далее выберите раздел, налоги выписки справки. В поле меню, Другое, выберите Справка об активах. Изготовление справки занимает до двух рабочих дней.
            script:
                 if (identificationAO($client.profileCRM) == "success"){
                        $context.session.lastState = $context.currentState;
                        $session.questionText = "Хотите получить в чат ссылку для заказа в личном кабинете Справки об активах?";
                        $session.insrtuctionText = "В продолжение разговора направляю вам [ссылку для заказа Справки об активах|https://lk.finam.ru/orders/AssetStatement]";
                        $reactions.transition("/Отправка инструкции в чат");
                    }
            a: Чем я могу еще помочь?
            script: 
        ...
        state: Заказ_документы_откр_Брокер
            a: Заказать документы об открытии брокерского счета, можно в личном кабинете на сайте, фина'м точка ру, для этого выберите меню отчетность, далее выберите раздел, основные документы. 
        ...
        state: Заказ_справка_госслуж_Брокер
            a: Заказать справку для гос служащего по форме 57 98 уу, можно в личном кабинете на сайте, фина'м точка ру. Авторизуйтесь в личном кабинете. Выберите раздел Услуги, далее выберите раздел, налоги выписки справки. В поле меню, Другое, выберите Справка для гос служащих.
            ...
            state: Подробнее_госслуж
                ...
                a: Для получения сведений об остатках средств на брокерских счетах, можно заказать справку по счету в личном кабинете. 
        ...
        state: Заказ_1042s_Брокер
            ...
            a: Готовые формы загружаются автоматически в личный кабинет на сайте, фина'м точка ру. Авторизуйтесь в личном кабинете. Выберите раздел Услуги, далее выберите раздел налоги выписки справки. В поле меню Налоги, выберите, форма 10 42 ЭС. Справка формируется за отчетный период, то есть за один календарный год. 
            script:
             if (identificationAO($client.profileCRM) == "success"){
                    $context.session.lastState = $context.currentState;
                    $session.questionText = "Хотите получить в чат ссылку для скачивания в личном кабинете Формы 10 42 эс?";
                    $session.insrtuctionText = "В продолжение разговора направляю вам [ссылку для заказа Формы 1042S|https://lk.finam.ru/orders/Form1042S]";
                    $reactions.transition("/Отправка инструкции в чат");
                }
            a: Чем я могу еще помочь?
            script:
        ...
        state: Заказ_w8_Брокер
            a: В связи с приостановкой соглашения, об избежании двойного налогообложения между США и эРэФ, СПБ биржа и национальный расчетный депозитарий приостановили обработку формы дабл ю 8 бэн; ранее подписанные формы дабл ю 8 бэн также - недействительны.
            a: Чем я могу еще помочь?
            script: 
                $context.session = {};
            q: @repeat_please * ||toState = "."
        ...
        state: Заказ_выписка_квал_Брокер
            ...
            a: в личном кабинете на сайте, Фина'м точка ру. для этого выберите меню, Услуги, далее выберите раздел, Налоги, выписки, справки, в поле меню другОе, выберите, Выписка из реестра квалифицированных лиц.
            script:
             if (identificationAO($client.profileCRM) == "success"){
                    $context.session.lastState = $context.currentState;
                    ...
                    $session.insrtuctionText = "В продолжение разговора направляю вам [ссылку для заказа Выписки из реестра квалифицированных лиц|https://lk.finam.ru/orders/QualifiedInvestorRequestStatementStatus]";
                    $reactions.transition("/Отправка инструкции в чат");
                }
            a: Чем я могу еще помочь?
            script: 
        ...
        state: Заказ_справка_Банк
            ...
            a: Для этого выберите нужный счет, в поле справа выберите, выписка по счету. Если у вас есть также брокерский счет, то заказать выписку можно и в личном кабинете брокера на сайте, Фина'м точка ру.
        ...
        state: Заказ_справка_госслуж_Банк
            a: Заказать справку для гос служащего, можно в личном кабинете брокера на сайте, фина'м точка ру. Авторизуйтесь в личном кабинете. Выберите раздел Услуги, далее выберите раздел, налоги выписки справки. В поле меню, Другое, выберите Справка для гос служащих.
        ...
        state: Заказ_документы_откр_Форекс
            a: Заказать документы об открытии форекс счёта, можно в личном кабинете брокера фина'м на сайте, фина'м точка ру, для этого выберите меню отчетность, далее выберите раздел, основные документы.
    ...
    state: MoneyTransfer || modal = true
        ...
        state: MoneyTransferGetMethod
            ...
            if: $session.moneyTransfer.type === 'Вывод'
                ...
                a: Обратите внимание, вывод валюты возможен только по реквизитам. 
        ...
        state: MoneyTransfer Инструкция пополнение Реквизиты
            script:
                if (identificationAO($client.profileCRM) == "success"){
                    ...
                    $session.insrtuctionText = "В продолжение разговора направляю вам ссылку на [пополнение счёта по реквизитам в личном кабинете|https://lk.finam.ru/money/topup/requisites?]";
                    $reactions.transition("/Отправка инструкции в чат");
                }
        ...
        state: MoneyTransfer Инструкция вывод Реквизиты
            script:
                if (identificationAO($client.profileCRM) == "success"){
                    ...
                    $session.insrtuctionText = "В продолжение разговора направляю вам ссылку на [вывод средств по реквизитам в личном кабинете|https://lk.finam.ru/orders/MoneyOut/MoneyOut/Default.aspx]";
                    $reactions.transition("/Отправка инструкции в чат");
                }
            a: Хотите, я расскажу подробнее про комиссии?
            script: 
        ...
        state: MoneyTransfer Комиссии вывод Реквизиты
            a: Комиссии за вывод денежных средств с брокерского счета физическими лицами удерживаются в рублях по курсу центробанка на дату исполнения вывода. Вывод рублей РФ – бесплатно, срочный вывод рублей РФ – 300 рублей. Вывод китайских юаней - 5% от суммы вывода. Вывод и зачисление валют доллар США, евро и тенге, на счета банка фина'м - без комиссии.
            a: За хранение долларов и евро в банке фина'м от 3000 единиц валюты, комиссия – 0,03% в день. За вывод по реквизитам в сторонние банки казахстанских тенге, комиссия — 0,07% от суммы вывода, но не менее 25 и не более 100 €.
            a: За вывод долларов и евро на счета в других банках российской федерации, взимаются комиссии, За вывод — 0,3% от суммы вывода, но не менее 30 и не более 150 долларов; а также дополнительная комиссия за обработку поручений на вывод - 0,4% от суммы вывода, но не менее 1500 и не более 250000 рублей. 
            a: Чем я могу еще помочь?
            script: 
                $context.session = {};
            q: @repeat_please * ||toState = "."
        ...
        state: MoneyTransfer Инструкция пополнение Карта
            script:
                if (identificationAO($client.profileCRM) == "success"){
                    ...
                    $session.insrtuctionText = "В продолжение разговора направляю вам ссылку на [пополнение счёта по карте в личном кабинете|https://lk.finam.ru/money/topup/bankcardtopup]";
                    $reactions.transition("/Отправка инструкции в чат");
                }
            a: Хотите, я расскажу подробнее про комиссии?
            script: 
        ...
        state: MoneyTransfer Инструкция вывод Карта
            script:
                if (identificationAO($client.profileCRM) == "success"){
                    ...
                    $session.insrtuctionText = "В продолжение разговора направляю вам ссылку на [вывод средств по реквизитам карты в личном кабинете|https://lk.finam.ru/money/moneytransfer/MoneyOutByRequisites]";
                    $reactions.transition("/Отправка инструкции в чат");
                }
            a: Хотите, я расскажу подробнее про комиссии?
            script: 
        ...
        state: MoneyTransfer Комиссии вывод Карта
            a: Комиссии за вывод денежных средств с брокерского счета физическими лицами удерживаются в рублях по курсу центробанка на дату исполнения вывода. Вывод рублей РФ – бесплатно, срочный вывод рублей РФ – 300 рублей. Вывод китайских юаней - 5% от суммы вывода.
            a: Вывод и зачисление валют доллар США, евро и тенге, на счета банка фина'м - без комиссии. За хранение долларов и евро в банке фина'м от 3000 единиц валюты, комиссия – 0,03% в день. За вывод по реквизитам в сторонние банки казахстанских тенге, комиссия — 0,07% от суммы вывода, но не менее 25 и не более 100 €.
            a: За вывод долларов и евро на счета в других банках российской федерации, взимаются комиссии, За вывод — 0,3% от суммы вывода, но не менее 30 и не более 150 долларов; а также дополнительная комиссия за обработку поручений на вывод - 0,4% от суммы вывода, но не менее 1500 и не более 250000 рублей. 
            a: Чем я могу еще помочь?
            script: 
                $context.session = {};
    ...
    state: Установка_ИТС
        ...
        state: Установка_ИТС_Quik
            ...
            a: Если при открытии брокерского счета, вы не подключали терминал квик к счету, то вы можете подключить КВИК в личном кабинете, на сайте фина'м точка ру, в разделе, 
        ...
        state: Установка_ИТС_Transaq
            ...
            a: А также, вы можете открыть доступ к системе ТРАНЗАК, для уже имеющегося счета, зайдите в личный кабинет на сайте фина'м точка ру.
        ...
        state: Установка_ИТС_TrConnector
            ...
            a: Зайдите в личный кабинет на сайте фина'м точка ру. в разделе, торговля, выберите пункт меню, информационно торговые системы, И Т эС, и подключите желаемый счет. 
            ...
            a: Логин находится в личном кабинете на сайте фина'м точка ру, выберите нужный счет, далее разверните раздел, торговые программы.
            a: Чем я могу еще помочь?
            script: 
                $context.session = {};
            q: @repeat_please * ||toState = "."
        ...
        state: Установка_ИТС_MT5
            ...
            a: Также, подключить брокерский счет к терминалу Мета Трейдер 5, можно в личном кабинете на сайте фина'м точка ру.
            a: В разделе, Торговля, выберите пункт меню, информационно торговые системы, И Т эС, и подключите терминал к желаемому счету. 
    ...
    state: NLK
        ...
        state: ЗаменаДанных_passport
            ...
            a: Для этого авторизуйтесь в личном кабинете; затем, в верхнем правом углу страницы нажмите на иконку профиля, далее нажмите на ваше ФИО.
        ...
        state: ЗаменаДанных_fullName
            ...
            a: Копии должны хорошо читаться, не иметь бликов, посторонних надписей, или рисунков.
            a: Чем я могу еще помочь?
            q: @repeat_please * ||toState = "."
            q: @disagree ||toState = "/Могу еще чем то помочь?/NO"
            # final answer
    ...
    state: Авторизация_Логин - Пароль
        ...
        state: Уточнение системы
            ...
            state: LocalCatchAll
                event: noMatch
                a: Я не понял вас. Уточните наименование системы или сайта где нужна авторизация?
        ...
        state: QuikX
            a: Подключить торговую систему веб квик к брокерскому счету, можно в личном кабинете на сайте фина'м точка ру. Авторизуйтесь в личном кабинете.
        ...
        state: Transaq
            ...
            a: Посмотреть логин можно в личном кабинете на сайте, фина'м точка ру. Нажмите на счет, к которому нужен логин от транзак. Вы увидите список подключенных ко счету платформ. Найдите в нем идентификатор транзак.
            a: Чем я могу еще помочь?
            script: 
                $context.session = {};
            q: @repeat_please * ||toState = "."
        ...
        state: TransaqConnector
            a: Подключить счет к транзак коннектор можно в личном кабинете, на сайте фина'м точка ру, в разделе Торговля – Информационно торговые системы – Подключение счёта на терминал.
        ...
        state: MT5
            ...
            a: Если сообщение утеряно, вы можете восстановить пароль в личном кабинете, в разделе Торговля – Информационно торговые системы – Смена пароля на терминал.
            a: Логин отображается в личном кабинете на сайте, фин''ам точка ру. Авторизуйтесь в личном кабинете.
        ...
        state: FinamBonus
            a: Чтобы перейти в личный кабинет участника  акции Фина'м бонус, зайдите на сайт фина'м точка ру. В верхней части страницы сайта выберите раздел Программа лояльности; далее выберите вкладку Мои бонусы.
    ...
    state: Тарифы 
        ...
        state: Тариф_Долгосрочный портфель
            a: Абонентская плата в месяц, по тарифу, Долгосрочный портфель, 0 рублей. Ознакомиться с описанием тарифного плана можно на сайте Фина'м точка ру. Для этого в верхней части страницы сайта выберите раздел Инвестиции, далее выберите раздел Тарифы.
            a: Также, полные условия тарифных планов можно изучить в Приложении номер 7, к Регламенту брокерского обслуживания Фина'м. Регламент представлен на сайте фина'м точка ру.
            a: Чтобы открыть регламент, в верхней части страницы сайта выберите раздел Инвестиции, далее выберите раздел Информация, документы и регламенты.
            a: Брокерская комиссия за сделки зависит от выбранного рынка и тарифного плана.
            a: Чем я могу еще помочь?
        ...
        state: Тариф_ФриТрейд
            a: С 3 июля, подключение тарифа Фри Трэйд - недоступно. По счетам, с уже подключенным ранее тарифом Фри Трэйд, пройдет смена тарифа на тарифный план Стратег по истечении 30 дней со дня подключения.
            a: Чем я могу еще помочь?
            script: 
                $context.session = {};
            q: @repeat_please * ||toState = "."
    ...
    state: Налоговые вычеты
        ...
        state: Брокер_IISDeduction_TypeA
            ...
            a: Авторизуйтесь в личном кабинете. В разделе Услуги выберите, Налоги выписки справки, далее в поле меню Инвестиционный налоговый вычет выберите нужное.
        ...
        state: Брокер_IISDeduction_TypeB  
            ...
            a: Авторизуйтесь в личном кабинете. В разделе Услуги выберите, Налоги выписки справки, далее в поле меню Инвестиционный налоговый вычет выберите нужное. После подачи заявления в течение двух рабочих дней, ожидайте новый статус заявления, Принято к исполнению. 
        ...
        state: Брокер_3YearsBenefit
            ...
            a: Проверить наличие бумаг, попадающих под трех годичную льготу на счетах в Фина'м, а также подать заявление на ее предоставление, можно в личном кабинете брокера на сайте, Фина'м точка ру. Авторизуйтесь в личном кабинете.
    ...
    state: ИИС_Дата открытия
        ...
        state: Ответ_Брокер
            a: Проверить дату открытия договора ИИС; посмотреть актуальный тариф и перечень действующих счетов, можно в личном кабинете на сайте, фина'м точка ру. Авторизуйтесь в личном кабинете, и выберите нужный счет с названием КэЛэ эФ ИИС, далее выберите вкладку, детали. История пополнения договора ИИС, во вкладке Операции, а также во вкладке Справка.
    ...
    state: ИИС_Пополнение
        ...
        state: Ответ_Брокер
            ...
            a: Рекомендуем пополнять счет ИИС наличными в кассе, безналичным платежом по реквизитам или через систему быстрых платежей. При оформлении вычета, налоговая имеет право запросить платежное поручение с подтверждением внесения средств на ИИС.
            a: Пополнить счёт ИИС по реквизитам, можно в личном кабинете на сайте, фина'м точка ру. Авторизуйтесь в личном кабинете. Нажмите кнопку Пополнить, под разделом, Все' средства. Далее выберите, Пополнение по реквизитам.
            a: При пополнении счёта по реквизитам, деньги поступят в течение дня. Срок может быть увеличен до трех рабочих дней, в зависимости от исполнения платежа банком-отправителем. За данную операцию, фина'м не взимает комиссию. Однако возможна комиссия со стороны банка-отправителя.
            a: Через кассу представительства фина'м, можно пополнить ИИС наличными средствами, без комиссии. Для этого вам понадобится действующий паспорт гражданина Российской Федерации.
    ...
    state: ИИС_Статус вычета
        ...
        state: Ответ_Брокер
            ...
            a: Авторизуйтесь в личном кабинете. В разделе Услуги выберите, Налоги выписки справки, далее в поле меню Инвестиционный налоговый вычет выберите Упрощенный порядок получения вычета.
            ...
            a: Статус отправки сведений в ФНС, по упрощенному порядку для получения вычета типа Б, можно отслеживать в разделе личного кабинета отчетность, в история операций.
    ...
    state: ИИС 3
        ...
        a: Трансформация ИИС доступна в личном кабинете на сайте фина'м точка ру. В разделе личного кабинета Услуги; Прочие операции. Договоры ИИС, открытые в период с 2024ого по 2026й год, нельзя закрыть раньше, чем через 5 лет.
    ...
    state: Маржинальная торговля_открытие позиции
        ...
        state: Ответ_Брокер
            a: Для открытия маржинальной позиции нужно подключить возможность маржинальной торговли по счету. Чтобы подключить маржинальную торговлю авторизуйтесь в личном кабинете на сайте, фина'м точка ру.
    ...
    state: Маржинальная торговля_подключение|отключение
        ...
        state: Ответ_Брокер
            ...
            a: Авторизуйтесь в личном кабинете. В разделе Торговля выберите, Тестирование для неквалифицированного инвестора. 
            a: Для квалифицированных инвесторов доступ предоставляется автоматически. Чтобы отключить маржинальную торговлю обратитесь к менеджеру фина'м.
    ...
    state: Срочный рынок  
        ...
        q: * @which_GO_u * ||toState = "/Срочный рынок_гарантийное обеспечение по счету"
        q: * @what_profit_loss_u * ||toState = "/Срочный рынок_прибыль|убыток по счету"
        q: * @lower_GO_u * ||toState = "/Срочный рынок_уменьшение гарантийного обеспечения"
        q: * @futures_trading_u * ||toState = "/Срочный рынок_покупка|продажа фьючерса"
    ...
    state: Срочный рынок_гарантийное обеспечение по счету
        ...
        state: Ответ_Брокер
            a: При открытии позиции по фьючерсу, на счете блокируется гарантийное обеспечение, или ГэО'. При закрытии позиции, заблокированные сре'дства освобождаются.
    ...
    state: Срочный рынок_уменьшение гарантийного обеспечения
        ...
        state: Ответ_Открытие моносчета
            ...
            a: Далее нажать кнопку Открыть новый счет; в поле меню Брокерская компания, выбрать Договор с отдельными брокерскими счетами.
            a: Чем я могу еще помочь?
            script: 
                $context.session = {};
            q: @repeat_please * ||toState = "."
        ...
        state: Ответ_Отключение ФС и ВС
            ...
            a: Для смены инвест профиля, авторизуйтесь в личном кабинете на сайте фина'м точка ру; в правом верхнем углу нажмите на значок персоны; и перейдите в раздел Инвестиционный статус; далее выберите вкладку Инвестиционный профиль.
        ...
        state: Ответ_Подключение ПГО
            ...
            a: Узнать условия подключения, подключить или отключить услугу пониженного гарантийного обеспечения, можно в личном кабинете на сайте фина'м точка ру. Авторизуйтесь в личном кабинете.
    ...
    state: Срочный рынок_покупка|продажа фьючерса
        ...
        state: Ответ_Брокер
            ...
            a: Пройти тестирование можно в личном кабинете на сайте фина'м точка ру. Авторизуйтесь в личном кабинете. В разделе Торговля выберите, Тестирование для неквалифицированного инвестора. 
    ...
    state: Валюта
        ...
        a: Обращаем ваше внимание, В связи с введением ограничительных мер в отношении Московской биржи, с 13 июня приостановлено проведение биржевых торгов валютами доллар США, евро, и гонконгский доллар.
        a: Какая информация по валютным операциям Вас интересует? Подать заявку на конвертацию долларов на внеби'рже? Купить или продать валюту в торговом терминале. Как купить или продать неполный лот валюты. Комиссии за хранение валют.
        ...
        q: * @vnebirzh_currency_u * ||toState = "/Валюта_внебиржевая торговля"
        q: * @choice_1 * ||toState = "/Валюта_внебиржевая торговля"
        q: * @choice_2 * ||toState = "/Валюта_покупка|продажа"
        q: * @choice_3 * ||toState = "/Валюта_неполный лот"
        q: * @choice_4 * ||toState = "/Валюта_стоимость хранения"
    ...
    state: Валюта_покупка|продажа
        ...
        state: Ответ_Брокер
            a: Обращаем ваше внимание, В связи с введением ограничительных мер в отношении Московской биржи, с 13 июня приостановлено проведение биржевых торгов валютами доллар США, евро, и гонконгский доллар. Торговля валютной парой с китайским юанем в режиме today, а также открытие коротких позиций по валютным парам с юанем недоступны.
            a: Клиентам фина'м доступна покупка и продажа долларов США через сервис внебиржевых торгов.
            ...
            a: Хотите узнать подробнее о конвертации долларов на внебирже.
            script: 
                $context.session = {};
            q: @agree ||toState = "/Валюта_внебиржевая торговля"
            q: @disagree ||toState = "/Могу еще чем то помочь?"
            q: @repeat_please * ||toState = "."
            # final answer
    ...
    state: Валюта_стоимость хранения
        ...
        state: Ответ_Брокер
            a: С 17 июня, хранение валюты на брокерском счёте в Фина'м – бесплатно. Комиссия за хранение валюты также не взимается по счета'м Сегрегированный Global.
            a: Чем я могу еще помочь?
            script: 
                $context.session = {};
            q: @repeat_please * ||toState = "."
    ...
    state: Валюта_внебиржевая торговля
        intent!: /027 Валюта/Валюта_внебиржевая торговля  
        a: С 16 сентября клиентам фина'м доступна покупка и продажа долларов США на внебирже через торговые системы Фина'м трейд и Транза'к. Сделки с долларом США доступны от одного доллара, код инструмента – ЮэСДэ РУБ.
        a: Торговля данным инструментом доступна по счетам единая денежная позиция, включая счета ИИС; и не доступна – для Сегрегированных и Моно счетов.
        a: Для получения возможности торговать данным инструментом, пользователям и Фина'м трейд и Транза'к; нужно в личном кабинете в разделе Торговля, Информационно торговые системы; отвязать счёт от терминала Транза'к, и привязать заново, чтобы новый раздел по торговле валютой на внебирже появился в торговом терминале.
        a: Обращаем ваше внимание, отражение средней цены по новому инструменту ЮэСДэ РУБ появится с 21 сентября. На текущий момент возможно отображение отрицательного баланса валюты в связи с переходом на новый тип торговли иностранной валютой в рамках раздела o T C. 
        a: Баланс по позиции будет скорректирован, как сумма отрицательного и положительного значений, после обновления лимитов утром на следующий рабочий день. Также, продать доллары США можно в интернет-банке фина'м. Зачисление долларов США на банковские счета Банка фина'м – без комиссии. 
        a: Чем я могу еще помочь?
        script: 
            $context.session = {};
        q: @repeat_please * ||toState = "."
        ...
        # a: Соединить вас с менеджером для подачи заявки на покупку или продажу долларов на внебирже.
        # script:
        #     $session.operatorPhoneNumber =  '1000';
        #     # $context.session = {};
        # q: @agree ||toState = "/Оператор/Оператор по номеру"    
    ...
    state: Контакты  
        ...
        a: Какая информация о компании вас интересует? Написать в поддержку; адреса офисов; реквизиты компании; лицензии компании фина'м.
    ...
    state: Контакты_Почта
        ...
        state: Ответ_Брокер
            a: Служба технической поддержки работает в режиме 24 на 7. Написать сообщение, или направить документы, можно в чат с поддержкой брокера фина'м.
            a: Написать в чат можно в любом удобном месте; например, в торговой системе фина'м трейд, значок сообщения находится в левом вертикальном меню; а в личном кабинете, оранжевый значок сообщения в нижнем правом углу; и на сайте фина'м точка ру, значок сообщения находится в верхнем правом углу страницы сайта.
            a: Чем я могу еще помочь?
            script: 
                $context.session = {};
            q: @repeat_please * ||toState = "."
        ...
        state: Ответ_Банк
            a: Контактная информация и часы работы банка представлены на сайте фина'м точка ру. В разделе сайта Банк, контактная информация.
            a: Также можно написать в чат с технической поддержкой клиентов фина'м, на сайте фина'м точка ру, значок сообщения находится в верхнем правом углу страницы сайта.
            a: Чем я могу еще помочь?
            script: 
                $context.session = {};
            q: @repeat_please * ||toState = "."
        ...
        state: Ответ_Форекс
            a: Служба технической поддержки работает в режиме 24 на 7.  Контактная информация фина'м форекс представлена на сайте фина'м точка ру. В разделе сайта Форекс, контакты.
            a: Также можно написать в чат с технической поддержкой клиентов фина'м, на сайте фина'м точка ру, значок сообщения находится в верхнем правом углу страницы сайта.
            a: Чем я могу еще помочь?
            script: 
                $context.session = {};
        ...
        state: Ответ_УК
            a: Контактная информация Управляющей компании фина'м, представлены на сайте фина'м точка ру. В разделе сайта Управление активами, о компании. 
            a: Также можно написать в чат с технической поддержкой клиентов фина'м, на сайте фина'м точка ру, значок сообщения находится в верхнем правом углу страницы сайта.
            a: Чем я могу еще помочь?
            script: 
                $context.session = {};
    ...
    state: Контакты_Реквизиты
        ...
        state: Ответ_Брокер
            a: Реквизиты для перевода денежных средств и ценных бумаг на брокерские счета, можно найти в личном кабинете на сайте, фина'м точка ру. Авторизуйтесь в личном кабинете; В разделе Помощь, выберите Реквизиты для перевода средств.
            a: Чем я могу еще помочь?
            script: 
                $context.session = {};
            q: @repeat_please * ||toState = "."
        ...
        state: Ответ_УК
            a: Реквизиты Управляющей компании фина'м, для перевода средств по договорам доверительного управления, можно найти в личном кабинете на сайте, фина'м точка ру, в разделе помощь.
            a: Чем я могу еще помочь?
            script: 
                $context.session = {};
            q: @repeat_please * ||toState = "."
    ...
    state: Комиссии_Брокер
        ...
        a: Ознакомиться с описанием тарифных планов: Стратег, Инвестор, Единый дневной, Долгосрочный портфель, Единый консультационный, можно на сайте Фина'м точка ру. Для этого в верхней части страницы сайта выберите раздел Инвестиции, далее выберите раздел Тарифы.
        ...
        state: Комиссия_сделки
            a: Размер биржевых, брокерских, депозитарных или комиссий за кредитование, зависит от тарифного плана установленного по счету, рынка ценных бумаг, даты открытия счета.
            a: Ознакомиться с описанием тарифных планов: Стратег, Инвестор, Единый дневной, Долгосрочный портфель, Единый консультационный, можно на сайте Фина'м точка ру. 
        ...
        state: Комиссия_обслуживание
            a: Размер комиссии за обслуживание брокерского счета, зависит от выбранного тарифного плана, и уменьшается на размер брокерской комиссии, удержанной за операции, совершенные в течение месяца. Списание в последний календа'рный день месяца. Комиссии за обслуживание самых популярных тарифов.
            a: Обслуживание по тарифным планам, Стратег, и Долгосрочный портфель - бесплатно. Тарифный план Инвестор - 200 рублей в месяц. Тарифные планы, Дневной, Консультационный, и другие тарифы - 177 рублей в месяц.
    ...
    state: Комиссии_Форекс
        ...
        a: При торговле с Фина'м Форекс всегда выгодные спрэды, бесплатное обслуживание счёта, без комиссий за сделки. Обращаем ваше внимание на условия торговли, такие как спрэд, то есть разница покупки и продажи.
    ...
    state: Обучение
        ...
        q: * @learning_ITS_u * ||toState = "/Обучение_ИТС"
        q: * @learning_online_u * ||toState = "/Обучение_онлайн"
        q: * @learning_offline_u * ||toState = "/Обучение_офлайн"
    ...
    state: Overnight_REPO
        ...
        a: Хотите получить информацию по за'йму брокером Ценных бумаг?
    ...
    state: КВАЛ_документы
        ...
        state: Ответ_Брокер
            ...
            state: Оборот
                ...
                a: Отправить документы можно в чате с поддержкой клиентов.
            ...
            state: Активы
                ...
                a: Чтобы получить статус квалифицированного инвестора по сумме активов более шести миллионов, нужно предоставить соответствующие документы в чате с поддержкой клиентов.
            ...
            state: Образование
                ...
                a: Чтобы получить статус квалифицированного инвестора по образованию или квалификации, нужно предоставить соответствующие документы в чате с поддержкой клиентов.
            ...
            state: Работа
                ...
                a: Чтобы получить статус квалифицированного инвестора по опыту работы, нужно предоставить соответствующие документы в чате с поддержкой клиентов.
    ...
    state: КВАЛ_перенос
        ...
        state: Ответ_Брокер    
            a: Чтобы перенести статус квалифицированного инвестора от другого брокера, нужно предоставить в чате с поддержкой клиентов, заверенную выписку из реестра квалифицированных инвесторов.
    ...
    state: Документы для налоговой
        ...
        state: Ответ_Брокер
            a: Для оформления налоговой декларации 3-НДФЛ через налоговый орган, Вам могут потребоваться документы от брокера, которые легко заказать в личном кабинете на сайте, фина'м точка ру. Авторизуйтесь в личном кабинете. В разделе Услуги, перейдите в раздел Налоги выписки справки, и  выберите нужное.
    ...
    state: Методика расчета ндфл
        ...
        state: Ответ_Брокер
            ...
            a: Подробнее про методику расчёта НДФЛ можно прочитать в личном кабинете на сайте, Фина'м точка ру, в разделе, Помощь, Инструкции шаблоны, расчет ндфл.
            a: Чем я могу еще помочь?
            script: 
                $context.session = {};
            q: @repeat_please * ||toState = "."
    ...
    state: Налоговое резидентство
        ...
        state: РФ
            ...
            a: Если вы хотите стать налоговым не' резидентом РФ в брокере фина'м, то нужно предоставить в электронном виде, в чате с поддержкой клиентов, скан-копии всех страниц загранпаспорта или документ подтверждающий статус налогового резидента в другой стране.
    ...
    state: Доступные биржи
        ...
        state: Форекс
            a: Фина'м Форекс предоставляет возможность торговли более чем с двадцатью видами валютных пар. При торговле с Фина'м Форекс всегда выгодные спрэды, и отсутствуют комиссии за сделки и обслуживание счёта.
        ...
        state: Американский рынок
            ...
            a: Все расчеты производятся в долларах США, автоконвертация валюты при покупке не осуществляется. Торги бумагами на иностранных биржах проходят в режиме Т+1, то есть расчеты проходят на следующий день после заключения сделки.
        ...
        state: Гонконгская биржа
            a: На бирже Гонконга доступно только закрытие позиций через отдел голосового трейдинга; Закрытие позиций в торговых системах недоступно.
        ...
        state: Внебиржевой рынок
            a: Фина'м предоставляет сервис по продаже и покупке на Московской бирже иностранных ценных бумаг, ранее заблокированных европейскими депозитариями Euroclear и Clearstream.
            a: В рамках сервиса заблокированные ИЦБ представляют собой торговый инструмент с тикером, состоящим из оригинального торгового кода бумаги и окончания, MM Бэ Зэ.
            a: Торги доступны в дни работы биржи с 11:00 до 17:00 по московскому времени, через торговые системы фина'м трейд и ТРАНЗА'К.
    ...
    state: Время торгов
        ...
        state: Московская биржа
            ...
            state: Срочная секция
                a: Торговая сессия начинается вечером и длится с 19:05 до 23:50, и продолжается на следующий день с 9:50 до 14:00 и с 14:05 до 18:50 по московскому времени. Клиринг проходит с 14 до 14:05, и с 18:50 до 19:05.
            ...
            state: Валютная секция
                a: Торги драгоценными металлами в режиме TOM проводятся с 9:50 до 19:00 по московскому времени. Торги валютными парами в режиме SPТ, ТОМ, и ТМС проводятся с 9:50 до 19:00 по московскому времени.
                ...
                a: То есть торговая сессия для пар в режиме TOD начинается с 9:50, а заканчивается в зависимости от валютной пары.
                a: Па'ры с белорусским рублём, турецкой лирой, и казахстанским тенге' к рублю эРэФ, торгуются до 11:45 по московскому времени.
                a: Чем я могу еще помочь?
                script:
                    # SessionClearing036();
                    $session.exchanges = undefined;
    ...
    state: Режим расчетов
        ...
        state: Московская биржа
            ...
            a: То есть у валютной пары с окончанием ТОД, расчёты проходят день в день; с окончанием ТОМ, или ТэМээС, расчёты пройдут на следующий день; и у пары с окончанием эСПэТэ, расчеты пройдут через день после сделки.
        ...
        state: Американский рынок
            a: Торги ценными бумагами на американских биржах проходят в режиме Т+1, то есть расчеты проходят на следующий день после заключения сделки.
            a: Чем я могу еще помочь?
            script:
                $session.exchanges = undefined;
                $session.section = undefined;
        ...
        state: Гонконгская биржа
            a: Торги ценными бумагами на бирже Гонконга проходят в режиме Т+2, то есть расчеты проходят через день после заключения сделки.
            a: Чем я могу еще помочь?
            script:
                $session.exchanges = undefined;
                $session.section = undefined;
        ...
        state: Ответ_Брокер
            ...
            script:
                if (identificationAO($client.profileCRM) == "success"){
                    ...
                    $session.insrtuctionText = "В продолжение разговора направляю вам [инструкцию, как пополнить Сегрегированный счёт|https://www.finam.ru/dicwords/file/files_chatbot_rekvizityglobal]";
                    $reactions.transition("/Отправка инструкции в чат");
                }
            a: Чем я могу еще помочь?
            script: 
    ...
    state: Сегрегированные счета_Вывод ДС_ЦБ
        ...
        state: Ответ_Брокер
            ...
            a: Вывод и зачисление средств на счета банка фина'м - без комиссии; комиссия за хранение валюты в банке от 3000 единиц валюты. 
    ...
    state: Дивиденды_Дивидендный календарь
        ...
        state: Повторное уточнение актива
            a: Правильно ли я Вас услышал. Вы назвали акцию {{$session.list_stocks.name}} ?
    ...
    state: Обезличенные сделки
        ...
        a: Подключить или отключить поток обезличенных сделок можно в личном кабинете на сайте фина'м точка ру. Авторизуйтесь в личном кабинете.
    ...
    state: Заявки_Покупка - Продажа
        ...
        state: Уточнение актива
            ...
            state: LocalCatchAll
                event: noMatch
                a: Возможно, я не так вас понял. Уточните вид актива, с которым планируете торговые операции.       
        ...
        state: Ценные бумаги
            ...
            state: Заблокированные ЦБ
                ...
                a: Перед совершением сделок нужно подписать Согласие на торговые операции с заблокированными бумагами. Для этого, в личном кабинете на сайте фина'м точка ру, в разделе Услуги, выберите пункт Операции с ценными бумагами, далее выберите Торговые операции с заблокированными ИЦБ.
    ...
    state: Заявки_Ошибки
        ...
        state: Доступное количество в портфеле 0
            a: При продаже уже рассчитанных и поставленных валютных пар, или металлов, с помощью контрактов ТОМ или ТэМээС, в портфеле может отображаться количество ноль, при этом выставление заявки будет доступно. Поставленные валюты и металлы отображаются как контракты ТОД или ТэМээС.
            ...
            a: Для продажи валюты, доллар США, евро, или юань, в количестве менее 1000 единиц, то есть менее одного лота валюты, можно использовать инструменты с окончанием ТэМээС.
            a: Чем я могу еще помочь?
            script: 
                $context.session = {};
            q: @repeat_please * ||toState = "."
    ...
    state: Заявки_Виды заявок
        ...
        state: Защитный спрэд и уровень коррекции
                a: Можно увеличить вероятность совершения сделки при исполнении тэйка, задав условие, Защитный спрэд, либо использовав галочку, Рыночная. Если указать ноль, в поле Защитный спрэд, то на Биржу будет отправлена заявка с ценой, равной цене первой сделки на рынке, которая удовлетворяет цене активации.
    ...
    state: Финам бонус
        ...
        a: Участвуйте в акции фина'м бонус. Пройдите регистрацию, и получи'те 500 приветственных бонусов. До конца сентября 2024 года, накопите 3000 бонусов за использование сервисов Фина'м, и получи'те их на брокерский счет.
        ...
        a: Обращаем ваше внимание, Бонусный баланс в кабинете участника акции, обновляется один раз в сутки. Вознаграждение за участие в акции, с июля по сентябрь 2024 года, будет выплачено до 15 октября 2024 года, если участник не предпочтёт обменять бонусы на образовательный онлайн курс.
    ...
    state: Подписание документов_СМС подпись
        ...
        state: Ответ_Брокер
            ...
            a: Настроить СМС подпись можно в Личном кабинете, на сайте фина'м точка ру; в разделе Сервис. Услуга предоставляется бесплатно.
            a: Чем я могу еще помочь?
            script: 
                $context.session = {};
            q: @repeat_please * ||toState = "."
    ...
    state: Подписание документов_ЭЦП
        ...
        state: Ответ_Брокер
            ...
            a: Чтобы скачать плагин, авторизуйтесь в личном кабинете на сайте фина'м точка ру, и перейдите в раздел Помощь, далее выберите пункт, Инструкции шаблоны ПэО. Скачайте Плагин и выполните настройку.
            a: После установки плагина для генерации электронной подписи, можно приступить к ее выпуску. Для этого, в личном кабинете, на сайте фина'м точка ру, перейдите в раздел Сервис; далее выберете, Электронная подпись, Создание ключей, Создать сертификат.
    ...
    state: Разблокировка ЦБ_Обмен по указу 844
        ...
        a: 12 августа, Фина'м получил данные от организатора торгов ООО Инвестиционная палата, по первому этапу обмена заблокированных иностранных ценных бумаг российских инвесторов в рамках Указа номер 844. Денежные средства по исполненным заявкам зачислены на счета инвесторов.
        a: Зачисления отображены в личном кабинете в разделе Отчетность – История операций -  Брокерские операции; и в справке по счёту, за 12 августа.
        a: Посмотреть в справке по счету зачисление средств и по какому активу прошел обмен, можно в разделе справки «Торговые движения ценных бумаг и иностранных финансовых инструментов, не квалифицированных в качестве ценных бумаг», с пометкой «Оферта на продажу ИЦэБэ».
        a: Обратите внимание, организатор торгов мог удовлетворить заявку инвестора частично, или не удовлетворить совсем. Тем не менее, заявки на обмен поданные в рамках первого этапа, ещё действуют и могут быть акцептованы организатором торгов в период до 12 октября 2024 года.
        a: При необходимости, в период со второго по 13 сентября, инвестор может отозвать ранее поданные заявки на обмен, обратившись к менеджеру поддержки. Также, важно учитывать, что случае перемещения ценных бумаг между счетами, в обмене будет отказано, так как поручение подано по месту хранения бумаг.
        a: Чем я могу еще помочь?
        q: @repeat_please * ||toState = "."
        q: @disagree ||toState = "/Могу еще чем то помочь?/NO"
        # final answer
    ...
    state: Яндекс
        intent!: /058 Яндекс
        a: Для держателей акций Yandex, приобретенных на Московской и СПБ Биржах, был проведен биржевой обмен акций. Биржевой обмен проходил через проведение сделок продажи и покупки бумаг на бирже. Комиссии за сделки взимались в соответствии с тарифным планом.
        a: Если комиссия списалась в минус, в валюте доллар США, то нужно пополнить счёт рублями на недостающую сумму для погашения минуса, и обратиться к менеджеру поддержки для конвертации валютного минуса. Информация для держателей ценных бумаг Yandex эН ВЭ, которые не попали под обмен;
        a: С 29 августа вступил в силу пункт регламента брокерского обслуживания об условном поручении, в соответствие с которым, брокером будет заключена сделка РЭПО' с данными акциями и дальнейшее перемещение их на зарубежную площадку.
        a: Данная процедура позволит, в момент возобновления листинга яндекс на бирже На'сдак, предоставить клиентам фина'м возможность продать их на биржевых торгах. Для перемещения бумаг на иностранную биржу, у держателя акций должен быть статус квалифицированного инвестора.
        a: Комиссия за совершение сделки РэПО' – 11 рублей 80 копеек, комиссия за внутридепозитарный перевод бумаг – 150 рублей.
        a: Чем я могу еще помочь?
        q: @repeat_please * ||toState = "."
        q: @disagree ||toState = "/Могу еще чем то помочь?/NO"
        # final answer
    ...
    state: Кодовое слово
        intent!: /060 Кодовое слово
        a: С 5 сентября 2024 года, для идентификации клиентов при обращении в компанию фина'м, а также при совершении голосовых торговых операций, требуется персональное кодовое слово. Создать или изменить кодовое слово, можно в личном кабинете на сайте фина'м точка ру; авторизуйтесь в личном кабинете; в верхнем правом углу нажмите на значок персоны, и перейдите в раздел Персональные данные, нажав на ваше ФИО.
        script:
            if (identificationAO($client.profileCRM) == "success"){
                $context.session.lastState = $context.currentState;
                $session.questionText = "Хотите я направлю ссылку на раздел Учётные данные, вам в чат?";
                $session.insrtuctionText = "С 5 сентября 2024 года для идентификации клиентов при обращении в компанию «Финам», а также при совершении голосовых торговых операций требуется персональное кодовое слово. [Создать или изменить свое кодовое слово в личном кабинете|https://lk.finam.ru/Client/CodeWords]";
                $reactions.transition("/Отправка инструкции в чат");
            }
        a: Чем я могу еще помочь?
        script: 
    ...
    # state: Элемент
    #     intent!: /061 Элемент 
    #     a: Размещение ценной бумаги ПАО Элемент, состоялось 30 мая. После размещения ПАО Элемент с местом хранения в Национальном расчетном депозитарии эНэРДэ', зачисление ценных бумаг на счета инвесторов прошло 31 мая.
    #     a: Обращаем ваше внимание, в эНэРДэ' данный актив доступен для хранения; торговые операции недоступны, так как торги ПАО Элемент проходят только на СПБ Бирже. Открытие новых позиций по ценным бумагам на СПБ бирже недоступно, возможно только закрытие позиций.
    #     a: Поставленные бумаги не отображаются в торговой системе, но их наличие отражено в личном кабинете брокера во вкладке Портфель.
    #     a: При необходимости продать актив, его можно перевести из эНэРДэ' в депозитарий СПБ; перевод доступен в личном кабинете, фина'м точка ру, в разделе Услуги; Операции с ценными бумагами; Перемещение ценных бумаг.
    #     a: За перевод между торговыми площадками взимается комиссия согласно депозитарному тарифу; 365 ₽ по депозитарному тарифу номер один, дополнительно может удерживаться комиссия за обслуживание счёта депо, если это первое движение по счёту в течение месяца; 715 ₽ по депозитарному тарифу номер два.
    #     # script:
    #     #     if (identificationAO($client.profileCRM) == "success"){
    #     #         $context.session.lastState = $context.currentState;
    #     #         $session.questionText = "Хотите я направлю подробную информацию вам в чат?";
    #     #         $session.insrtuctionText = "Размещение ценной бумаги ПАО «Элемент» состоялось 30 мая 2024 года. Начисление ценных бумаг ПАО «Элемент» с местом хранения в НРД на счета инвесторов прошло 31 мая. В НРД актив доступен для хранения, торговые операции недоступны, так как торги ПАО «Элемент» проходят только на СПБ Бирже. Открытие новых позиций по ценным бумагам на СПБ бирже недоступно, только закрытие позиций. Поставленные бумаги не отображаются в торговых системах, но их наличие отражено во вкладке «Портфель» в [личном кабинете|https://lk.finam.ru/]. При необходимости продать актив, совершить его перевод из НРД в депозитарий СПБ можно в личном кабинете в разделе [«Перемещение ценных бумаг».|https://edox.finam.ru/Orders/ChangeSecurityPlace] Комиссия за перевод между торговыми площадками: 365 ₽ по депозитарному тарифу №1 (дополнительно может удерживаться комиссия за обслуживание счёта депо, если это первое движение по счёту в течение месяца), 715 ₽ по депозитарному тарифу №2.";
    #     #         $reactions.transition("/Отправка инструкции в чат");
    #     #     }
    #     a: Чем я могу еще помочь?
    #     script: 
    ...
    state: Престиж
        intent!: /062 Престиж 
        a: Статус Престиж предоставляется для клиентов с активами от 1 миллиона рублей. И даёт набор эксклюзивных привилегий, таких как. Персональный менеджер. Выделенная линия поддержки 24 на 7, с приоритетным обслуживанием в чате и по телефону. 
        a: Индивидуальный подбор тарифа. Расширенная аналитическая поддержка. Индивидуальные инвестиционные решения. А так же Бесплатный доступ к платным обучающим мероприятиям.
        script:
            if (identificationAO($client.profileCRM) == "success"){
                $context.session.lastState = $context.currentState;
                $session.questionText = "Хотите получить иллюстрированную инструкцию по пополнению счёта в чат?";
                $session.insrtuctionText = "В продолжение разговора направляю вам инструкцию на [пополнение брокерского счета в личном кабинете|https://www.finam.ru/dicwords/file/files_chatbot_instrukciyapopopolneniyuscheta]";
                $reactions.transition("/Отправка инструкции в чат");
            }
        a: Чем я могу еще помочь?
        script: 
    ...
    state: Отмена поручений
        intent!: /063 Отмена поручений
        # a: Перевод на оператора по отмене поручений
        script:
            $session.operatorPhoneNumber = '2222';
            $reactions.transition("/Оператор/Оператор по номеру");
        
    state: Отмена поручений по выводу ДС
        intent!: /063 Отмена поручений/Отмена поручений на вывод ДС
        # a: Перевод на оператора по отмене поручений на вывод ДС
        script:
            $session.operatorPhoneNumber = '2222';
            $reactions.transition("/Оператор/Оператор по номеру");

    state: Отмена поручений по выводу ЦБ
        intent!: /063 Отмена поручений/Отмена поручений на вывод ЦБ
        # a: Перевод на оператора по отмене поручений на вывод ЦБ
        script:
            $session.operatorPhoneNumber = '2222';
            $reactions.transition("/Оператор/Оператор по номеру");
    ...
    state: Оператор
        ...
        state: Оператор по номеру
            ...
            elseif: $session.operatorPhoneNumber == '2200'
                a: Для выставления заявки, подготовьте и назовите трейдеру ваше кодовое слово, которое установлено в разделе личного кабинета Персональные данные. Перевожу вас на {{ $session.departmentName }}.
    ...
    state: NoChoice || noContext = true
        q!: @no_choice
        script:
            var regexp = /(уточнение|MoneyTransferGetCompany|MoneyTransferGetAssetType|MoneyTransferGetType|MoneyTransferGetBetweenAccounts|MoneyTransferGetMethod)/gi;
            var result = $context.contextPath.match(regexp);
            
            if(result == undefined) {
            $reactions.transition("/NoMatch");
            }
        a: Позвольте вам помочь; пожалуйста, выберите один из предложенных вариантов.
        script:
            $reactions.transition($context.contextPath);        
